...
def _convert_order(client, X, chunksizes, order, n_features, dtype):
...
def make_regression(n_samples=100, n_features=100, n_informative=10,
    ...
    if effective_rank is None:
        ...
        X = _convert_order(client, X, data_chunksizes, order,
                           n_features, dtype)
    ...
    if order == 'F' and n_targets > 1:
        y = _convert_order(client, y, y.chunks[0], order, n_targets, dtype)
        if coef:
            ground_truth = _convert_order(client, ground_truth,
