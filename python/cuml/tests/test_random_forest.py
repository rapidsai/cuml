...
def test_unlimited_max_depth_classifier(max_depth):
    X, y = make_classification(n_samples=500, n_features=10, random_state=42)

    clf = curfc(n_estimators=10, max_depth=max_depth, random_state=42)
    clf.fit(X, y)
    preds = clf.predict(X)
    ...
    assert clf2.get_params()["max_depth"] == max_depth
...
@pytest.mark.parametrize("max_depth", [-1, None])
def test_unlimited_max_depth_regressor(max_depth):
    X, y = make_regression(n_samples=500, n_features=10, random_state=42)

    reg = curfr(n_estimators=10, max_depth=max_depth, random_state=42)
    reg.fit(X, y)
    ...
    assert reg2.get_params()["max_depth"] == max_depth
