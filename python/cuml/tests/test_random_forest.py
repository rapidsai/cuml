...
def test_rf_classification_sparse(small_clf, datatype, nvforest_layout):
    ...
    nvforest_model = cuml_model.as_nvforest(layout=nvforest_layout)
...
def test_rf_regression_sparse(special_reg, datatype, nvforest_layout):
    ...
    nvforest_model_preds = cp.reshape(
        nvforest_model.predict(X_test), np.shape(y_test)
    )
    nvforest_model_r2 = r2_score(cp.asarray(y_test), nvforest_model_preds)
...
def test_rf_classifier_class_weight_matches_sklearn(small_clf, class_weight):
    ...
    cu_pred = cu_clf.predict(X)
    assert cu_pred.shape == (len(y),)
    if X.shape[0] < 500000:
        sk_clf = skrfc(
            n_estimators=50, random_state=42, class_weight=class_weight
        ).fit(X, y)
        cu_score = accuracy_score(y, cu_pred)
...
def test_rf_regressor_sample_weight_matches_sklearn(large_reg):
    ...
    cu_pred = cu_reg.predict(X)
    assert cu_pred.shape == (len(y),)
    assert np.all(np.isfinite(cu_pred))
    if X.shape[0] < 500000:
...
def test_rf_feature_importances_weighted_tracks_weighting(small_clf):
    ...
    if X.shape[0] < 500000:
        sk_w = skrfc(n_estimators=50, random_state=42).fit(
            X, y, sample_weight=sw
        )
...
def test_rf_classifier_score_uses_sample_weight(small_clf):
    ...
    pred = clf.predict(X)
    # Flip 10 correctly-predicted labels and weight them 50x; weighted
    # accuracy must drop below unweighted. Picking from `pred == y` keeps
    # the test deterministic when leading rows are mispredicted.
    y_eval = y.copy()
    wrong = np.flatnonzero(pred == y)[:10]
    assert len(wrong) == 10
