...
@pytest.mark.parametrize("model", [cuml.LinearRegression,
                                   ...
                                   cuml.SVR])
def test_permutation_gpu_cpu_shap(dtype, nfeatures, nbackground, model):
    if not has_shap():
    ...
    shap_values = explainer(X_test[0].reshape(1, 10))

    cu_explainer = \
        cuml.experimental.explainer.PermutationSHAP(model=model.predict,
                                                    masker=X_train,
                                                    gpu_model=True)
...
def test_cuml_models(single_dataset, model_name):
    ...
    cu_explainer = cuml.experimental.explainer.PermutationSHAP(
        model=mod.predict,
        data=X_train,
        gpu_model=True
    )
...
@pytest.mark.parametrize("model", [sklearn.linear_model.LinearRegression,
                                   sklearn.neighbors.KNeighborsRegressor,
                                   sklearn.svm.SVR])
def test_sklearn_models(model):
    X, y = make_regression(n_samples=81, n_features=10, noise=0.1,
                           random_state=42)
    ...
    cu_explainer = \
        cuml.experimental.explainer.PermutationSHAP(model=mod.predict,
                                                    masker=X_train,
                                                    gpu_model=True)
