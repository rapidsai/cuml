...
def test_elastic_net(input_type, selection):
	...
	elastic_sk = ElasticNet(alpha=np.array([0.001]), fit_intercept=True, 
	...
	elastic_cu = cuElasticNet(alpha=np.array([0.001]), fit_intercept=True, 
			normalize=False, max_iter=1000, 
			selection=selection, tol=1e-10)

	if input_type=='datafame':
	...
	assert array_equal(error_sk, error_cu, 1e-1, with_sign=True)
