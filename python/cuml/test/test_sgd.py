...
def test_svd(datatype, learning_rate, input_type, penalty,loss):
    ...
    sk_sgd = SGDClassifier(learning_rate='constant',eta0=0.005, max_iter=2000, tol=0.0,fit_intercept=True, penalty=penalty,loss=loss)
    sk_reg = sk_sgd.fit(X,y)
    ...
    cu_sgd = cumlSGD(learning_rate='constant', eta0=0.005, epochs=2000, fit_intercept=True, batch_size=2, tol=0.0, penalty=penalty,loss=loss)
    cu = cu_sgd.fit(X,y)
