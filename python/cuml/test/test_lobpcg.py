...
def test_lobpcg(dtype, k, n, method, largest, maxiter, isB):
    ...
    while(isB):
        ...
        # making sure B is not singular
        if(not math.isclose(np.linalg.det(B), 0, abs_tol=1e-04)):
            break
    ...
    try:  # (please work :3)
        ...
        with cp.prof.time_range(message='start', color_id=10):
            ...
            cp_val, cp_vec = cp_lobpcg(A_gpu_array, X=X_gpu_array,
                                       B=B_gpu_array, maxiter=maxiter,
                                       largest=largest, method=method)

            cp.cuda.Stream.null.synchronize()

        isnan_and_reduc_kernel = cp.ReductionKernel(
            'T x', 'T y', 'x == x', 'a and b', 'a', '1', 'isnan_and_reduc')
        ...
        print('''{} occured in running the cupy lobpcg method for method: {} k: {}
              and n: {} with maxiter: {}!'''.format(
                e,
                method,
                k,
                n,
    ...
    """
    try:
        scipy_val, scipy_vec = scipy_lobpcg(
            A, X, B=B, maxiter=maxiter, largest=largest)
    except BaseException:
        print(
            '''error occured in running the scipy lobpcg method for
    ...
    if(scipy_error_flag or cp_error_flag):
        ...
        warnings.warn(
            "cholesky failed for k:{}, n:{}, maxiter:{}".format(k, n, maxiter))
        return

    # taking absolutes of eigen vectors as they can toggle between positive
    # and negative
    ...
    cp_weighted_res = np.average(
        abs_cp_res, axis=1, weights=cp.asnumpy(cp_val))
    ...
    """
    TEST-STRATEGY--------------------
    The following is a comparison after norm-reduction
    (ie, (n,1) -> 1) of eigen-value-weighted-reduction (ie, (n,k) -> (n,1))
    of absolutes of resulting residuals
    """
    if cp_norm > scipy_norm:
        assert math.isclose(cp_norm, scipy_norm, abs_tol=1e-2,
                            rel_tol=1e-2), "cp_norm:{} scipy_norm:{}".format(
                            cp_norm, scipy_norm)
