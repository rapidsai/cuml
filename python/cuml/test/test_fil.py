...
def _build_and_save_xgboost(model_path, X_train, y_train, classification):
...
def test_fil_class(n_rows, n_columns, num_round, tmp_path):
    # settings
    ...
    bst = _build_and_save_xgboost(model_path, X_train, y_train, classification)
...
@pytest.mark.parametrize('num_round', [unit_param(20), quality_param(50),
                         stress_param(90)])
@pytest.mark.skipif(has_xgboost() is False, reason="need to install xgboost")
def test_fil_reg(n_rows, n_columns, num_round, tmp_path):
    # settings
