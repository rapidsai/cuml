...
def preprocess_labels(
    ...
    # Use cupy for supported dtypes, cudf otherwise
    if y.dtype.kind in "iufb":
        y = y.to_output("cupy")
    else:
        y = y.to_output(
            "cudf", output_dtype=np.dtype("O") if y.dtype.kind in "U" else None
        )
...
def decode_labels(y_encoded, classes, output_type="cupy"):
    ...
    if isinstance(classes, list):
        ...
        if dtype is not None and dtype.kind in "iufb":
            ...
            out = cudf.DataFrame(
                [
                    cudf.Series(c).take(y_encoded[:, i]).reset_index(drop=True)
