...
def make_final_proxy_type(
    ...
    *,
    ...
    @nvtx.annotate(
        "COPY_SLOW_TO_FAST",
        color=_CUML_ACCEL_NVTX_COLORS["COPY_SLOW_TO_FAST"],
        domain="cudf_pandas",
    )
    ...
    @nvtx.annotate(
        "COPY_FAST_TO_SLOW",
        color=_CUML_ACCEL_NVTX_COLORS["COPY_FAST_TO_SLOW"],
        domain="cudf_pandas",
    )
...
def _fast_slow_function_call(
    ...
    **kwargs,
    ...
    try:
        with nvtx.annotate(
            "EXECUTE_FAST",
            color=_CUML_ACCEL_NVTX_COLORS["EXECUTE_FAST"],
            domain="cudf_pandas",
        ):
            ...
            if _env_get_bool("CUDF_PANDAS_DEBUGGING", False):
                try:
                    with nvtx.annotate(
                        "EXECUTE_SLOW_DEBUG",
                        color=_CUML_ACCEL_NVTX_COLORS["EXECUTE_SLOW"],
                        domain="cudf_pandas",
                    ):
