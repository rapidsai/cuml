...
class ModuleAccelerator(ModuleAcceleratorBase):
    ...
    def _populate_module(self, mod: ModuleType):
        ...
        # Note (dgd): this doesn't work for resources.files(data_module)
    ...
    def install(
        ...
        with ImportLock():
            ...
            mode = deduce_cuml_accel_mode(slow_lib, fast_lib)
...
def _caller_in_denylist(calling_module, denylist):
    CUML_ACCELERATOR_PATH = __file__.rsplit("/", 1)[0]
    return not calling_module.is_relative_to(CUML_ACCELERATOR_PATH) and any(
