...
from cuml.internals import api_return_generic
...
IF GPUBUILD == 1:
    ...
    def fuzzy_simplicial_set_impl(X,
                                  n_neighbors,
                                  random_state=None,
                                  metric="euclidean",
                                  metric_kwds=None,
    ...
    def simplicial_set_embedding_impl(
        data,
        graph,
        n_components=2,
        initial_alpha=1.0,
...
def fuzzy_simplicial_set(X,
    ...
    def fallback(args, kwargs):
        from umap.umap_ import fuzzy_simplicial_set as reference_fuzzy_simplicial_set

        kwargs['return_dists'] = None
        result, sigmas, rhos = reference_fuzzy_simplicial_set(*args, **kwargs)
        return result.tocoo()
    ...
    if GPUBUILD:
        if is_cuda_available():
            return fuzzy_simplicial_set_impl(*args, **kwargs)
        else:
...
def simplicial_set_embedding(
    ...
    """
    def fallback(args, kwargs):
        from umap.umap_ import simplicial_set_embedding as reference_simplicial_set_embedding

        graph = args[1]
        if not isinstance(graph, sp.sparse.spmatrix):
            raise ValueError("Please use a scipy/numpy sparse array as the graph argument.")

        if isinstance(kwargs['random_state'], int):
        ...
        embedding, aux_data = reference_simplicial_set_embedding(*args, **kwargs)
        return embedding
    ...
    if GPUBUILD:
        if is_cuda_available():
            return simplicial_set_embedding_impl(*args, **kwargs)
        else:
            return fallback(args, kwargs)
    else:
        return fallback(args, kwargs)
