...
if [ "$BUILD_CUML" == '1' ]; then
  if [[ -z "$PROJECT_FLASH" || "$PROJECT_FLASH" == "0" ]]; then
    ...
    gpuci_conda_retry build --croot ${CONDA_BLD_DIR} -c ci/artifacts/cuml/cpu/.conda-bld/ -c rapidsai-nightly/label/testing --dirty --no-remove-work-dir conda/recipes/cuml --python=${PYTHON}
  fi
fi
