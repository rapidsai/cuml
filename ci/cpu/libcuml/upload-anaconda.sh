...
if [ "$BUILD_LIBCUML" == "1" ]; then
  ...
  if [ "$BUILD_ABI" == "1" ]; then
    export UPLOADFILE=`conda build conda/recipes/libcuml -c conda-forge -c numba -c nvidia/label/cuda${CUDA_REL} -c rapidsai/label/cuda${CUDA_REL} -c pytorch -c defaults --python=${PYTHON}`
  else
    export UPLOADFILE=`conda build conda/recipes/libcuml -c conda-forge/label/cf201901 -c numba -c nvidia/label/cf201901-cuda${CUDA_REL} -c rapidsai/label/cf201901-cuda${CUDA_REL} -c pytorch -c defaults --python=${PYTHON}`
  fi
