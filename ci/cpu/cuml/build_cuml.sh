...
if [ "$BUILD_CUML" == '1' ]; then
  ...
  if [ "${CUDA:0:2}" == '10' ]; then
    ...
    CUDA_REL=${CUDA:0:4}
    if [ "$BUILD_ABI" == "1" ]; then
      conda build conda/recipes/cuml-cuda10 -c conda-forge -c numba -c rapidsai/label/cuda${CUDA_REL} -c nvidia/label/cuda${CUDA_REL} -c pytorch -c defaults --python=${PYTHON}
    else
  ...
  # CUDA 9 builds with the normal process
  else
    if [ "$BUILD_ABI" == "1" ]; then
