...
void loadDefaultDataset(std::vector<float>& inputData, size_t& nRows,
                        size_t& nCols, int& minPts, float& eps, size_t& max_elems)
{
    constexpr size_t NUM_ROWS = 25;
    constexpr size_t NUM_COLS = 3;
    constexpr int    MIN_PTS  = 2;
    constexpr float  EPS      = 1.0f;
    constexpr size_t MAX_ELEMS = (size_t)2e7;

    constexpr float data[NUM_ROWS*NUM_COLS] =
    {
        -7.497668f, 9.218568f, -4.924911f,
        8.001691f, -2.377415f, -3.496702f,
        -7.402899f, 9.162857f, -4.894407f,
        -7.590056f, 9.375731f, -4.762814f,
        7.822048f, -2.388025f, -3.403690f,
        -7.376115f, 9.441934f, -4.801385f,
        -7.531280f, 9.230399f, -4.763294f,
        8.042177f, -2.665680f, -3.316565f,
        7.944115f, -2.557312f, -3.185993f,
        7.922114f, -2.423922f, -3.194180f,
        7.897527f, -2.466402f, -3.311819f,
        -7.569343f, 9.266988f, -4.779115f,
        -7.528063f, 9.156666f, -4.887371f,
        -7.296247f, 9.187418f, -4.754778f,
        7.825963f, -2.351993f, -3.419239f,
        -7.608446f, 9.386856f, -4.750009f,
        8.087856f, -2.330975f, -3.392595f,
        -7.503101f, 9.391059f, -4.762857f,
        7.936867f, -2.410410f, -3.397487f,
        -7.565027f, 9.248172f, -5.000937f,
        -7.339392f, 9.317035f, -4.778559f,
        7.803362f, -2.304214f, -3.173147f,
        -7.510096f, 9.441537f, -4.718324f,
...
int main(int argc, char * argv[])
{
    ...
    size_t max_elems = get_argval<size_t>(argv, argv+argc, "-max_elems_per_batch", (size_t)2e7);

    {
        ...
        if ( cudaSuccess != cudaStatus )
        {
            std::cerr << "ERROR: Could not select CUDA device with the id: "
                      << devId << "(" << cudaGetErrorString(cudaStatus)
                      << ")" << std::endl;
            return 1;
        }
        ...
        if ( cudaSuccess != cudaStatus )
        {
            std::cerr << "ERROR: Could not initialize CUDA on device: "
                      << devId << "(" << cudaGetErrorString(cudaStatus)
                      << ")" << std::endl;
            return 1;
        }
    ...
    if(input == "")
    {
        ...
        loadDefaultDataset(h_inputData, nRows, nCols, minPts, eps, max_elems);
    }
    ...
    else
    {
        ...
        std::cout << "Trying to read samples from " 
                  << input << std::endl;
    ...
    CUDA_RT_CALL( cudaMemcpyAsync(d_inputData, h_inputData.data(), 
                    nRows*nCols*sizeof(float), cudaMemcpyHostToDevice, stream) );
    ...
    ML::dbscanFit(cumlHandle, d_inputData, nRows, nCols, eps, minPts, d_labels, max_elems);
