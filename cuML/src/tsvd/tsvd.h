...
void tsvdFit(const cumlHandle_impl& handle, math_t *input, math_t *components,
    ...
    Matrix::seqRoot(explained_var_all.data(), singular_vals, scalar, prms.n_components);
}
...
void tsvdFitTransform(const cumlHandle_impl& handle, math_t *input,
    ...
    device_buffer<math_t> mu_trans(handle.getDeviceAllocator(), handle.getStream(),
                                   prms.n_components);
    ...
    device_buffer<math_t> mu(handle.getDeviceAllocator(), handle.getStream(),
                             prms.n_cols);
    device_buffer<math_t> vars(handle.getDeviceAllocator(), handle.getStream(),
                               prms.n_cols);

    Stats::mean(mu.data(), input, prms.n_cols, prms.n_rows, true, false);
    Stats::vars(vars.data(), input, mu.data(), prms.n_cols, prms.n_rows, true, false);

    device_buffer<math_t> total_vars(handle.getDeviceAllocator(), handle.getStream(), 1);
