namespace UMAP {
	namespace FuzzySimplSet {
		...
		namespace Naive {
			 ...
			 */
			template<typename T>
			void smooth_knn_dist(const T *knn_dists, int n, int k,
				 ...
				 *
				 * 		    psum = 0.0
				 * 		    loop through columns j of dist matrix:
			...
			void compute_membership_strength(const T *knn_indices, const T *knn_dists,
				...
				/**
				 * Nested loop: samples then neighbors
				 *
			...
			void launcher(const T *X, int n, int n_neighbors,
				...
				/**
				 * - result = <Create coo matrix>
				 * - result = <Eliminate zeros from coo>
				 * - transpose = <Transpose result>
				 * - prod_matrix = <Multiply coo matrix by its transpose>
				 *
