...
namespace ML {
	namespace DecisionTree {
		...
		template<class T>
		void Question<T>::update(GiniQuestion<T> ques) {
		...
		template<typename T>
		void DecisionTreeClassifier<T>::fit(T *data, const int ncols, const int nrows, int *labels, unsigned int *rowids, const int n_sampled_rows, int unique_labels, int maxdepth, int max_leaf_nodes, const float colper, int n_bins, int split_algo)
			{
		...
		template<typename T>
		void DecisionTreeClassifier<T>::split_branch(T *data, GiniQuestion<T> & ques, const int n_sampled_rows, int& nrowsleft, int& nrowsright, unsigned int* rowids)
		{
		...
		template<typename T>
		void DecisionTreeClassifier<T>::print_node(const std::string& prefix, TreeNode<T>* node, bool isLeft)
		{
			if (node != NULL) {
				std::cout << prefix;
