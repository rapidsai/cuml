...
*/
float batch_evaluate_gini(const float *column, const int *labels, const float base_quesval, const float delta, const int batch_bins, int & batch_id, const int nrows, const int n_unique_labels, GiniInfo split_info[3], TemporaryMemory* tempmem) {
	...
	batch_evaluate_kernel<<< (int)(nrows /128) + 1, 128, n_hists_bytes, tempmem->stream>>>(column, labels, base_quesval, batch_bins, delta, nrows, n_unique_labels, dhist);
	CUDA_CHECK(cudaGetLastError());
	...
	for (int i = 0; i < batch_bins; i++) {
		...
		//separate loop for now to avoid overflow. FIXME
	...
	ASSERT(best_batch_id != -1, "Invalid best_batch_id"); // To ensure if-statement in loop above executed. Otherwise dummy gini values.
