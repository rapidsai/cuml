...
void pcaInverseTransform(cumlHandle& handle, float *trans_input, float *components,
    ...
    pcaInverseTransform(handle.getImpl(), trans_input, components, singular_vals, mu, input,
                        prms, handle.getStream());
}
...
void pcaTransform(cumlHandle& handle, float *input, float *components, float *trans_input,
    ...
    pcaTransform(handle.getImpl(), input, components, trans_input, singular_vals, mu,
                 prms, handle.getStream());
}
