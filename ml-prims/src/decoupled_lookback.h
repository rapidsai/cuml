 ...
 */
template <typename Type> struct DecoupledLookBack {
  /** default ctor */
...
private:
  ...
  DI void sumDone(Type sum) {
    ...
    if (isLast() && blockIdx.x == 0)
      myFlag->incl_prefix = sum;
    // make sure that sum never crosses flag update!
    __threadfence();
    if (isLast())
      myFlag->status = blockIdx.x == 0 ? 2 : 1;
    __threadfence();
  }
