...
CUTLASS_HOST_DEVICE void extract_index_from_iterator(OutputIterator& iterator,
  ...
  for (int d = 0; d < OutputIterator::Iterations::kD; ++d) {
    for (int h = 0; h < OutputIterator::Iterations::kH; ++h) {
      for (int w = 0; w < OutputIterator::Iterations::kW; ++w) {
        ...
        index[st++] = iterator.valid(d, h, w, 0)?  (&iterator.params.pointer[imm] - base_ptr) : -1;
...
struct DistanceGemmEpilogue {
  ...
  typedef cutlass::Copy<typename SharedLoadIteratorD::Fragment> SharedLoadTransformerD;
  ...
  CUTLASS_DEVICE void epilogue(cutlass::Coord<3> const& block, Accumulators& accumulators) {
    if (cutlass::gemm::is_zero(params.functor.beta)) {
      if(params.functor.enable_sqrt)
        epilogue_with_or_without_beta<true, true>(block, accumulators);
      else
        epilogue_with_or_without_beta<true, false>(block, accumulators);
    } else {
      if(params.functor.enable_sqrt)
        epilogue_with_or_without_beta<false, true>(block, accumulators);
      else
        epilogue_with_or_without_beta<false, false>(block, accumulators);
    }
  }

  template <bool kBetaIsZero_, bool enable_sqrt>
  CUTLASS_DEVICE void epilogue_with_or_without_beta(cutlass::Coord<3> const& block,
                                                    Accumulators& accumulators) {

    // The problem size.
    cutlass::Coord<3> const bounds = cutlass::make_Coord(0, n, m);
    ...
    cutlass::Coord<3> const bb_bounds = cutlass::make_Coord(0, 1, m);
    // The BB block size.
    ...
    typename GlobalStoreIteratorD::Pointer global_base_ptr = params.iterator_d.pointer;
    ...
    for (int h = 0; h < Iterations::kH; ++h) {
      ...
      cutlass::Coord<3> const aa_bounds = cutlass::make_Coord(0, n, 1);
      // The AA block size.
      ...
      int const aa_pointer_offset =
          ((params.functor.iterator_aa.inc_h * (GlobalLoadIteratorAA::Iterations::kH - 1) +
            params.functor.iterator_aa.inc_advance) *
               Iterations::kW +
           Traits::Delta::kH) *
          h;
      int const aa_predicate_offset =
          ((params.functor.iterator_aa.predicate_inc_h * (GlobalLoadIteratorAA::Iterations::kH - 1) +
            params.functor.iterator_aa.predicate_inc_advance) *
               Iterations::kW +
           Traits::Delta::kH) *
          h;
      // The iterator to load the elements of the AA column vector.
      GlobalLoadIteratorAA global_load_iterator_aa(
          params.functor.iterator_aa,
          aa_bounds,
          aa_block,
          aa_pointer_offset,
          aa_predicate_offset);
      ...
      for (int w = 0; w < Iterations::kW; ++w) {
        ...
        extract_index_from_iterator(global_store_iterator, global_base_ptr, index);
        ...
        if (kBetaIsZero_) {
          functor.evaluate<enable_sqrt>(fetched_d, fragment_d, index, fragment_aa, fragment_bb);
        } else {
          // Transform C fragment.
          transformer_c.transform(fragment_c, transformed_c);
          // Do the math.
          functor.evaluate<enable_sqrt>(fetched_d, transformed_c, fragment_d, index, fragment_aa, fragment_bb);
        }
