...
void distance(InType *x, InType *y, OutType *dist, int m, int n, int k,
              DistanceType type, void *workspace, size_t &worksize,
...
void distance(InType *x, InType *y, OutType *dist, int m, int n, int k,
              DistanceType type, void *workspace,
              size_t &worksize, cudaStream_t stream = 0) {
  auto default_fin_op =
      [] __device__(AccType d_val, int g_d_idx) { return (OutType)d_val; };
  distance<InType, AccType, OutType, OutputTile_>(
    x, y, dist, m, n, k, type, workspace, worksize, default_fin_op, stream);
}
