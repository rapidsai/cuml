...
/**
 * @defgroup square root math operation on the input matrix. Square root of
 * every element in the input matrix
 ...
 * @param scalar: every element is multiplied with scalar
 * @param len: number elements of input matrix
 * @param setzero: (default false) when true and |value|<thres, avoid dividing
 * by (almost) zero
 * @param thres: Threshold to avoid dividing by zero (|value| < thres -> result
 * = 0)
 * @{
 */
template <typename math_t, typename IdxType = int>
...
template <typename math_t, typename IdxType = int>
void ratio(math_t *src, math_t *dest, IdxType len, DeviceAllocator &mgr,
           cudaStream_t stream) {
  auto d_src = src;
  auto d_dest = dest;
  ...
  MLCommon::LinAlg::unaryOp(
      d_dest, d_src, len, [=] __device__(math_t a) { return a / (*d_sum); },
      stream);
