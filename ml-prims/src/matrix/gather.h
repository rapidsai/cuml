...
// gatherKernel conditionally copies rows from the source matrix 'in' into the
// destination matrix 'out' according to a map (or a transformed map)
 ...
 *
 * @tparam MatrixIteratorT      Random-access iterator type, for reading input
 * matrix (may be a simple pointer type).
 * @tparam MapIteratorT         Random-access iterator type, for reading input
 * map (may be a simple pointer type).
 * @tparam StencilIteratorT     Random-access iterator type, for reading input
 * stencil (may be a simple pointer type).
 ...
 * MapTransformOp's result type must be convertible to IndexT (= int) type.
 *
 * @param  in           Pointer to the input matrix (assumed to be row-major)
 * @param  D            Leading dimension of the input matrix 'in', which
 * in-case of row-major storage is the number of columns
