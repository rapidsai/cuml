 ...
 * - It does not need to be but it can allow asynchronous allocate and
 * deallocate.
...
public:
   ...
   * @param[in] stream    stream to issue the possible asynchronous allocation
   * in
   * @returns a pointer to a n byte properly aligned device buffer on the
   * configured device.
   */
...
public:
   ...
   *
   * An implementation of this need to return a allocation of n bytes properly
   * align bytes on the host. The allocation can optionally be asynchronous in
   * the sense that it is only save to use after all work submitted to the
   * passed in stream prior to the call to allocate has completed. If the
   * allocation is used before, e.g. in another stream the behaviour may be
   * undefined.
   * @todo: Add alignment requirments.
   *
   * @param[in] n         number of bytes to allocate
