...
/**
 * RAII object owning a contigous typed host buffer. The passed in allocator
 * supports asynchronus allocation and deallocation so this can be used for
 * temporary memory
 * @code{.cpp}
 * template<typename T>
 * void foo( const cumlHandle_impl& h, const T* in_d , T* out_d, ...,
 * cudaStream_t stream )
 * {
 *     ...
 ...
 *     cudaMemcpyAsync( temp.data(), in_d, temp.size()*sizeof(T),
 * cudaMemcpyDeviceToHost );
 *     ...
 *     cudaMemcpyAsync( out_d, temp.data(), temp.size()*sizeof(T),
 * cudaMemcpyHostToDevice ); temp.release(stream);
 * }
 * @endcode
 * @todo: Add missing doxygen documentation
 */
template <typename T> class host_buffer : public buffer_base<T> {
public:
  ...
  host_buffer(std::shared_ptr<hostAllocator> allocator, cudaStream_t stream,
    ...
    if (_capacity > 0) {
      _data = static_cast<value_type *>(
          _allocator->allocate(_capacity * sizeof(value_type), get_stream()));
  ...
  ~host_buffer() {
    if (nullptr != _data) {
      _allocator->deallocate(_data, _capacity * sizeof(value_type),
                             get_stream());
    }
  }
  ...
  void reserve(const size_type new_capacity, cudaStream_t stream) {
    ...
    if (new_capacity > _capacity) {
      value_type *new_data = static_cast<value_type *>(_allocator->allocate(
          new_capacity * sizeof(value_type), get_stream()));
      if (_size > 0) {
        CUDA_CHECK(cudaMemcpyAsync(new_data, _data, _size * sizeof(value_type),
                                   cudaMemcpyHostToHost, get_stream()));
      }
      if (nullptr != _data) {
        _allocator->deallocate(_data, _capacity * sizeof(value_type),
                               get_stream());
  ...
  std::shared_ptr<hostAllocator> getAllocator() const { return _allocator; }
