...
__global__ void coo_remove_zeros_kernel(int nnz, const int *rows,
                                        const int *cols, const T *vals,
 ...
 */
template <int TPB_X, typename T>
void coo_remove_zeros(int nnz, const int *rows, const int *cols, const T *vals,
                      int *crows, int *ccols, T *cvals, int *cnnz, int cnnz_n) {
  ...
  coo_remove_zeros_kernel<TPB_X><<<grid, blk>>>(
      nnz, rows, cols, vals, crows, ccols, cvals, dev_ex_scan.get(), cnnz_n);
 ...
 */
template <int TPB_X, typename T>
__global__ void coo_row_count(int *rows, int nnz, int *results, int n) {
 ...
 */
template <int TPB_X, typename T>
__global__ void coo_row_count_nz(int *rows, T *vals, int nnz, int *results,
                                 int n) {
  int row = (blockIdx.x * TPB_X) + threadIdx.x;
...
template <int TPB_X, typename T>
__global__ void from_knn_graph_kernel(long *knn_indices, T *knn_dists, int m,
                                      int k, int *rows, int *cols, T *vals) {
 ...
 */
template <typename T>
void from_knn(long *knn_indices, T *knn_dists, int m, int k, int *rows,
              int *cols, T *vals) {
