...
// Transform ncols*nrows read of int in 2*nrows reads of int + ncols*rows reads
// of chars
//
...
template <typename T> struct quad { T x, y, z, w; };
//
// Functor for reduce by key, small k
//
template <typename T> struct quadSum {
  __host__ __device__ __forceinline__ quad<T>
  operator()(const quad<T> &a, const quad<T> &b) const {
    // wasting a double4..
    quad<T> c;
    c.x = a.x + b.x;
    c.y = a.y + b.y;
...
__launch_bounds__(SUM_ROWS_SMALL_K_DIMX, 4) __global__
  ...
  for (int idim = blockIdx.y; idim < ncols; idim += gridDim.y) {
    ...
    // threadIdx.x stores partial sum for current dim and key=threadIdx.x in
    // this reg
    ...
    for (int block_offset_irow = blockIdx.x * blockDim.x;
      ...
      // (experimentation gave a 10% speed up - not worth the many code lines
      // added)
...
__global__ void sum_rows_by_key_large_nkeys_kernel_colmajor(
    const DataIteratorT d_A, int lda, KeysIteratorT d_keys, int nrows,
    int ncols, int key_offset, int nkeys, DataIteratorT d_sums) {
...
__global__ void sum_rows_by_key_large_nkeys_kernel_rowmajor(
  ...
  for (int r = start_row; r < end_row; r++) {
    ...
    if (sh_keys[sh_key_inx] != this_key)
      continue; // No divergence since this_key is the
    // same for the whole block
    sh_key_inx++;
#else
    if (d_keys[r] != this_key)
      continue; // No divergence since this_key is the
                // same for the whole block
#endif
  ...
  if (sum != 0.0)
    myAtomicAdd(&d_sums[this_key * ncols + this_col], sum);
}
...
void reduce_rows_by_key(const DataIteratorT d_A, int lda,
  ...
  if (nkeys <= SUM_ROWS_BY_KEY_SMALL_K_MAX_K) {
    // sum_rows_by_key_small_k is BW bounded. d_keys is loaded ncols time -
    // avoiding wasting BW with doubles we have ~20% speed up - with floats we
    // can hope something around 2x Converting d_keys to char
