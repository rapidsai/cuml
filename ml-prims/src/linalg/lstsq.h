...
void lstsqQR(math_t *A, int n_rows, int n_cols, math_t *b, math_t *w,
  ...
  CUSOLVER_CHECK(cusolverDngeqrf(cusolverH, m, n, A, lda, d_tau, d_work, lwork,
                                 d_info, stream));

  CUDA_CHECK(cudaMemcpy(&info, d_info, sizeof(int), cudaMemcpyDeviceToHost));
  ASSERT(0 == info, "lstsq.h: QR wasn't successful");

  CUSOLVER_CHECK(cusolverDnormqr(cusolverH, side, trans, m, 1, n, A, lda, d_tau,
                                 b, ldb, d_work, lwork, d_info, stream));
