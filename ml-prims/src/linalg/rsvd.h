...
void rsvdFixedRank(math_t *M, int n_rows, int n_cols, math_t *&S_vec,
  ...
  for (int j = 1; j < q; j++) {
    if ((2 * j - 2) % s == 0) {
      qrGetQ(Y.data(), Yorth.data(), m, l, cusolverH, allocator, stream);
    ...
    if ((2 * j - 1) % s == 0) {
      qrGetQ(Z.data(), Zorth.data(), n, l, cusolverH, allocator, stream);
  ...
  qrGetQ(Y.data(), Q.data(), m, l, cusolverH, allocator, stream);
  ...
  if (!use_bbt) {
    ...
    qrGetQR(Bt.data(), Qhat.data(), Rhat.data(), n, l, cusolverH, allocator,
            stream);
    ...
    Matrix::copyUpperTriangular(BBt.data(), Uhat_dup.data(), l, l);
    if (use_jacobi)
      eigJacobi(Uhat_dup.data(), l, l, Uhat.data(), S_vec_tmp.data(), tol,
                max_sweeps, cusolverH, allocator, stream);
    else
        eigDC(Uhat_dup.data(), l, l, Uhat.data(), S_vec_tmp.data(), cusolverH,
              allocator, stream);
    Matrix::seqRoot(S_vec_tmp.data(), l);
