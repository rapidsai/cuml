...
template <typename math_t, int veclen_, typename Lambda>
__global__ void unaryScalarOpKernel(math_t *out, const math_t *in, math_t scalar,
                              int len, Lambda op) {
  typedef TxN_t<math_t, veclen_> VecType;
  VecType a;
  int idx = (threadIdx.x + (blockIdx.x * blockDim.x)) * VecType::Ratio;
...
template <typename math_t, int veclen_, typename Lambda, int TPB>
void unaryScalarOpImpl(math_t *out, const math_t *in, math_t scalar, int len,
                 Lambda op, cudaStream_t stream = 0) {
  const int nblks = ceildiv(veclen_ ? len / veclen_ : len, TPB);
  unaryScalarOpKernel<math_t, veclen_, Lambda><<<nblks, TPB, 0, stream>>>(
    out, in, scalar, len, op);
  CUDA_CHECK(cudaPeekAtLastError());
}
 ...
 */
template <typename math_t, typename Lambda, int TPB = 256>
void unaryScalarOp(math_t *out, const math_t *in, math_t scalar, int len, Lambda op,
             cudaStream_t stream = 0) {
  size_t bytes = len * sizeof(math_t);
  uint64_t inAddr = uint64_t(in);
  uint64_t outAddr = uint64_t(out);
  if (16 / sizeof(math_t) && bytes % 16 == 0 && inAddr % 16 == 0 &&
      outAddr % 16 == 0) {
    unaryScalarOpImpl<math_t, 16 / sizeof(math_t), Lambda, TPB>(out, in, scalar, len,
                                                          op, stream);
  } else if (8 / sizeof(math_t) && bytes % 8 == 0 && inAddr % 8 == 0 &&
             outAddr % 8 == 0) {
    unaryScalarOpImpl<math_t, 8 / sizeof(math_t), Lambda, TPB>(out, in, scalar, len,
                                                         op, stream);
  } else if (4 / sizeof(math_t) && bytes % 4 == 0 && inAddr % 4 == 0 &&
             outAddr % 4 == 0) {
    unaryScalarOpImpl<math_t, 4 / sizeof(math_t), Lambda, TPB>(out, in, scalar, len,
                                                         op, stream);
  } else if (2 / sizeof(math_t) && bytes % 2 == 0 && inAddr % 2 == 0 &&
             outAddr % 2 == 0) {
    unaryScalarOpImpl<math_t, 2 / sizeof(math_t), Lambda, TPB>(out, in, scalar, len,
                                                         op, stream);
  } else if (1 / sizeof(math_t)) {
    unaryScalarOpImpl<math_t, 1 / sizeof(math_t), Lambda, TPB>(out, in, scalar, len,
                                                         op, stream);
  } else {
    unaryScalarOpImpl<math_t, 1, Lambda, TPB>(out, in, scalar, len, op, stream);
  }
}
