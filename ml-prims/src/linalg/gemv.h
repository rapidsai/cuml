...
void gemv(const math_t *a, int n_rows, int n_cols, const math_t *x, int incx,
  ...
  // in BLAS https://docs.nvidia.com/cuda/cublas/index.html is opposite to
  // Machine Learning In blas:
  //  m - number of rows in input matrix
  //  n - number of columns in input matrix
  //  lda - purpose of it  to have ability to operate on submatrices of matrix
  //  without copying.
  //        If you're not think about it it's always should be equal to m
  //  lda has deal with memory layout, but has nothing with the requirement for
  //  cuBLAS perform transpose
