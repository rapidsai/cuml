...
/** Substract single value pointed by singleScalarDev parameter in device memory
 * from inDev[i] and write result to outDev[i]
 * @tparam math_t data-type upon which the math operation will be performed
 * @tparam IdxType Integer type used to for addressing
 * @param out the output buffer
 * @param in the input buffer
...
void subtractDevScalar(math_t *outDev, const math_t *inDev,
                       ...
                       cudaStream_t stream) {
  // Just for the note - there is no way to express such operation with cuBLAS
  // in effective way
