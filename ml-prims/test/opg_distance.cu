...
// TODO(minseok): double check the result difference between CPU and GPU for the
// small matrx size
// TODO(minseok): it would be useful if we have a macro for the current device
// recovery
...
template <typename Type>
__global__ void naiveL1DistanceKernel(Type *out, const Type *x, const Type *y,
                                      int m, int n, int k) {
  int midx = threadIdx.x + blockIdx.x * blockDim.x;
  int nidx = threadIdx.y + blockIdx.y * blockDim.y;
  if (midx >= m || nidx >= n) {
...
template <typename Type>
__global__ void naiveCosineDistanceKernel(Type *out, const Type *x,
...
enum PartitionScheme { XOnly, YOnly, Both };

template <typename T> struct OpgDistanceInputs {
  T tolerance;
  int m, n, k;
  DistanceType type;
...
::std::ostream &operator<<(::std::ostream &os,
                           const OpgDistanceInputs<T> &dims) {
  return os;
}
...
Worker<T>::Worker(int device_id)
    : device_id_(device_id), workspace_(nullptr), worksize_(0) {}

template <typename T> Worker<T>::~Worker() {
  if (workspace_)
    CUDA_CHECK(cudaFree(workspace_));
}

template <typename T> void Worker<T>::enqueue(Task<T> *t) {
  ...
  distance<T, T, Task<T>, Task<T>, OutputTile_t>(
      t->d_X, t->d_Y, t->m, t->n, t->k, *t, *t, t->type, nullptr, new_worksize);
  if (new_worksize != 0) {
...
template <typename T> void Worker<T>::execute() {
  int current_device_id;
  CUDA_CHECK(cudaGetDevice(&current_device_id));
  ...
  for (auto it = active_queue_.begin(); it != active_queue_.end(); ++it) {
    Task<T> *t = *it;
    distance<T, T, Task<T>, Task<T>, OutputTile_t>(
        t->d_X, t->d_Y, t->m, t->n, t->k, *t, *t, t->type, (void *)workspace_,
        worksize_);
  }
...
template <typename T> Task<T> *Worker<T>::dequeue() {
  if (empty())
    return nullptr;
...
template <typename T>::testing::AssertionResult Worker<T>::verify(T tolerance) {
  int current_device_id;
  CUDA_CHECK(cudaGetDevice(&current_device_id));
  ...
  for (auto it = active_queue_.begin(); it != active_queue_.end(); ++it) {
    ...
    auto ret =
        devArrMatch(dist_ref, t->dist, t->m, t->n, CompareApprox<T>(tolerance));
...
template <typename T> bool Worker<T>::empty() const {
  return active_queue_.empty();
}

template <typename T> int Worker<T>::getDeviceId() const { return device_id_; }

void getNumberOfTiles(PartitionScheme scheme, int m, int n, int k, int n_gpu,
                      int &n_vertical_tiles, int &n_horizontal_tiles) {
  switch (scheme) {
  case XOnly:
...
template <typename T>
void assignTasks(std::vector<Worker<T> *> &workers, int n_gpu, int m, int n,
                 int k, DistanceType type, PartitionScheme scheme,
                 unsigned long long int seed) {
  ASSERT(workers.size() == n_gpu, "# workers(%d) != # GPUs(%d)", workers.size(),
         n_gpu);
  ...
  getNumberOfTiles(scheme, m, n, k, n_gpu, n_vertical_tiles,
                   n_horizontal_tiles);
  ...
  for (int y = 0; y < n_vertical_tiles; y++) {
    for (int x = 0; x < n_horizontal_tiles; x++) {
      ...
      ASSERT(id == worker->getDeviceId(), "id(%d) != deviceId(%d)", id,
             worker->getDeviceId());
...
template <typename T> void finalizeTasks(std::vector<Worker<T> *> &workers) {
  int current_device_id;
  CUDA_CHECK(cudaGetDevice(&current_device_id));
...
protected:
  void SetUp() override {
    ...
    assignTasks(workers, n_gpu, params.m, params.n, params.k, params.type,
                params.scheme, params.seed);
...
#pragma omp parallel for num_threads(n_gpu)
  ...
  void TearDown() override { finalizeTasks(workers); }
