...
protected:
    void SetUp() override {
        ...
        lasso(out_lasso, in, len, alpha);
        ridge(out_ridge, in, len, alpha);
        elasticnet(out_elasticnet, in, len, alpha, l1_ratio);
        lassoGrad(out_lasso_grad, in, len, alpha);
        ridgeGrad(out_ridge_grad, in, len, alpha);
        elasticnetGrad(out_elasticnet_grad, in, len, alpha, l1_ratio);
...
const std::vector<PenaltyInputs<float> > inputsf = {
    {0.01f, 4}
};

const std::vector<PenaltyInputs<double> > inputsd = {
    {0.01, 4}
};
...
TEST_P(PenaltyTestF, Result) {
    ...
    ASSERT_TRUE(devArrMatch(out_elasticnet_grad_ref, out_elasticnet_grad, params.len,
                            CompareApprox<float>(params.tolerance)));
