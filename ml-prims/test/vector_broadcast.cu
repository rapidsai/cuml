...
template <typename T> struct VecBcastInputs {
  T tolerance;
  int rows, cols;
...
// within its class
template <typename T>
void vectorBroadcastLaunch(T *out, const T *mat, const T *vec, int rows,
                           int cols) {
...
INSTANTIATE_TEST_CASE_P(VecBcastTests, VecBcastTestF,
                        ::testing::ValuesIn(inputsf));

INSTANTIATE_TEST_CASE_P(VecBcastTests, VecBcastTestD,
