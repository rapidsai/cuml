...
template <typename T> struct RngInputs {
  T tolerance;
  int len;
...
// choose to set this at 3-4 x sigma, i.e., a 99.7-99.9% confidence interval so
// that the test will indeed pass. In quick experiments (using the identical
...
template <typename T> T quick_mean(const std::vector<T> &d) {
  T acc = T(0);
  for (const auto &di : d) {
    acc += di;
  }
...
template <typename T> T quick_std(const std::vector<T> &d) {
  T acc = T(0);
