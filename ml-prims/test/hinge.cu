...
template <typename T> struct HingeLossInputs {
  T tolerance;
  T n_rows;
  T n_cols;
  int len;
...
protected:
  void SetUp() override {
    ...
    hingeLoss(in, params.n_rows, params.n_cols, labels, coef, out,
              penalty::NONE, alpha, l1_ratio, cublas_handle, stream);
    ...
    hingeLoss(in, params.n_rows, params.n_cols, labels, coef, out_lasso,
              penalty::L1, alpha, l1_ratio, cublas_handle, stream);
    ...
    hingeLoss(in, params.n_rows, params.n_cols, labels, coef, out_ridge,
              penalty::L2, alpha, l1_ratio, cublas_handle, stream);
    ...
    hingeLoss(in, params.n_rows, params.n_cols, labels, coef, out_elasticnet,
              penalty::ELASTICNET, alpha, l1_ratio, cublas_handle, stream);
