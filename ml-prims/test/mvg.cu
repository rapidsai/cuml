...
template<typename T>
__global__ void En_KF_dif(const int nPoints, const int dim, const T *X, const T *x,
                          T *X_diff) {
    int idx = threadIdx.x + blockDim.x * blockIdx.x;
    int col = idx % dim;
    int row = idx / dim;
...
protected:
    void SetUp() override {
        ...
        CUBLAS_CHECK(LinAlg::cublasgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, dim,
                                        dim, nPoints, &alfa, X_d, dim, X_d, dim,
                                        &beta, Rand_cov, dim));
...
const std::vector<MVGInputs<float> > inputsf = {
  ...
  { 0.1f, MultiVarGaussian<float>::Decomposer::chol_decomp, Correlation::UNCORRELATED, 5, 30000, 6ULL},
  ...
  // { 0.2f,          MultiVarGaussian<float>::Decomposer::qr, Correlation::UNCORRELATED, 5, 30000, 6ULL}
};
const std::vector<MVGInputs<double> > inputsd = {
  {0.25, MultiVarGaussian<double>::Decomposer::chol_decomp,   Correlation::CORRELATED, 10, 3000000, 6ULL},
  { 0.1, MultiVarGaussian<double>::Decomposer::chol_decomp, Correlation::UNCORRELATED, 10, 3000000, 6ULL},
  ...
  // { 0.2,          MultiVarGaussian<double>::Decomposer::qr, Correlation::UNCORRELATED, 10, 3000000, 6ULL}
};
...
TEST_P(MVGTestF, MeanIsCorrectF){
    EXPECT_TRUE(devArrMatch(x_d, Rand_mean, dim,
                            CompareApprox<float>(tolerance)))
                            << " in MeanIsCorrect";
}
TEST_P(MVGTestF, CovIsCorrectF){
    EXPECT_TRUE(devArrMatch(P_d, Rand_cov, dim, dim,
                            CompareApprox<float>(tolerance)))
                            << " in CovIsCorrect";
}
TEST_P(MVGTestD, MeanIsCorrectD){
    EXPECT_TRUE(devArrMatch(x_d, Rand_mean, dim,
                            CompareApprox<double>(tolerance)))
                            << " in MeanIsCorrect";
}
