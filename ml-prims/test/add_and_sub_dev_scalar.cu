...
public:
  TestBuffer(size_t arrayLength)
      : devContainterRaw(nullptr), hostContainter(arrayLength, T()) {
    MLCommon::allocate(devContainterRaw, arrayLength);
    EXPECT_TRUE(devContainterRaw != nullptr);
  }

  ~TestBuffer() { EXPECT_TRUE(cudaFree(devContainterRaw) == cudaSuccess); }

  T *getDevPtr() { return devContainterRaw; }
  ...
  size_t size() const { return hostContainter.size(); }
  ...
  void copy2Device() {
    EXPECT_TRUE(cudaMemcpy(getDevPtr(), getHostPtr(), size() * sizeof(T),
                           cudaMemcpyHostToDevice) == cudaSuccess);
  }

  void copy2Host() {
    EXPECT_TRUE(cudaMemcpy(getHostPtr(), getDevPtr(), size() * sizeof(T),
...
template <typename T> void test_add(size_t arraLength) {
  ...
  MLCommon::LinAlg::addDevScalar(out.getDevPtr(), in.getDevPtr(),
                                 extraScalar.getDevPtr(), in.size());
  out.copy2Host();

  T maxError = T();
  for (int i = 0; i < arraLength; i++) {
    maxError = std::max(maxError,
                        abs((in.hostValueAt(i) + extraScalar.hostValueAt(0)) -
                            out.hostValueAt(i)));
  }
  EXPECT_TRUE(maxError < std::numeric_limits<T>::epsilon())
      << "Max deviation in test_add is greater then "
      << std::numeric_limits<T>::epsilon();
}

template <typename T> void test_subtract(size_t arraLength) {
  TestBuffer<T> in(arraLength);
  TestBuffer<T> extraScalar(1);
  TestBuffer<T> out(arraLength);
  in.fillArithmeticSeq();
  ...
  MLCommon::LinAlg::subtractDevScalar(out.getDevPtr(), in.getDevPtr(),
                                      extraScalar.getDevPtr(), in.size());
  out.copy2Host();
  ...
  for (int i = 0; i < arraLength; i++)
    maxError = std::max(maxError,
                        abs((in.hostValueAt(i) - extraScalar.hostValueAt(0)) -
                            out.hostValueAt(i)));
  EXPECT_TRUE(maxError < std::numeric_limits<T>::epsilon())
      << "Max deviation test_subtract is greater then "
      << std::numeric_limits<T>::epsilon();
}
