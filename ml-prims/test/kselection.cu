...
#pragma unroll
        ...
        template <typename TypeV,typename TypeK, bool Greater>
            ::testing::AssertionResult checkResult(TypeV* d_arr, TypeV* d_outv,
                    TypeK* d_outk, int rows,int N, int k,TypeV tolerance){
            ...
            class WarpTopKTest: public ::testing::TestWithParam<WarpTopKInputs<T> > {
                protected:
                    void SetUp() override {
                        ...
                        warpTopK<T,int,Greater,Sort>(outv, outk, arr, params.k, 
        ...
        // Milestone 2.2: Using the full thread queue and warp queue  code to support all the values 
        ...
        TEST_P(TestD2_0, Result){
            ...
            ASSERT_TRUE((checkResult<float, int,Greater>(arr, outv, outk, params.rows,
                    params.cols,params.k,params.tolerance)));
        }
