...
public:
  ...
  buffer_base(std::shared_ptr<Allocator> allocator, cudaStream_t stream, size_type n=0)
    :  _size(n), _capacity(n), _data(nullptr), _stream(stream), _allocator(allocator)
  {
    if ( _capacity > 0 )
    {
  ...
  void reserve( const size_type new_capacity, cudaStream_t stream )
  {
    ...
    if ( new_capacity > _capacity )
    {
      ...
      if ( _size > 0 ) {
        CUDA_CHECK( cudaMemcpyAsync( new_data, _data, _size*sizeof(value_type), cudaMemcpyDefault, _stream ) );
      }
      if ( nullptr != _data ) {
        _allocator->deallocate( _data, _capacity*sizeof(value_type), _stream );
      }
