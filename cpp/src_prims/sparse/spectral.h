...
template <typename T>
void fit_clusters(int *rows, int *cols, T *vals,
                  int nnz, int n, int n_clusters, float eigen_tol,
                  int *out,
  ...
  NVGRAPH_CHECK(nvgraphSpectralClustering(graphHandle, graph, weight_index,
                                          &clustering_params, out, eigVals.data(),
                                          embedding.data()));
...
template <typename T>
void fit_clusters(long *knn_indices, T *knn_dists,
  ...
  MLCommon::Sparse::from_knn(knn_indices, knn_dists, m,
                             n_neighbors,
                             rows.data(),
                             cols.data(),
                             vals.data());
  ...
  fit_clusters(rows.data(), cols.data(), vals.data(),
               m * n_neighbors, m, n_clusters,
...
void fit_clusters(T *X, int m, int n, int n_neighbors,
  ...
  fit_clusters(knn_indices.data(), knn_dists.data(),
               m, n_neighbors, n_clusters,
               eigen_tol, out, allocator, stream);
...
template <typename T>
void fit_embedding(int *rows, int *cols, T *vals,
  ...
  NVGRAPH_CHECK(nvgraphSpectralClustering(grapHandle, graph, weight_index,
                                          &clustering_params, labels.data(), eigVals.data(),
                                          eigVecs.data()));

  NVGRAPH_CHECK(nvgraphDestroyGraphDescr(grapHandle, graph));
  NVGRAPH_CHECK(nvgraphDestroy(grapHandle));

  MLCommon::copy<T>(out, eigVecs.data() + n,
                 n_components * n, stream);
...
template <typename T>
void fit_embedding(long *knn_indices,
                   float *knn_dists, int m, int n_neighbors,
                   int n_components,
                   T *out,
                   std::shared_ptr<deviceAllocator> allocator,
                   cudaStream_t stream) {
  ...
  fit_embedding(rows.data(), cols.data(), vals.data(),
      m * n_neighbors, m, n_components,
      out, allocator, stream);
}

template <typename T>
void fit_embedding(T *X,
                   int m,
                   int n,
                   int n_neighbors,
                   int n_components,
