 ...
 */
template <typename math_t> __global__ static void
symmetric_find_size(const math_t *restrict data,
 ...
 */
template <typename math_t> __global__ static void
symmetric_sum(int *restrict edges,
 ...
 */
template <typename math_t, int TPB_X = 32, int TPB_Y = 32> void
from_knn_symmetrize_matrix(const long *restrict knn_indices,
                          const math_t *restrict knn_dists,
                          const int n, const int k, COO<math_t> *out,
                          cudaStream_t stream)
{   
  // (1) Find how much space needed in each row
  // We look through all datapoints and increment the count for each row.
  const dim3 threadsPerBlock(TPB_X, TPB_Y);
  const dim3 numBlocks(ceil_uint(k, threadsPerBlock.x), ceil_uint(n, threadsPerBlock.y));
  ...
  reduce_find_size<<<ceil_uint(n, 1024), 1024, 0, stream>>>(n, k, row_sizes, row_sizes2);
  CUDA_CHECK(cudaPeekAtLastError());
  ...
  symmetric_sum<<<numBlocks, threadsPerBlock, 0, stream>>>(
    edges, knn_dists, knn_indices, 
    out->vals, out->cols, out->rows, n, k);
  CUDA_CHECK(cudaPeekAtLastError());
