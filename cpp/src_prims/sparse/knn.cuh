...
class sparse_knn_t {
 ...
 public:
  sparse_knn_t(const value_idx *idxIndptr_, const value_idx *idxIndices_,
    ...
    idxIndptr(idxIndptr_), idxIndices(idxIndices_), idxData(idxData_), idxNNZ(idxNNZ_),
    n_idx_rows(n_idx_rows_), n_idx_cols(n_idx_cols_), queryIndptr(queryIndptr_),
    queryIndices(queryIndices_), queryData(queryData_), queryNNZ(queryNNZ_),
    n_query_rows(n_query_rows_), n_query_cols(n_query_cols_), output_indices(output_indices_),
    output_dists(output_dists_), k(k_), cusparseHandle(cusparseHandle_), allocator(allocator_),
    stream(stream_), batch_size_index(batch_size_index_), batch_size_query(batch_size_query_),
    metric(metric_), metricArg(metricArg_), expanded_form(expanded_form_){}
      ...
      for (int i = 0; i < n_batches_query; i++) {
        ...
        for (int j = 0; j < n_batches_idx; j++) {
          ...
          if (idx_batcher.batch_start() > 0) {
            ...
            merge_batches(idx_batcher, query_batcher, merge_buffer_dists.data(), merge_buffer_indices.data(),
 ...
 private:
     ...
     void merge_batches(csr_batcher_t<value_idx, value_t> &idx_batcher,
                        csr_batcher_t<value_idx, value_t> &query_batcher,
                        value_t *merge_buffer_dists, value_idx *merge_buffer_indices,
       ...
       updateDevice(trans.data(), id_ranges.data(), id_ranges.size(),
                    stream);
       ...
       MLCommon::Selection::knn_merge_parts(
         merge_buffer_dists, merge_buffer_indices,
     ...
     void perform_k_selection(csr_batcher_t<value_idx, value_t> idx_batcher,
       ...
       select_k(batch_dists, batch_indices, batch_rows,
                batch_cols, out_dists, out_indices,
                ascending, n_neighbors, stream);
     ...
     void compute_distances(csr_batcher_t<value_idx, value_t> &idx_batcher,
                            ...
                            value_idx *idx_batch_indptr, value_idx *idx_batch_indices,
                            value_t *idx_batch_data,
                            value_idx *query_batch_indptr, value_idx *query_batch_indices,
...
void brute_force_knn(const value_idx *idxIndptr, const value_idx *idxIndices,
  ...
  sparse_knn_t<value_idx, value_t>(idxIndptr, idxIndices,
  idxData, idxNNZ, n_idx_rows,
    n_idx_cols, queryIndptr,
  queryIndices, queryData,
  queryNNZ, n_query_rows, n_query_cols,
    output_indices, output_dists, k,
    cusparseHandle,
    allocator,
    stream,
    batch_size_index,
