...
class coo_spmv_strategy {
 public:
  ...
  template <typename strategy_t, typename product_f, typename accum_f,
            typename write_f>
  void _dispatch_base(strategy_t &strategy, value_idx smem_dim,
    ...
    CUDA_CHECK(cudaFuncSetCacheConfig(
      balanced_coo_generalized_spmv_kernel<strategy_t, value_idx, value_t, tpb,
                                           false, product_f, accum_f, write_f>,
      cudaFuncCachePreferShared));

    balanced_coo_generalized_spmv_kernel<strategy_t, value_idx, value_t, tpb,
                                         false>
      <<<n_blocks, tpb, smem, config.stream>>>(
        strategy, config.a_indptr, config.a_indices, config.a_data, coo_rows_b,
  ...
  void _dispatch_base_rev(strategy_t &strategy, value_idx smem_dim,
                          value_t *out_dists, value_idx *coo_rows_a,
                          product_f product_func, accum_f accum_func,
                          write_f write_func, int chunk_size) {
    CUML_LOG_DEBUG("n_blocks: %d", n_blocks);
    CUML_LOG_DEBUG("n_warps_per_row: %d", n_blocks_per_row);
    ...
    CUDA_CHECK(cudaFuncSetCacheConfig(
      balanced_coo_generalized_spmv_kernel<strategy_t, value_idx, value_t, tpb,
                                           true, product_f, accum_f, write_f>,
      cudaFuncCachePreferShared));

    balanced_coo_generalized_spmv_kernel<strategy_t, value_idx, value_t, tpb,
                                         true>
      <<<n_blocks, tpb, smem, config.stream>>>(
        strategy, config.b_indptr, config.b_indices, config.b_data, coo_rows_a,
...
class dense_smem_strategy : public coo_spmv_strategy<value_idx, value_t, tpb> {
 public:
  ...
  void dispatch(value_t *out_dists, value_idx *coo_rows_b,
                ...
                int chunk_size) {
    this->n_blocks_per_row =
      raft::ceildiv(this->config.b_nnz, chunk_size * tpb);
    this->n_blocks = this->config.a_nrows * this->n_blocks_per_row;

    this->_dispatch_base(*this, this->config.b_ncols, out_dists, coo_rows_b,
  ...
  void dispatch_rev(value_t *out_dists, value_idx *coo_rows_a,
                    ...
                    write_f write_func, int chunk_size) {
    this->n_blocks_per_row =
      raft::ceildiv(this->config.a_nnz, chunk_size * tpb);
    this->n_blocks = this->config.b_nrows * this->n_blocks_per_row;

    this->_dispatch_base_rev(*this, this->config.a_ncols, out_dists, coo_rows_a,
                             product_func, accum_func, write_func, chunk_size);
  }
...
class hash_strategy : public coo_spmv_strategy<value_idx, value_t, tpb> {
 public:
  ...
  void dispatch(value_t *out_dists, value_idx *coo_rows_b,
    ...
    this->_dispatch_base(*this, map_size(), out_dists, coo_rows_b, product_func,
                         accum_func, write_func, chunk_size);
  }
  ...
  void dispatch_rev(value_t *out_dists, value_idx *coo_rows_a,
    ...
    this->_dispatch_base_rev(*this, map_size(), out_dists, coo_rows_a,
                             product_func, accum_func, write_func, chunk_size);
  }
 ...
 private:
  __host__ __device__ constexpr int map_size() {
    return (46000 - ((tpb / raft::warp_size()) * sizeof(value_t))) /
