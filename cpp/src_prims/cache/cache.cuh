 ...
 *
 *        SW managed cache in device memory, for ML algos where we can trade
 *        memory access for computation. The two main functions of this class
 *        are the management of cache indices, and methods to retrieve/store
 *        data using the cache indices.
 *
 *        The index management can be considered as a hash map<int, int>, where
 *        the int keys are the original vector indices that we want to store,
 *        and the values are the cache location of these vectors. The keys are
 *        hashed into a bucket whose size equals the associativity. These are
 *        the cache sets. If a cache set is full, then new indices are stored by
 *        replacing the oldest entries.
 *
 *        Using this index mapping we implement methods to store and retrive
 *        data from the cache buffer, where a unit of data that we are storing
 *        is math_t[n_vec]. For example in SVM we store full columns of the
 *        kernel matrix at each cache entry.
 *
 *        Note: we should have a look if the index management could be
 *        simplified using concurrent_unordered_map.cuh from cudf. See Issue
 *        #914.
 *
 *        Example usage:
 * @code{.cpp}
 ...
 * @tparam associativity { description }
 */
...
class Cache {
 public:
   ...
   *
   *        On exit, the tile array is filled the following way: out[i +
   *        n_vec*k] = cache[i + n_vec * idx[k]]), where i=0..n_vec-1, k =
   *        0..n-1
   *
   ...
   *
   *        Roughly the opposite of GetVecs, but the input vectors can be
   *        scattered in memory. The cache is updated using the following
   *        formula:
   *
   *        cache[i + cache_idx[k]*n_vec] = tile[i + tile_idx[k]*n_vec], for
   *        i=0..n_vec-1, k=0..n-1
   *
   ...
   * @param[in] n         number of vectors that need to be stored in the cache
   *                      (a subset of all the vectors in the tile)
   ...
   *
   *        For each k in 0..n-1, if keys[k] is found in the cache, then
   *        cache_idx[k] will tell the corresponding cache idx, and is_cached[k]
   *        is set to true.
   *
   *        If keys[k] is not found in the cache, then is_cached[k] is set to
   *        false. In this case we assign the cache set for keys[k], and
   *        cache_idx[k] will store the cache set.
   *
   ...
   * @brief Map a set of keys to cache indices.
   *
   *        Same as GetCacheIdx, but partitions the keys, and cache_idx arrays
   *        in a way that keys[0..n_cached-1] and cache_idx[0..n_cached-1] store
   *        the indices of vectors that are found in the cache, while
   *        keys[n_cached..n-1] are the indices of vectors that are not found in
   *        the cache. For the vectors not found in the cache,
   *        cache_idx[n_cached..n-1] stores the cache set, and this can be used
   *        to call AssignCacheIdx.
   *
   ...
   *
   *        Note: call GetCacheIdx first, to get the cache_set assigned to the
   *        keys. Keys that cannot be cached are assigned to -1.
   *
   * @param[inout] keys   device array of keys, size [n]
   * @param[in]    n      number of elements that we want to cache
   * @param[inout] cidx   on entry: cache_set, on exit: assigned cache_idx or
   *                      -1, size[n]
   * @param[in]    stream cuda stream
   */
