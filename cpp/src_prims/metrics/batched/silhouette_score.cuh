...
__global__
void fill_b_kernel(value_t *b, label_idx *y, value_idx n_rows, label_idx n_labels, value_idx *cluster_counts) {
...
template <typename value_t, typename value_idx, typename label_idx>
__global__
void compute_chunked_a_b_kernel(value_t *a, value_t *b,
                                value_idx row_offset, value_idx col_offset,
                                label_idx *y, label_idx n_labels,
                                value_idx *cluster_counts,
                                value_t *distances,
                                value_idx dist_rows,
                                value_idx dist_cols) {
    ...
    if (col_cluster == row_cluster) {
        ...
        atomicAdd(&b[pw_row_id * n_labels + col_cluster], distances[row_id * dist_cols + col_id] / col_cluster_counts);
    }
...
rmm::device_uvector<value_idx> get_cluster_counts(const raft::handle_t &handle, label_idx *y,
                                                  value_idx &n_rows, label_idx &n_labels) {
    ...
    MLCommon::Metrics::countLabels(y, cluster_counts.data(), n_rows, n_labels, workspace, allocator, stream);

    return cluster_counts;
}

template <typename value_t, typename value_idx>
rmm::device_uvector<value_t> get_pairwise_distance(const raft::handle_t &handle, value_t *left_begin, value_t *right_begin,
    ...
    MLCommon::Distance::pairwise_distance(left_begin, right_begin, distances.data(),
...
template <typename value_t, typename value_idx, typename label_idx>
void compute_chunked_a_b(const raft::handle_t &handle, value_t *a, value_t *b, value_idx &row_offset,
                         value_idx &col_offset,
                         label_idx *y, label_idx &n_labels, value_idx *cluster_counts,
                         value_t *distances, value_idx &dist_rows,
                         value_idx &dist_cols, cudaStream_t stream) {

    dim3 block_size(std::min(dist_rows, 32), std::min(dist_cols, 32));
    dim3 grid_size( raft::ceildiv(dist_rows, (value_idx) block_size.x), raft::ceildiv(dist_cols, (value_idx) block_size.y) );

    detail::compute_chunked_a_b_kernel<<<grid_size, block_size, 0, stream>>>(a, b, row_offset, col_offset, y, n_labels, cluster_counts, distances, dist_rows, dist_cols);
}

template <typename value_t, typename value_idx, typename label_idx>
value_t silhouette_score(const raft::handle_t &handle, value_t *X, value_idx n_rows,
                                 value_idx n_cols, label_idx *y, label_idx n_labels, value_t *scores,
                                 value_idx chunk, raft::distance::DistanceType metric =
    ...
    dim3 grid_size( raft::ceildiv(n_rows, (value_idx) block_size.x), raft::ceildiv(n_labels, (value_idx) block_size.y) );
    detail::fill_b_kernel<<<grid_size, block_size, 0, stream>>>(b_ptr, y, n_rows, n_labels, cluster_counts.data());
    ...
    for (value_idx i = 0; i < n_rows; i += chunk) {
        for (value_idx j = 0; j < n_rows; j += chunk) {
            ...
            auto chunk_stream = raft::select_stream(stream, handle.get_internal_streams().data(),
            ...
            rmm::device_uvector<value_t> distances = get_pairwise_distance(handle, left_begin,
                right_begin, n_left_rows, n_right_rows, n_cols, metric, chunk_stream);

            compute_chunked_a_b(handle, a_ptr, b_ptr, i, j, y, n_labels, cluster_counts.data(), distances.data(), n_left_rows, n_right_rows, chunk_stream);
        }
    }
    ...
    raft::linalg::reduce<value_t, value_t, value_idx, raft::Nop<value_t>, MLCommon::Metrics::MinOp<value_t>>(
        b_ptr, b_ptr, n_labels,
        n_rows, std::numeric_limits<value_t>::max(), true, true, stream, false,
        raft::Nop<value_t>(), MLCommon::Metrics::MinOp<value_t>());

    //calculating the silhouette score per sample using the d_aArray and d_bArray
    raft::linalg::binaryOp<value_t, MLCommon::Metrics::SilOp<value_t>, value_t, value_idx>(a_ptr,
                                                a_ptr, b_ptr,
                                                n_rows, MLCommon::Metrics::SilOp<value_t>(), stream);
