...
void qrGetQ(math_t *M, math_t *Q, int n_rows, int n_cols,
  ...
  CUSOLVER_CHECK(
    raft::linalg::cusolverDngeqrf_bufferSize(cusolverH, m, n, Q, m, &Lwork));
  device_buffer<math_t> workspace(allocator, stream, Lwork);
  CUSOLVER_CHECK(raft::linalg::cusolverDngeqrf(cusolverH, m, n, Q, m,
                                               tau.data(), workspace.data(),
                                               Lwork, devInfo.data(), stream));
...
#endif
  CUSOLVER_CHECK(raft::linalg::cusolverDnorgqr_bufferSize(
    cusolverH, m, n, k, Q, m, tau.data(), &Lwork));
  workspace.resize(Lwork, stream);
  CUSOLVER_CHECK(raft::linalg::cusolverDnorgqr(cusolverH, m, n, k, Q, m,
                                               tau.data(), workspace.data(),
                                               Lwork, devInfo.data(), stream));
}
...
void qrGetQR(math_t *M, math_t *Q, math_t *R, int n_rows, int n_cols,
  ...
  CUSOLVER_CHECK(raft::linalg::cusolverDngeqrf_bufferSize(
