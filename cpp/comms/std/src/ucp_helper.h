...
struct comms_ucp_handle {
    ...
    ucs_status_ptr_t (*send_func)(ucp_ep_h, const void*,
                                              size_t, ucp_datatype_t,
                                              ucp_tag_t,
                                              ucp_send_callback_t);
    ucs_status_ptr_t (*recv_func)(ucp_worker_h, void *,                                              size_t count, ucp_datatype_t datatype,
                                              ucp_tag_t, ucp_tag_t,
...
void load_ucp_handle(struct comms_ucp_handle* ucp_handle ) {
  ucp_handle->ucp_handle = dlopen("libucp.so", RTLD_LAZY | RTLD_NOLOAD | RTLD_NODELETE);
...
void load_send_func(struct comms_ucp_handle *ucp_handle) {
  ucp_handle->send_func = (ucs_status_ptr_t(*)(
    ucp_ep_h, const void *, size_t, ucp_datatype_t,
    ucp_tag_t, ucp_send_callback_t))dlsym(ucp_handle->ucp_handle, "ucp_tag_send_nb");
  assert_dlerror();
}
...
void load_print_info_func(struct comms_ucp_handle *ucp_handle) {
  ucp_handle->print_info_func = (void(*)(ucp_ep_h, FILE *))dlsym(ucp_handle->ucp_handle, "ucp_ep_print_info");
  assert_dlerror();
}

void load_worker_progress_func(struct comms_ucp_handle *ucp_handle) {
  ucp_handle->worker_progress_func =
    (void (*)(ucp_worker_h))dlsym(ucp_handle->ucp_handle, "ucp_worker_progress");
  assert_dlerror();
}

void load_recv_func(struct comms_ucp_handle *ucp_handle) {
  ucp_handle->recv_func = (ucs_status_ptr_t(*)(
    ucp_worker_h, void *, size_t, ucp_datatype_t,
    ucp_tag_t, ucp_tag_t,
 ...
 */
struct ucx_context *ucp_isend(struct comms_ucp_handle *ucp_handle, ucp_ep_h ep_ptr,
 ...
 */
struct ucx_context *ucp_irecv(struct comms_ucp_handle *ucp_handle, ucp_worker_h worker,
