...
void inject_comms_py(cumlHandle *handle, ncclComm_t comm, void *ucp_worker,
  ...
  auto communicator = std::make_shared<raft::comms::comms_t>(
    std::unique_ptr<raft::comms::comms_iface>(new raft::comms::std_comms(
      comm, (ucp_worker_h)ucp_worker, eps_sp, size, rank, d_alloc, stream)));
...
void inject_comms_py_coll(cumlHandle *handle, ncclComm_t comm, int size,
  ...
  auto communicator = std::make_shared<raft::comms::comms_t>(
    std::unique_ptr<raft::comms::comms_iface>(
