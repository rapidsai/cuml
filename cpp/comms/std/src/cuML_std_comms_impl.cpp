...
void cumlStdCommunicator_impl::isend(const void *buf, int size, int dest,
  ...
  struct ucx_context *ucp_request =
    ucp_isend((struct comms_ucp_handle*)_ucp_handle, ep_ptr, buf, size, tag, default_tag_mask, getRank());
...
void cumlStdCommunicator_impl::irecv(void *buf, int size, int source, int tag,
  ...
  struct ucx_context *ucp_request = ucp_irecv((struct comms_ucp_handle*)_ucp_handle, _ucp_worker, ep_ptr,
...
void cumlStdCommunicator_impl::waitall(int count,
  ...
  while (requests.size() > 0) {
    for (std::vector<struct ucx_context *>::iterator it = requests.begin();
      ...
      if (req->completed == 1) {
        req->completed = 0;
        if (req->needs_release) free_ucp_request((struct comms_ucp_handle*)_ucp_handle, req);
