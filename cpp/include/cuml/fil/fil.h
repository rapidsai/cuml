...
/** @note FIL only supports inference with single precision. TODO(canonizer):
 *       parameterize the functions and structures by the data type and the
 ...
 */
enum algo_t {
  /** choose the algorithm automatically; currently chooses NAIVE for sparse
   forests and BATCH_TREE_REORG for dense ones */
  ALGO_AUTO,
  /** naive algorithm: 1 thread block predicts 1 row; the row is cached in shared
   memory, and the trees are distributed cyclically between threads */
  NAIVE,
  /** tree reorg algorithm: same as naive, but the tree nodes are rearranged into a
   more coalescing-friendly layout: for every node position, nodes of all trees
   at that position are stored next to each other */
  TREE_REORG,
  /** batch tree reorg algorithm: same as tree reorg, but predictions multiple rows
   (up to 4) in a single thread block */
  BATCH_TREE_REORG
};
...
/** free deletes forest and all resources held by it; after this, forest is no
 * longer usable
 *
...
/** predict predicts on data (with n rows) using forest and writes results into
 * preds; the number of columns is stored in forest, and both preds and data
 * point to GPU memory
 *
 * @param h             cuML handle used by this function
 * @param f             forest used for predictions
 * @param preds         array in GPU memory to store predictions into size ==
 *                      predict_proba ? (2*num_rows) : num_rows
 ...
 * @param predict_proba for classifier models, this forces to output both class
 *                      probabilities instead of binary class prediction. format
 *                      matches scikit-learn API
 */
