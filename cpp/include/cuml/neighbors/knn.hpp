...
/**
 * @brief Flat C++ API function to perform a brute force knn on a series of
 *        input arrays and combine the results into a single output array for
 ...
 *                           This is ignored if the metric_type is not
 *                           Minkowski.
 * @param[in]  expanded      should lp-based distances be returned in their
 *                           expanded form (e.g., without raising to the 1/p
 *                           power).
 */
...
/**
 * @brief Flat C++ API function to perform a knn classification using a given a
 *        vector of label arrays. This supports multilabel classification by
 *        classifying on multiple label arrays. Note that each label is
 *        classified independently, as is done in scikit-learn.
 *
 * @param[in]  handle       the cuml handle to use
 * @param[out] out          output array on device (size n_samples * size of y
 *                          vector)
 * @param[in]  knn_indices  index array on device resulting from knn query (size
 *                          n_samples * k)
 * @param[in]  y            vector of label arrays on device vector size is
 *                          number of (size n_samples)
...
/**
 * @brief Flat C++ API function to perform a knn regression using a given a
 *        vector of label arrays. This supports multilabel regression by
 *        clasifying on multiple label arrays. Note that each label is
 *        classified independently, as is done in scikit-learn.
 *
 * @param[in]  handle       the cuml handle to use
...
/**
 * @brief Flat C++ API function to compute knn class probabilities using a
 *        vector of device arrays containing discrete class labels. Note that
 *        the output is a vector, which is
 *
 * @param[in]  handle       the cuml handle to use
 * @param[out] out          vector of output arrays on device. vector size =
 *                          n_outputs. Each array should have size(n_samples,
 *                          n_classes)
 * @param[in]  knn_indices  array on device of knn indices (size n_samples * k)
 * @param[in]  y            array of labels on device (size n_samples)
