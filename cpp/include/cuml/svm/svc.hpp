 ...
 * @f[ f(x_i) = \sum_{j=1}^n_support K(x_i, x_j) * dual_coefs[j] + b)
 * @f]
 *
 * pred(x_i) = label[sign(f(x_i))], if predict_class==true, or pred(x_i) =
 * f(x_i),       if predict_class==falsee.
 *
 * @tparam math_t floating point type
 *
 ...
 *
 * This is a Scikit-Learn like wrapper around the stateless C++ functions. See
 * Issue #456 for general discussion about stateful Sklearn like wrappers.
 *
 * The classifier will be fitted using the SMO algorithm in dual space.
 *
 * The decision function takes the following form
 * @f[ sign\left( \sum_{i=1}^{N_{support}} y_i \alpha_i K(x_i,x) + b \right),
 * @f] where
 * @f$x_i\f$ are the support vectors, and
 * @f$ y_i \alpha_i
 * @f$ are the dual coordinates.
 *
...
class SVC {
 public:
   ...
   * @brief Fit a support vector classifier to the training data.
   *
   *        Each row of the input data stores a feature vector. We use the SMO
   *        method to fit the SVM.
   *
   * @param     input         device pointer for the input data in column major
   *                          format. Size n_rows x n_cols.
   ...
   * @param[out] preds  device pointer to store the predicted class labels. Size
   *                    [n_rows]. Should be allocated on entry.
   */
   ...
   * @param[in]  n_rows number of vectors
   * @param[in]  n_cols number of features
   * @param[out] preds  device pointer to store the decision function value Size
   *                    [n_rows]. Should be allocated on entry.
   */
