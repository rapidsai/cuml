 ...
 *
 *        Least Angle Regression (LAR or LARS) is a model selection algorithm.
 *        It builds up the model using the following algorithm:
 *
 ...
 *
 *        Note: this algorithm assumes that the offset is removed from X and y,
 *        and each feature is normalized:
 * - sum_i y_i = 0,
 ...
 *                   size [n_rows * n_cols]. Note that the columns of X will be
 *                   permuted if the Gram matrix is not specified. It is
 *                   expected that X is normalized so that each column has zero
 *                   mean and unit variance.
 * @param n_rows     number of training samples
 * @param n_cols     number of feature columns
 * @param y          device array of the regression targets, size [n_rows]. y
 *                   should be normalized to have zero mean.
 ...
 *                   regularization path. Must be allocated on entry, size
 *                   [max_iter+1].
 * @param n_active   host pointer to return the number of active elements
 *                   (scalar)
 * @param Gram       device array containing Gram matrix containing X.T * X. Can
 *                   be nullptr.
 ...
 *                   here. Must be nullptr, or a device array already allocated
 *                   on entry. Size [max_iter * (max_iter+1)].
 ...
 * @param active_idx device array containing the indices of active variables.
 *                   Only these columns of X will be used for prediction, size
 *                   [n_active].
 * @param intercept  The intercept
 * @param preds      device array to store the predictions, size [n_rows]. Must
 *                   be allocated on entry.
 */
template <typename math_t, typename idx_t>
