...
struct DecisionTreeParams {
  ...
  /**
   * Control bootstrapping for features. If features are drawn with or without
   * replacement
   */
  bool bootstrap_features;
  /**
   * Whether a quantile needs to be computed for individual trees in RF.
   * Default: compute quantiles once per RF. Only affects GLOBAL_QUANTILE
   * split_algo.
   */
  bool quantile_per_tree;
  /**
   * Node split criterion. GINI and Entropy for classification, MSE or MAE for
   * regression.
   */
  CRITERION split_criterion;
  /**
   * Minimum impurity decrease required for spliting a node. If the impurity
   * decrease is below this value, node is leafed out. Default is 0.0
   */
   ...
   * tree training implementation would be used: split_algo = 1
   * (GLOBAL_QUANTILE) max_features = 1.0 (Feature sub-sampling disabled)
 ...
 * @param[in]     cfg_max_features             maximum number of features;
 *                                             default 1.0f
 * @param[in]     cfg_n_bins                   number of bins; default 8
 * @param[in]     cfg_split_algo               split algorithm; default
 *                                             SPLIT_ALGO::HIST
 * @param[in]     cfg_min_samples_leaf         min. rows in each leaf node;
 *                                             default 1
 * @param[in]     cfg_min_samples_split        min. rows needed to split an
 *                                             internal node; default 2
 * @param[in]     cfg_min_impurity_decrease    split a node only if its
 *                                             reduction in impurity is more
 *                                             than this value
 * @param[in]     cfg_bootstrap_features       bootstrapping for features;
 *                                             default false
 * @param[in]     cfg_split_criterion          split criterion; default
 *                                             CRITERION_END, i.e., GINI for
 *                                             classification or MSE for
 *                                             regression
 * @param[in]     cfg_quantile_per_tree        compute quantile per tree;
 *                                             default false
 * @param[in]     cfg_use_experimental_backend If set to true, experimental
 *                                             batched backend is used (provided
 *                                             other conditions are met).
 *                                             Default is false.
 * @param[in]     cfg_max_batch_size           Maximum number of nodes that can
 *                                             be processed in a batch. This is
 *                                             used only for batched-level algo.
 *                                             Default value 128.
 */
 ...
 * @param[in, out] tree           CPU pointer to TreeMetaDataNode. User
 *                                allocated.
 * @param[in]      data           train data (nrows samples, ncols features) in
 *                                column major format, excluding labels. Device
 *                                pointer.
 * @param[in]      ncols          number of features (i.e., columns) excluding
 *                                target feature.
 * @param[in]      nrows          number of training data samples of the whole
 *                                unsampled dataset.
 * @param[in]      labels         1D array of target features (int only). One
 *                                label per training sample. Device pointer.
 *                                Assumption: labels need to be preprocessed to
 *                                map to ascending numbers from 0; needed for
 *                                current gini impl. in decision tree.
 * @param[in,out]  rowids         array of n_sampled_rows integers in [0, nrows)
 *                                range. Device pointer. The same array is then
 *                                rearranged when splits are made, allowing us
 *                                to construct trees without rearranging the
 *                                actual dataset.
 * @param[in]      n_sampled_rows number of training samples, after sampling. If
 *                                using decision tree directly over the whole
 *                                dataset: n_sampled_rows = nrows
 * @param[in]      unique_labels  number of unique label values. Number of
 *                                categories of classification.
 * @param[in]      tree_params    Decision Tree training hyper parameter struct.
 * @param[in]      seed           Controls the randomness in tree
 *                                fitting/growing algorithm.
 *
 * @{
 */
 ...
 *        feature supported. Inference of trees is CPU only for now.
 *
 * @param[in]     handle      raft::handle_t (currently unused; API placeholder)
 * @param[in]     tree        CPU pointer to TreeMetaDataNode.
 * @param[in]     rows        test data (n_rows samples, n_cols features) in row
 *                            major format. Current impl. expects a CPU pointer.
 *                            TODO future API change.
 * @param[in]     n_rows      number of  data samples.
 * @param[in]     n_cols      number of features (excluding target feature).
 * @param[in,out] predictions n_rows predicted labels. Current impl. expects a
 *                            CPU pointer, user allocated. TODO future API
 *                            change.
 * @param[in]     verbosity   verbosity level for logging messages during
 *                            execution. A negative value means to not perform
 *                            an explicit `setLevel()` call, but to continue
 *                            with the level that the caller itself might have
 *                            set.
 *
 * @{
 */
 ...
 *        supported. Inference of trees is CPU only for now.
 *
 * @param[in]     handle      raft::handle_t (currently unused; API placeholder)
 * @param[in]     tree        CPU pointer to TreeMetaDataNode.
 * @param[in]     rows        test data (n_rows samples, n_cols features) in row
