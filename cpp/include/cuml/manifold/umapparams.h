...
class UMAPParams {
 public:
  ...
  /**
   * The number of neighbors to use to approximate geodesic distance. Larger
     numbers induce more global estimates of the manifold that can miss finer
     detail, while smaller values will focus on fine manifold structure to the
     detriment of the larger picture.
  */
  int n_neighbors = 15;
   ...
   * The effective minimum distance between embedded points. Smaller values will
     ...
     manifold are drawn closer together, while larger values will result on a
     more even dispersal of points. The value should be set relative to the
     ``spread`` value, which determines the scale at which embedded points will
     be spread out.
  */
  float min_dist = 0.1;
  ...
  */
   ...
   * Interpolate between (fuzzy) union and intersection as the set operation
     ...
     simplicial sets. Both fuzzy set operations use the product t-norm. The
     value of this parameter should be between 0.0 and 1.0; a value of 1.0 will
     use a pure fuzzy union, while 0.0 will use a pure fuzzy intersection.
  */
  float set_op_mix_ratio = 1.0;

  /**
   * The local connectivity required -- i.e. the number of nearest neighbors
     that should be assumed to be connected at a local level. The higher this
     value the more connected the manifold becomes locally. In practice this
     should be not more than the local intrinsic dimension of the manifold.
  */
  ...
  */
   ...
   * The number of negative samples to select per positive sample in the
     ...
     repulsive force being applied, greater optimization cost, but slightly more
     accuracy.
  */
   ...
   * For transform operations (embedding new points using a trained model_ this
     will control how aggressively to search for nearest neighbors. Larger
