...
struct treelite_traversal_node : public traversal_node<TREELITE_NODE_ID_T> {
  ...
  auto default_distant() const {
    return tl_tree_.DefaultChild(node_id_) == distant_child();
  }
  ...
  auto get_categories() const {
    return tl_tree_.CategoryList(node_id_);
  }
 ...
 private:
  ...
  auto left_is_hot() const {
    auto result = false;
    if (is_categorical()) {
        if (tl_tree_.CategoryListRightChild(node_id_)) {
          result = true;
        }
    } else {
      auto tl_operator = tl_tree_.ComparisonOp(node_id_);
...
template <typename tl_threshold_t, typename tl_output_t>
struct treelite_traversal_forest : public traversal_forest<treelite_traversal_node<tl_threshold_t, tl_output_t>> {
 private:
  using base_type = traversal_forest<
    treelite_traversal_node<tl_threshold_t, tl_output_t>
  >;
...
void node_transform(
  ...
  node_for_each<order>(tl_model, [&output_iter, &lambda](auto&& tree_id, auto&& node, auto&& depth, auto&& parent_index) {
...
auto node_accumulate(
  treelite::Model const& tl_model,
  T init,
  lambda_t&& lambda
) {
  auto result = init;
  node_for_each<order>(tl_model, [&result, &lambda](auto&& tree_id, auto&& node, auto&& depth, auto&& parent_index) {
