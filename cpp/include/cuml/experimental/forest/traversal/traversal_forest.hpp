...
struct traversal_forest {
  ...
  traversal_forest(std::vector<node_uid_type>&& root_node_uids) :
    root_node_uids_{root_node_uids} {}
  ...
  void for_each(lambda_t&& lambda) const {
    auto to_be_visited = detail::traversal_container<
      order,
      std::conditional_t<
        order == forest_order::layered_children_segregated ||
      ...
      >
    >{};
    auto parent_indices = detail::traversal_container<
      order,
      index_type
    >{};
    ...
    if constexpr (
      ...
      for (auto const& root_node_uid : root_node_uids_) {
        ...
        while(!to_be_visited.empty()) {
          ...
          if (!node.is_leaf()) {
            auto hot_uid = std::make_pair(std::make_pair(
              node_uid.first,
              node.hot_child()
            ), depth + index_type{1});
            auto distant_uid = std::make_pair(std::make_pair(
              node_uid.first,
              node.distant_child()
            ), depth + index_type{1});
 ...
 private:
  auto get_node(node_uid_type node_uid) const {
