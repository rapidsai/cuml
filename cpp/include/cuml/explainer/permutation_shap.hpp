 ...
 * adding a forward and backward permutation pass of the observation `row` on
 * the positions defined by `idx`. Example:
 *
 * background = [[0, 1, 2], [3, 4, 5], [6, 7, 8]] idx = [2, 0, 1] row = [100,
 * 101, 102] output: [[  0,   1,   2] [  3,   4,   5] [  6,   7,   8] [  0,   1,
 * 102] [  3,   4, 102] [  6,   7, 102] [100,   1, 102] [100,   4, 102] [100,
 * 7, 102] [100, 101, 102] [100, 101, 102] [100, 101, 102] [100, 101,   2] [100,
 * 101,   5] [100, 101,   8] [  0, 101,   2] [  3, 101,   5] [  6, 101,   8] [
 * 0,   1,   2] [  3,   4,   5] [  6,   7,   8]]
 *
 * @param[in]  handle     cuML handle
 * @param[out] dataset    generated data in either row major or column major
 *                        format, depending on the `row_major` parameter [on
 *                        device] [dim = (2 * ncols * nrows_bg + nrows_bg) *
 *                        ncols]
 ...
 * background = [[0, 1, 2], [3, 4, 5], [6, 7, 8]] idx = [2, 0, 1] row = [100,
 * 101, 102] output: [[  0,   1,   2] [  3,   4,   5] [  6,   7,   8] [  0,   1,
 * 102] [  3,   4, 102] [  6,   7, 102] [100,   1,   2] [100,   4,   5] [100,
 * 7,   8] [  0, 101,   2] [  3, 101,   5] [  6, 101,   8]]
 *
 * @param[in]  handle     cuML handle
 * @param[out] dataset    generated data [on device] [dim = (2 * ncols *
 *                        nrows_bg + nrows_bg) * ncols]
 * @param[in]  background background data [on device] [dim = ncols * nrows_bg]
 * @param[in]  nrows_bg   number of rows in background dataset
 * @param[in]  ncols      number of columns
 * @param[in]  row        row to scatter in a permutated fashion [dim = ncols]
 ...
 * called with the permutation dataset, to estimate the SHAP values. It is
 * equivalent to the Python code: for i,ind in enumerate(idx): shap_values[ind]
 * += y_hat[i + 1] - y_hat[i] for i,ind in enumerate(idx): shap_values[ind] +=
 * y_hat[i + ncols] - y_hat[i + ncols + 1]
 *
 * @param[in]  handle      cuML handle
