 ...
 * the sum of squared errors by the total sum of squares with single precision.
 *
...
/**
 * Calculates the "Coefficient of Determination" (R-Squared) score normalizing
 * the sum of squared errors by the total sum of squares with double precision.
 *
 * This score indicates the proportionate amount of variation in an expected
 * response variable is explained by the independent variables in a linear
 * regression model. The larger the R-squared value, the more variability is
 ...
 *
 * @param handle raft::handle_t
 * @param y      Array of response variables of the first clustering
 *               classifications
 * @param y_hat  Array of response variables of the second clustering
 *               classifications
 * @param n      Number of elements in y and y_hat
 *
 ...
 * distance (a) and the mean nearest-cluster distance (b) for each sample. The
 * Silhouette Coefficient for a sample is (b - a) / max(a, b). To clarify, b is
 * the distance between a sample and the nearest cluster that the sample is not
 * a part of. Note that Silhouette Coefficient is only defined if number of
 ...
 * @param silScores Array that is optionally taken in as input if required to be
 *                  populated with the silhouette score for every sample (1 x
 *                  nRows), else nullptr is passed
 * @param metric    the numerical value that maps to the type of distance metric
...
namespace Batched {
/**
 * Calculates Batched "Silhouette Score" by tiling the pairwise distance matrix
 * to remove use of quadratic memory
 *
 * The Silhouette Coefficient is calculated using the mean intra-cluster
 * distance (a) and the mean nearest-cluster distance (b) for each sample. The
 * Silhouette Coefficient for a sample is (b - a) / max(a, b). To clarify, b is
 ...
 * @param[in]  y        Array containing labels for every data sample (1 x
 *                      n_rows)
 * @param[in]  n_labels number of Labels
 * @param[out] scores   Array that is optionally taken in as input if required
 *                      to be populated with the silhouette score for every
 *                      sample (1 x nRows), else nullptr is passed
 * @param[in]  chunk    the row-wise chunk size on which the pairwise distance
 *                      matrix is tiled
 * @param[in]  metric   the numerical value that maps to the type of distance
 *                      metric to be used in the calculations
 *
 * @return { description_of_the_return_value }
 */
 ...
 * approximates the probability distribution P It is often also used as a
 * 'distance metric' between two probablity ditributions (not symmetric)
 *
 * @param handle raft::handle_t
 ...
 *
 * Mutual Information is a measure of the similarity between two labels of the
 * same data.
 *
 * @param handle            raft::handle_t
 * @param y                 Array of response variables of the first clustering
 ...
 *
 * v-measure is the harmonic mean between the homogeneity and completeness
 * scores of 2 cluster classifications
 *
 * @param handle            raft::handle_t
 * @param y                 truth labels
 * @param y_hat             predicted labels
...
/**
 * @brief Calculates the ij pairwise distances between two input arrays of float
 *        type
 *
 * @param handle     raft::handle_t
 * @param x          pointer to the input data samples array (mRows x kCols)
