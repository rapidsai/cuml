...
*/
 ...
 namespace MLCommon {
    ...
    class PCAOpgTest : public testing::TestWithParam<PCAOpgParams> {
     public:
      void SetUp() {
        ...
        if (myRank == 0) {
          std::cout << "Testing PCA of " << params.M << " x "
                    << params.N << " matrix" << std::endl;
        }
        ...
        components =
          (T*)allocator->allocate(prmsPCA.n_components * prmsPCA.n_cols * sizeof(T), stream);
        ...
        mu =
          (T*)allocator->allocate(prmsPCA.n_cols * sizeof(T), stream);

        noise_vars =
          (T*)allocator->allocate(prmsPCA.n_components * sizeof(T), stream);
    
        ML::PCA::opg::fit(*handle, inParts, desc, 
                            components, explained_var, explained_var_ratio, singular_vals,
                        mu, noise_vars, prmsPCA, false);
      ...
      void TearDown() {
        const std::shared_ptr<deviceAllocator> allocator = handle->getDeviceAllocator();
        cudaStream_t stream = handle->getStream();

        allocator->deallocate(components, prmsPCA.n_components * prmsPCA.n_cols * sizeof(T), stream);
    
        allocator->deallocate(explained_var, prmsPCA.n_components * sizeof(T), stream);

        allocator->deallocate(explained_var_ratio, prmsPCA.n_components * sizeof(T), stream);

        allocator->deallocate(singular_vals, prmsPCA.n_components * sizeof(T), stream);
     ...
     protected:
      ...
      T *components, *explained_var, *explained_var_ratio, *singular_vals, *mu, *noise_vars;
    };
    ...
    TEST_P(PCAOpgTestF, Result) {
      if (myRank == 0) {
        MLCommon::myPrintDevVector("Singular Values: ", singular_vals, params.N_components);
        MLCommon::myPrintDevVector("Explained Variance: ", explained_var, params.N_components);
        MLCommon::myPrintDevVector("Explained Variance Ratio: ", explained_var_ratio, params.N_components);
        std::cout << "Components: ";
