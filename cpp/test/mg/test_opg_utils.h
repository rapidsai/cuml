...
namespace opg {
  ...
  /**
    ...
    */
   template<typename T>
   void data_alloc(Matrix::Data<T> &data, const Matrix::Descriptor &desc,
       int rank, bool setZero = false) {
     ...
     if (setZero)
       CUDA_CHECK(cudaMemset(data.ptr, 0, data.totalSize));
   }
   ...
   */
  template <typename Type>
  void redistribute(Matrix::Data<Type> &out,
                    const Matrix::Descriptor &outDesc,
