...
class GeneticEvolutionTest : public ::testing::Test{
  ...
  void SetUp() override {
    ...
    d_train = (float*)handle.get_device_allocator()->allocate(n_cols*n_tr_rows*sizeof(float),stream);
    d_trainlab = (float*)handle.get_device_allocator()->allocate(n_tr_rows*sizeof(float),stream);
    d_test = (float*)handle.get_device_allocator()->allocate(n_cols*n_tst_rows*sizeof(float),stream);
    ...
    // Memcpy HtoD
    CUDA_CHECK(cudaMemcpyAsync(d_train,h_train.data(),n_cols*n_tr_rows*sizeof(float),cudaMemcpyHostToDevice,stream));
    CUDA_CHECK(cudaMemcpyAsync(d_trainlab,h_trainlab.data(),n_tr_rows*sizeof(float),cudaMemcpyHostToDevice,stream));
    CUDA_CHECK(cudaMemcpyAsync(d_test,h_test.data(),n_cols*n_tst_rows*sizeof(float),cudaMemcpyHostToDevice,stream));
    CUDA_CHECK(cudaMemcpyAsync(d_testlab,h_testlab.data(),n_tst_rows*sizeof(float),cudaMemcpyHostToDevice,stream));
    CUDA_CHECK(cudaMemcpyAsync(d_trainwts,h_trainwts.data(),n_tr_rows*sizeof(float),cudaMemcpyHostToDevice,stream));
    CUDA_CHECK(cudaMemcpyAsync(d_testwts,h_testwts.data(),n_tst_rows*sizeof(float),cudaMemcpyHostToDevice,stream));
  }
...
TEST_F(GeneticEvolutionTest,SymReg){
  ...
  final_progs  = (program_t)handle.get_device_allocator()->allocate(hyper_params.population_size*sizeof(program),stream);
  ...
  symFit(handle,d_train,d_trainlab,d_trainwts,n_tr_rows,n_cols,hyper_params,final_progs,history);
  ...
  cuml::genetic::symRegPredict(handle,d_test,n_tst_rows,final_progs+best_idx,d_predlabels.data());

  std::vector<float> h_predlabels(n_tst_rows,0.0f);
  CUDA_CHECK(cudaMemcpy(h_predlabels.data(),d_predlabels.data(),n_tst_rows * sizeof(float),cudaMemcpyDeviceToHost));
