 ...
 protected:
  void ConcatenateTreeliteModels() {
    ...
    TREELITE_CHECK(TreeliteCompilerGenerateCode(
      compiler, treelite_indiv_handels[0], verbose, dir_name.c_str()));
    ...
    TREELITE_CHECK(TreeliteFreeModel(treelite_indiv_handels[0]));
    TREELITE_CHECK(TreeliteFreeModel(treelite_indiv_handels[1]));
    TREELITE_CHECK(TreeliteFreeModel(treelite_indiv_handels[2]));
  }
