...
struct ProtoCategorySets {
  ...
  {
    ASSERT(bits.size() == fid_cats.size(),
           "internal error: ProtoCategorySets::bits.size() != "
           "ProtoCategorySets::fid_cats.size()");
    std::vector<uint8_t> flat;
    for (std::vector<uint8_t> v : bits) {
      for (uint8_t b : v)
        flat.push_back(b);
    }
    return {flat, fid_cats};
  }
};
...
std::vector<ChildIndexTestParams> params = {
  ...
  // cannot match ( < 0 and realistic fid_cats)
  CHILD_INDEX_TEST_PARAMS(node         = NODE(is_categorical = true),
                          cso.bits     = {},
                          cso.fid_cats = {11},
                          input        = -5,
                          correct      = 1),
  // Skipping category < 0 and dummy categorical node: fid_cats == 0. Prevented by FIL
  ...
  CHILD_INDEX_TEST_PARAMS(node         = NODE(is_categorical = true),
                          ...
                          correct      = 1),
  // cannot match ( >= fid_cats and integer)
  CHILD_INDEX_TEST_PARAMS(node         = NODE(is_categorical = true),
                          cso.bits     = {0b1111'1111},
                          cso.fid_cats = {2},
                          input        = 2,
                          correct      = 1),
  // matches ( < fid_cats because comparison is floating-point and there's no rounding)
  CHILD_INDEX_TEST_PARAMS(node         = NODE(is_categorical = true),
                          cso.bits     = {0b1111'1111},
                          cso.fid_cats = {2},
                          input        = 1.8f,
                          correct      = 2),
  // cannot match ( >= fid_cats)
  CHILD_INDEX_TEST_PARAMS(node         = NODE(is_categorical = true),
                          cso.bits     = {0b1111'1111},
                          cso.fid_cats = {2},
                          input        = 2.1f,
  ...
  CHILD_INDEX_TEST_PARAMS(node         = NODE(is_categorical = true),
                          ...
                          input        = 1,
                          correct      = 1),
  // cannot match (fid_cats[fid=1] <= input)
  CHILD_INDEX_TEST_PARAMS(node         = NODE(is_categorical = true),
                          node.fid     = 1,
                          cso.bits     = {0b0000'0101},
                          cso.fid_cats = {3, 1},
