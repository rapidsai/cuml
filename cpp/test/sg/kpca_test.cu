...
class KPcaTest : public ::testing::TestWithParam<KPcaInputs<T>> {
 ...
 protected:
  void fitThenTransform()
  {
    ...
    raft::update_device(trans_data_ref.data(),
                        params.trans_data_ref_h.data(),
                        params.n_rows * params.n_components,
                        stream);
    raft::update_device(
      eigenvalues_ref.data(), params.eigenvalues_ref_h.data(), params.n_components, stream);
    raft::update_device(eigenvectors_ref.data(),
                        params.eigenvectors_ref_h.data(),
                        params.n_rows * params.n_components,
                        stream);
    ...
    *n_components     = params.n_components;
    ...
    raft::update_device(
      eigenvectors.data(), alphas.data(), prms.n_rows * prms.n_components, stream);
    raft::update_device(eigenvalues.data(), lambdas.data(), prms.n_components, stream);
    handle.sync_stream(stream);
    kpcaTransform(handle,
                  data.data(),
                  data.data(),
                  eigenvectors.data(),
                  eigenvalues.data(),
  ...
  void fitTransformTest()
  {
    ...
    *n_components     = prms.n_components;
    ...
    kpcaTransformWithFitData(
      handle, eigenvectors.data(), eigenvalues.data(), trans_data.data(), prms, stream);
  }

 protected:
  KPcaInputs<T> params;

  rmm::device_uvector<T> data, trans_data, eigenvectors, eigenvalues, trans_data_ref,
    eigenvectors_ref, eigenvalues_ref, lambdas, alphas;
...
std::vector<float> lin_trans_data_ref_h   = {
  -2.32318647, -0.35170213, 2.6748886, -0.39794495, 0.65716145, -0.25921649};
KPcaInputs<float> linear_inputs                   = {tolerance,
                                                     n_rows,
                                                     n_cols,
                                                     n_components,
                                                     algo,
                                                     lin_kern,
                                                     data_h,
                                                     lin_eigenvectors_ref_h,
                                                     lin_eigenvalues_ref_h,
                                                     lin_trans_data_ref_h,
                                                     false};
KPcaInputs<float> linear_inputs_for_fit_transform = {tolerance,
                                                     n_rows,
                                                     n_cols,
                                                     n_components,
                                                     algo,
...
raft::distance::kernels::KernelParams poly_kern = {
  raft::distance::kernels::POLYNOMIAL, 3, 1.0 / 2.0f, 1};
...
KPcaInputs<float> poly_inputs            = {tolerance,
                                            n_rows,
                                            n_cols,
                                            n_components,
                                            algo,
                                            poly_kern,
                                            data_h,
                                            poly_eigenvectors_ref_h,
                                            poly_eigenvalues_ref_h,
                                            poly_trans_data_ref_h,
                                            false};
KPcaInputs<float> poly_inputs_for_fit_transform = {tolerance,
                                                   n_rows,
                                                   n_cols,
                                                   n_components,
                                                   algo,
...
KPcaInputs<float> rbf_inputs              = {tolerance,
                                             n_rows,
                                             n_cols,
                                             n_components,
                                             algo,
                                             rbf_kern,
                                             data_h,
                                             rbf_eigenvectors_ref_h,
                                             rbf_eigenvalues_ref_h,
                                             rbf_trans_data_ref_h,
                                             false};
KPcaInputs<float> rbf_inputs_for_fit_transform = {tolerance,
                                                  n_rows,
                                                  n_cols,
                                                  n_components,
                                                  algo,
...
const std::vector<KPcaInputs<float>> inputs_f = {linear_inputs,
                                                 poly_inputs,
                                                 rbf_inputs,
                                                 linear_inputs_for_fit_transform,
                                                 poly_inputs_for_fit_transform,
                                                 rbf_inputs_for_fit_transform};

typedef KPcaTest<float> KPcaTestEigenvaluesF;
TEST_P(KPcaTestEigenvaluesF, Result)
{
  ASSERT_TRUE(MLCommon::devArrMatch(eigenvalues.data(),
                                    eigenvalues_ref.data(),
                                    params.n_components,
...
{
  ASSERT_TRUE(MLCommon::devArrMatch(eigenvectors.data(),
                                    eigenvectors_ref.data(),
                                    (params.n_rows * params.n_components),
                                    MLCommon::CompareApproxAbs<float>(params.tolerance)));
}
...
{
  ASSERT_TRUE(MLCommon::devArrMatch(trans_data.data(),
                                    trans_data_ref.data(),
                                    (params.n_rows * params.n_components),
                                    MLCommon::CompareApproxAbs<float>(params.tolerance)));
}
...
INSTANTIATE_TEST_CASE_P(KPcaTests, KPcaTestEigenvectorsF, ::testing::ValuesIn(inputs_f));

INSTANTIATE_TEST_CASE_P(KPcaTests, KPcaTestTransDataF, ::testing::ValuesIn(inputs_f));
...
std::vector<double> lin_trans_data_ref_hd   = {
  -2.32318647, -0.35170213, 2.6748886, -0.39794495, 0.65716145, -0.25921649};
KPcaInputs<double> linear_inputs_d                   = {tolerance_d,
                                                        n_rows,
                                                        n_cols,
                                                        n_components,
                                                        algo,
                                                        lin_kern,
                                                        data_hd,
                                                        lin_eigenvectors_ref_hd,
                                                        lin_eigenvalues_ref_hd,
                                                        lin_trans_data_ref_hd,
                                                        false};
KPcaInputs<double> linear_inputs_for_fit_transform_d = {tolerance_d,
                                                        n_rows,
                                                        n_cols,
                                                        n_components,
                                                        algo,
...
std::vector<double> poly_trans_data_ref_hd   = {
  -22.9760, -10.8554, 33.8314, -8.8438, 11.2426, -2.3987};
KPcaInputs<double> poly_inputs_d                   = {tolerance_d,
                                                      n_rows,
                                                      n_cols,
                                                      n_components,
                                                      algo,
                                                      poly_kern,
                                                      data_hd,
                                                      poly_eigenvectors_ref_hd,
                                                      poly_eigenvalues_ref_hd,
                                                      poly_trans_data_ref_hd,
                                                      false};
KPcaInputs<double> poly_inputs_for_fit_transform_d = {tolerance_d,
                                                      n_rows,
                                                      n_cols,
                                                      n_components,
                                                      algo,
...
KPcaInputs<double> rbf_inputs_d             = {tolerance_d,
                                               n_rows,
                                               n_cols,
                                               n_components,
                                               algo,
                                               rbf_kern,
                                               data_hd,
                                               rbf_eigenvectors_ref_hd,
                                               rbf_eigenvalues_ref_hd,
                                               rbf_trans_data_ref_hd,
                                               false};
KPcaInputs<double> rbf_inputs_for_fit_transform_d = {tolerance_d,
                                                     n_rows,
                                                     n_cols,
                                                     n_components,
                                                     algo,
...
const std::vector<KPcaInputs<double>> inputs_d = {linear_inputs_d,
                                                  poly_inputs_d,
                                                  rbf_inputs_d,
                                                  linear_inputs_for_fit_transform_d,
                                                  poly_inputs_for_fit_transform_d,
                                                  rbf_inputs_for_fit_transform_d};
...
INSTANTIATE_TEST_CASE_P(KPcaTests, KPcaTestEigenvectorsD, ::testing::ValuesIn(inputs_d));

INSTANTIATE_TEST_CASE_P(KPcaTests, KPcaTestTransDataD, ::testing::ValuesIn(inputs_d));
