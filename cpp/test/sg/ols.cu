...
TEST_P(OlsTestF, Fit)
{
  ...
  ASSERT_TRUE(
    devArrMatch(coef_sc_ref.data(), coef_sc.data(), 1, MLCommon::CompareApproxAbs<float>(params.tol)));
}
...
TEST_P(OlsTestD, Fit)
{
  ...
  ASSERT_TRUE(
    devArrMatch(coef_sc_ref.data(), coef_sc.data(), 1, MLCommon::CompareApproxAbs<double>(params.tol)));
}
