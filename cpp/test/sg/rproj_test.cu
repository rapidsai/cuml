...
class RPROJTest : public ::testing::Test {
 protected:
  ...
  void epsilon_check() {
    ASSERT(d_input != NULL and d_output1 != NULL and d_output2 != NULL,
           "Null pointer!");
    ...
    lwork =
      getWorkspaceSize<distance_type, T, T, T>(d_output1, d_output1, N, N, D);
    if (lwork > 0)
      allocate(work, lwork);
    else
      work = NULL;
    ...
    lwork =
      getWorkspaceSize<distance_type, T, T, T>(d_output2, d_output2, N, N, D);
    if (lwork > 0)
      allocate(work, lwork);
    else
      work = NULL;
