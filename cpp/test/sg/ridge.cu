...
class RidgeTest : public ::testing::TestWithParam<RidgeInputs<T>> {
 protected:
  void basicTest() {
    ...
    ridgeFit(handle, data, params.n_row, params.n_col, labels, &alpha,
             1, coef, &intercept, false, false, stream, params.algo);

    ridgePredict(handle, pred_data, params.n_row_2, params.n_col,
  ...
  void basicTest2() {
    ...
    ridgeFit(handle, data_sc, len, 1, labels_sc, &alpha_sc, 1,
