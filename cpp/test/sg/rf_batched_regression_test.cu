 ...
 protected:
  void basicTest() {
    ...
    Datasets::make_regression(*handle, data_row_major, labels, params.n_rows,
                     params.n_cols, params.n_cols, nullptr, 1,
                     0.0f, -1, 0.0, 0.0f, false, 3536699ULL);
  ...
  void TearDown() override {
    ...
    allocator->deallocate(data, params.n_rows * params.n_cols * sizeof(T),
                          stream);
    allocator->deallocate(data_row_major, params.n_rows * params.n_cols *
                          sizeof(T), stream);
