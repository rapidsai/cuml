...
class SparseKNNTest
 ...
 protected:
  void make_data() {
    ...
    std::vector<value_t> out_dists_ref_h = {0, 4, 8, 12, 16};
  ...
  void compare() {
    std::cout << arr2Str(out_indices, 4 * k, "out_indptr", stream) << std::endl;
    std::cout << arr2Str(out_dists, 4 * k, "out_indices", stream) << std::endl;
  }
