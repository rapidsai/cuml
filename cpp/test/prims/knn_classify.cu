...
class KNNClassifyTest : public ::testing::TestWithParam<KNNClassifyInputs> {
 protected:
  void basicTest() {
    ...
    MLCommon::Random::make_blobs<float, int>(train_samples, train_labels,
    ...
    knn_classify(pred_labels, knn_indices, y,
                      params.rows, params.k, uniq_labels,
                      n_unique, alloc, stream);
...
TEST_P(KNNClassifyTestF, Fit) {
  ASSERT_TRUE(devArrMatch(train_labels, pred_labels, params.rows, Compare<int>()));
}
