...
struct RegressionInputs {
  ...
  bool hardcoded_preds; // (hardcoded_preds) ? use predictions, ref_predictions : use randomly generated arrays.
...
std::ostream &operator<<(std::ostream &os, const RegressionInputs<T> &reg_inputs) {
  os << "RegressionInputs are {" << reg_inputs.tolerance << ", " << reg_inputs.n << ", " << reg_inputs.hardcoded_preds << ", ";
  if (reg_inputs.hardcoded_preds) {
      os << "{";
      for (int i = 0; i < reg_inputs.n; i++) os << reg_inputs.predictions[i] << ", ";
      os << "}, {";
      for (int i = 0; i < reg_inputs.n; i++) os << reg_inputs.ref_predictions[i] << ", ";
      os << "}";
...
template<typename T>
void host_regression_computations(std::vector<T> &predictions, std::vector<T> &ref_predictions, const int n, std::vector<double> &regression_metrics) {
...
protected:
  void SetUp() override {
    ...
    if (params.hardcoded_preds) {
      ...
      updateDevice(d_ref_predictions, params.ref_predictions.data(), params.n, stream);
    } else {
      params.predictions.resize(params.n);
      params.ref_predictions.resize(params.n);
      Random::Rng r(params.seed);
      // randomly generate arrays
      r.uniform(d_predictions, params.n, params.predictions_range[0], params.predictions_range[1], stream);
      r.uniform(d_ref_predictions, params.n, params.ref_predictions_range[0], params.ref_predictions_range[1], stream);
      // copy to host to compute reference regression metrics
      updateHost(params.predictions.data(), d_predictions, params.n, stream);
      updateHost(params.ref_predictions.data(), d_ref_predictions, params.n, stream);
      CUDA_CHECK(cudaStreamSynchronize(stream));
    }

    MLCommon::Score::regression_metrics(d_predictions, d_ref_predictions, params.n, d_allocator, stream,
        computed_regression_metrics[0], computed_regression_metrics[1], computed_regression_metrics[2]);

    host_regression_computations(params.predictions, params.ref_predictions, params.n, ref_regression_metrics);
    CUDA_CHECK(cudaStreamSynchronize(stream));
  }
...
const std::vector<RegressionInputs<float>> regression_inputs_float = {
  ...
  {0.00001f, 2, true, {10.2f, 40.2f}, {20.2f, 80.2f}, {}, {}, 1234ULL}, // two elements, mean same as median
  // next three inputs should result in identical regression metrics values
  {0.00001f, 6, true, {10.5f, 20.5f, 30.5f, 40.5f, 50.5f, 60.5f}, {20.5f, 40.5f, 55.5f, 80.5f, 100.5f, 120.5f}, {}, {}, 1234ULL}, // diffs all negative, reverse sorted
  {0.00001f, 6, true, {20.5f, 40.5f, 55.5f, 80.5f, 100.5f, 120.5f}, {10.5f, 20.5f, 30.5f, 40.5f, 50.5f, 60.5f}, {}, {}, 1234ULL}, // diffs all positive, already sorted
  {0.00001f, 6, true, {40.5f, 55.5f, 20.5f, 120.5f, 100.5f, 80.5f}, {20.5f, 30.5f, 10.5f, 60.5f, 50.5f, 40.5f}, {}, {}, 1234ULL}, // mix
  {0.00001f, 6, true, {10.5f, 20.5f, 30.5f, 40.5f, 50.5f, 60.5f}, {10.5f, 20.5f, 30.5f, 40.5f, 50.5f, 60.5f}, {}, {}, 1234ULL}, // identical predictions (0 error)
  {0.00001f, 6, true, {10.5f, 20.5f, 30.5f, 40.5f, 50.5f, 60.5f}, {20.5f, 30.5f, 40.5f, 50.5f, 60.5f, 70.5f}, {}, {}, 1234ULL}, // predictions[i] - ref_predictions[i] const for each i
  {0.00001f, 2048, false, {}, {}, {-2048.0f, 2048.0f}, {-2048.0f, 2048.0f}, 1234ULL}, // random mix, even number of elements
  {0.00001f, 2049, false, {}, {}, {-2048.0f, 2048.0f}, {-2048.0f, 2048.0f}, 1234ULL}, // random mix, odd number of elements
  {0.00001f, 1024, false, {}, {}, {0.0f, 2048.0f}, {8192.0f, 16384.0f}, 1234ULL}, // random mix, diffs are all negative
  {0.00001f, 1024, false, {}, {}, {8192.0f, 16384.0f}, {0.0f, 2048.0f}, 1234ULL} // random mix, diffs are all positive
};
...
const std::vector<RegressionInputs<double>> regression_inputs_double = {
  ...
  {0.0000001, 6, true, {10.5, 20.5, 30.5, 40.5, 50.5, 60.5}, {20.5, 40.5, 55.5, 80.5, 100.5, 120.5}, {}, {}, 1234ULL}, // diffs all negative, reverse sorted
  {0.0000001, 6, true, {20.5, 40.5, 55.5, 80.5, 100.5, 120.5}, {10.5, 20.5, 30.5, 40.5, 50.5, 60.5}, {}, {}, 1234ULL}, // diffs all positive, already sorted
  {0.0000001, 6, true, {40.5, 55.5, 20.5, 120.5, 100.5, 80.5}, {20.5, 30.5, 10.5, 60.5, 50.5, 40.5}, {}, {}, 1234ULL}, // mix
  {0.0000001, 6, true, {10.5, 20.5, 30.5, 40.5, 50.5, 60.5}, {10.5, 20.5, 30.5, 40.5, 50.5, 60.5}, {}, {}, 1234ULL}, // identical predictions (0 error)
  {0.0000001, 6, true, {10.5, 20.5, 30.5, 40.5, 50.5, 60.5}, {20.5, 30.5, 40.5, 50.5, 60.5, 70.5}, {}, {}, 1234ULL}, // predictions[i] - ref_predictions[i] const for each i
  {0.0000001, 2048, false, {}, {}, {-2048.0, 2048.0}, {-2048.0, 2048.0}, 1234ULL}, // random mix, even number of elements
  {0.0000001, 2049, false, {}, {}, {-2048.0, 2048.0}, {-2048.0, 2048.0}, 1234ULL}, // random mix, odd number of elements
  {0.0000001, 1024, false, {}, {}, {0, 2048}, {8192.0, 16384.0}, 1234ULL}, // random mix, diffs are all negative
  {0.0000001, 1024, false, {}, {}, {8192.0, 16384.0}, {0.0, 2048}, 1234ULL} // random mix, diffs are all positive
};
...
TEST_P(RegressionMetricsTestF, Result) {
  for (int i = 0; i < 3; i++)  {
     ASSERT_TRUE(match(computed_regression_metrics[i], ref_regression_metrics[i], CompareApprox<float>(params.tolerance)));
  }
}
...
TEST_P(RegressionMetricsTestD, Result) {
  for (int i = 0; i < 3; i++)  {
     ASSERT_TRUE(match(computed_regression_metrics[i], ref_regression_metrics[i], CompareApprox<double>(params.tolerance)));
  }
}

INSTANTIATE_TEST_CASE_P(RegressionMetricsTests, RegressionMetricsTestF, ::testing::ValuesIn(regression_inputs_float));
INSTANTIATE_TEST_CASE_P(RegressionMetricsTests, RegressionMetricsTestD, ::testing::ValuesIn(regression_inputs_double));
