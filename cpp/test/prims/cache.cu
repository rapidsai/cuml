...
TEST_F(CacheTest, TestArgFirst) {
  ...
  EXPECT_TRUE(
      devArrMatchHost(idx_exp, int_array_dev, 10, Compare<int>())
  );
}
...
TEST_F(CacheTest, TestNthOccurrence) {
  ...
  EXPECT_TRUE(
      devArrMatchHost(idx_exp2, int_array_dev, 10, Compare<int>())
  );
}
...
TEST_F(CacheTest, TestRankEntries) {
  ...
  EXPECT_TRUE(
      devArrMatchHost(idx_exp, int_array_dev, 12, Compare<int>())
  );
  ...
  print_vec(int_array_dev, 12, "rank:");
  EXPECT_TRUE(
      devArrMatchHost(idx_exp, int_array_dev, 12, Compare<int>())
  );
}
...
TEST_F(CacheTest, TestSimple) {
  ...
  EXPECT_TRUE(
      devArrMatch(false, is_cached, n_ws, Compare<bool>())
  );
...
TEST_F(CacheTest, TestAssignCacheIdx) {
  ...
  //print_vec(ws_idx_dev, n_ws, "ws_idx");
  //print_vec(ws_cache_idx_dev, n_ws, "cache_set");
  ...
  EXPECT_TRUE(
      devArrMatchHost(ws_idx_exp, ws_idx_dev, n_ws, Compare<int>())
  );

  // Now the elements that have been assigned a cache slot are considered cached.
  // So a subsequent cache lookup should give us these:
  updateDevice(ws_idx_dev, ws_idx_host, n_ws, stream);
  cache.GetCacheIdxPartitioned(ws_idx_dev, n_ws, ws_cache_idx_dev, &n_cached,
    stream);
  ...
  //print_vec(ws_idx_dev, n_ws, "ws_idx");
  //print_vec(ws_cache_idx_dev, n_ws, "cache_idx_or_set");
  ...
  EXPECT_TRUE(
      devArrMatchHost(cache_idx_exp2, ws_cache_idx_dev, n_cached, Compare<int>())
  );

  //std::cout<< "Find cache slots, when not available\n";
  //print_vec(cache.cache_time.data(), cache.cache_time.size(), "cache_time");

  int non_cached = n_ws - n_cached;
  cache.AssignCacheIdx(ws_idx_dev + n_cached, non_cached, ws_cache_idx_dev +
    n_cached, stream);

  int cache_idx_exp3[6] = {-1, -1, -1, -1, -1, -1};
  EXPECT_TRUE(
    devArrMatchHost(cache_idx_exp3, ws_cache_idx_dev + n_cached, non_cached, Compare<int>())
  );
}
...
TEST_F(CacheTest, TestEvict) {
  ...
  EXPECT_TRUE(
      devArrMatchHost(ws_idx_exp, ws_idx_dev, 5, Compare<int>())
  );
  ...
  //print_vec(ws_idx_dev, n_ws, "ws_idx");
  //print_vec(ws_cache_idx_dev, n_ws, "cache_idx_or_set");
  cache.AssignCacheIdx(ws_idx_dev+n_cached, 10-n_cached, ws_cache_idx_dev +
    n_cached, stream);
  //print_vec(ws_idx_dev, n_ws, "ws_idx");
  //print_vec(ws_cache_idx_dev, n_ws, "cache_idx_or_set");
  ...
  EXPECT_TRUE(
      devArrMatchHost(ws_idx_exp3, ws_idx_dev, 10, Compare<int>())
  );
  EXPECT_TRUE(
...
TEST_F(CacheTest, TestStoreCollect) {
  ...
  cache.GetCacheIdxPartitioned(ws_idx_dev, 5, ws_cache_idx_dev, &n_cached, stream);
  cache.AssignCacheIdx(ws_idx_dev, 5, ws_cache_idx_dev, stream);
  cache.GetCacheIdxPartitioned(ws_idx_dev, 5, ws_cache_idx_dev, &n_cached, stream);

  cache.StoreCols(x_dev, 10, n_cached, ws_cache_idx_dev, stream, ws_idx_dev);
  //print_vec(cache.cache.data(), cache.cache.size(), "cache");
  cache.GetCacheIdxPartitioned(ws_idx_dev, 5, ws_cache_idx_dev, &n_cached, stream);
  ...
  for (int i=0; i<n_cached; i++) {
   //std::cout<<i<<" "<<ws_idx_host[i]<<" "<<cache_idx_host[i]<<"\n";
   EXPECT_TRUE(
      devArrMatch(x_dev + ws_idx_host[i]*n_cols,
                  tile_dev + i*n_cols,
                  n_cols, Compare<int>())
    ) << "vector " << i;
  }
  ...
  for (int i=0; i<10; i++) {
  cache.GetCacheIdxPartitioned(ws_idx_dev, 10, ws_cache_idx_dev, &n_cached, stream);
  if (i==0) {
  ...
  cache.AssignCacheIdx(ws_idx_dev+n_cached, 10-n_cached, ws_cache_idx_dev +
    n_cached, stream);
  cache.StoreCols(x_dev, 10, 10-n_cached,
    ws_cache_idx_dev + n_cached, stream, ws_idx_dev + n_cached);

  //print_vec(ws_idx_dev, n_ws, "ws_idx");
  //print_vec(ws_cache_idx_dev, n_ws, "cache_idx");
