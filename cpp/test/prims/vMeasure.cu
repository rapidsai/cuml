...
class vMeasureTest : public ::testing::TestWithParam<vMeasureParam>{
  ...
  void SetUp() override {
    ...
    truthHomogeity = MLCommon::Metrics::homogeneityScore(truthClusterArray, predClusterArray, nElements, lowerLabelRange, upperLabelRange, allocator, stream);
    truthCompleteness = MLCommon::Metrics::completenessScore(truthClusterArray, predClusterArray, nElements, lowerLabelRange, upperLabelRange, allocator, stream);

    if(truthCompleteness + truthHomogeity == 0.0) truthVMeasure = 0.0;
    else truthVMeasure = ((1+params.beta)*truthHomogeity*truthCompleteness/(params.beta*truthHomogeity + truthCompleteness));
    //calling the vMeasure CUDA implementation
    computedVMeasure = MLCommon::Metrics::vMeasureScore(truthClusterArray,predClusterArray,nElements, lowerLabelRange, upperLabelRange, allocator,stream, params.beta);
