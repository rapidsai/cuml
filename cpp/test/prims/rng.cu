  ...
  void getExpectedMeanVar(T meanvar[2]) {
    switch (params.type) {
      ...
      case RNG_Exp:
        ...
        break;
      case RNG_Rayleigh:
        meanvar[0] = params.start * MLCommon::mySqrt(T(3.1415 / 2.0));
        meanvar[1] =
...
class ScaledBernoulliTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    MLCommon::allocate(data, len * sizeof(T), stream);
  ...
  void rangeCheck() {
    T* h_data = new T[len];
    MLCommon::updateHost(h_data, data, len, stream);
...
class BernoulliTest : public ::testing::Test {
 protected:
  void SetUp() override {
    CUDA_CHECK(cudaStreamCreate(&stream));
    Rng r(42);
    MLCommon::allocate(data, len * sizeof(bool), stream);
...
class RngNormalTableTest
 ...
 protected:
  void SetUp() override {
    ...
    MLCommon::allocate(data, len);
    MLCommon::allocate(stats, 2, true);
    MLCommon::allocate(mu_vec, params.cols);
...
class RngAffineTest : public ::testing::TestWithParam<RngAffineInputs> {
 protected:
  ...
  void check() {
    ASSERT_TRUE(MLCommon::gcd(a, params.n) == 1);
