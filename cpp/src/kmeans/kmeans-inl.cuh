...
CountT getDataBatchSize(CountT n_samples, CountT n_features) {
...
DataT computeClusterCost(const cumlHandle_impl &handle,
                         Tensor<DataT, 1, IndexT> &minClusterDistance,
                         MLCommon::device_buffer<char> &workspace,
                         ReductionOpT reduction_op, cudaStream_t stream) {
  Tensor<DataT, 1, IndexT> clusterCostD({1}, handle.getDeviceAllocator(),
                                        stream);

  size_t temp_storage_bytes = 0;
  CUDA_CHECK(cub::DeviceReduce::Reduce(
    nullptr, temp_storage_bytes, minClusterDistance.data(), clusterCostD.data(),
    minClusterDistance.numElements(), reduction_op, DataT(), stream));

  workspace.resize(temp_storage_bytes, stream);

  CUDA_CHECK(cub::DeviceReduce::Reduce(
    workspace.data(), temp_storage_bytes, minClusterDistance.data(),
    clusterCostD.data(), minClusterDistance.numElements(), reduction_op,
    DataT(), stream));

  DataT clusterCostInRank;
  MLCommon::copy(&clusterCostInRank, clusterCostD.data(),
                 clusterCostD.numElements(), stream);
...
void shuffleAndGather(const cumlHandle_impl &handle,
  ...
  if (workspace) {
    ...
    Random::permute<DataT>(indices.data(), nullptr, nullptr, in.getSize(1),
                           in.getSize(0), true, stream);
  } else {
    // shuffle indices on host and copy to device...
    host_buffer<IndexT> ht_indices(handle.getHostAllocator(), stream,
                                   n_samples);
  ...
  Matrix::gather(in.data(), in.getSize(1), in.getSize(0), indices.data(),
                 n_samples_to_gather, out.data(), stream);
}
...
void countSamplesInCluster(const cumlHandle_impl &handle,
  ...
  kmeans::detail::minClusterAndDistance(handle, X, centroids, pairwiseDistance,
                                        minClusterAndDistance, workspace,
                                        metric, stream);
...
template <typename DataT, typename IndexT>
void kmeansPlusPlus(const cumlHandle_impl &handle, int verbose, int n_clusters,
                    int seed, Tensor<DataT, 2, IndexT> &C,
  ...
  int dataBatchSize =
    kmeans::detail::getDataBatchSize(n_pot_centroids, n_features);
  ...
  for (int iter = 0; iter < n_clusters; iter++) {
    LOG(verbose, "KMeans++ - Iteraton %d/%d\n", iter, n_clusters);
    ...
    LOG(verbose, "Chosing centroid-%d randomly from %d potential centroids\n",
    ...
    kmeans::detail::minClusterDistance(handle, C, centroids, pairwiseDistance,
                                       minClusterDistance, workspace, metric,
                                       stream);

    DataT clusteringCost = kmeans::detail::computeClusterCost(
      handle, minClusterDistance, workspace,
...
// constructor
template <typename DataT, typename IndexT>
KMeans<DataT, IndexT>::KMeans(const ML::cumlHandle_impl &handle, int k,
                              MLCommon::Distance::DistanceType metric,
                              kmeans::InitMethod init, int max_iterations,
                              double tolerance, int seed, int verbose)
  : _handle(handle),
    n_clusters(k),
    tol(tolerance),
...
template <typename DataT, typename IndexT>
void KMeans<DataT, IndexT>::setCentroids(const DataT *X, int n_samples,
                                         int n_features) {
  cudaStream_t stream = _handle.getStream();
  ...
  MLCommon::copy(_centroidsRawData.begin(), X, n_samples * n_features, stream);

  auto centroids = std::move(Tensor<DataT, 2, IndexT>(
    _centroidsRawData.data(), {n_clusters, n_features}));
}

template <typename DataT, typename IndexT>
DataT *KMeans<DataT, IndexT>::centroids() {
...
void KMeans<DataT, IndexT>::initRandom(Tensor<DataT, 2, IndexT> &X) {
  ...
  kmeans::detail::shuffleAndGather(_handle, X, centroids, n_clusters,
                                   options.seed, stream);
}
...
template <typename DataT, typename IndexT>
void KMeans<DataT, IndexT>::initKMeansPlusPlus(Tensor<DataT, 2, IndexT> &X) {
  cudaStream_t stream = _handle.getStream();
  auto n_samples = X.getSize(0);
  auto n_features = X.getSize(1);

  Random::Rng rng(options.seed, options.gtype);
  ...
  host_buffer<int> h_isSampleCentroid(_handle.getHostAllocator(), stream,
                                      n_samples);
  std::fill(h_isSampleCentroid.begin(), h_isSampleCentroid.end(), 0);
  h_isSampleCentroid[cIdx] = 1;

  // flag the sample that is chosen as initial centroid
  Tensor<int, 1> isSampleCentroid({n_samples}, _handle.getDeviceAllocator(),
                                  stream);
  ...
  device_buffer<DataT> pairwiseDistanceRaw(_handle.getDeviceAllocator(),
                                           stream);
  pairwiseDistanceRaw.resize(dataBatchSize * n_clusters, stream);

  Tensor<DataT, 1, IndexT> minClusterDistance(
    {n_samples}, _handle.getDeviceAllocator(), stream);
  Tensor<DataT, 1, IndexT> uniformRands({n_samples},
                                        _handle.getDeviceAllocator(), stream);
  Tensor<DataT, 1, IndexT> clusterCostD({1}, _handle.getDeviceAllocator(),
                                        stream);
  ...
  kmeans::detail::minClusterDistance(_handle, X, potentialCentroids,
                                     pairwiseDistance, minClusterDistance,
                                     _workspace, _metric, stream);

  DataT phi = kmeans::detail::computeClusterCost(
    _handle, minClusterDistance, _workspace,
    [] __device__(const DataT &a, const DataT &b) { return a + b; }, stream);
  ...
  for (int iter = 0; iter < niter; ++iter) {
    ...
    DataT clusterCost = kmeans::detail::computeClusterCost(
      _handle, minClusterDistance, _workspace,
      [] __device__(const DataT &a, const DataT &b) { return a + b; }, stream);
    ...
    kmeans::detail::SamplingOp<DataT> select_op(
      clusterCost, options.oversampling_factor, uniformRands.data(),
      isSampleCentroid.data());

    auto Cp = kmeans::detail::sampleCentroids(_handle, X, minClusterDistance,
    ...
    MLCommon::copy(centroidsRawData.end() - Cp.numElements(), Cp.data(),
    ...
    potentialCentroids = std::move(Tensor<DataT, 2, IndexT>(
      centroidsRawData.data(), {tot_centroids, n_features}));
    /// <<<< End of Step-5 >>>
  }  /// <<<< Step-6 >>>
  ...
  if (potentialCentroids.getSize(0) > n_clusters) {
    ...
    kmeans::detail::countSamplesInCluster(_handle, X, potentialCentroids,
                                          _workspace, _metric, weights, stream);

    // <<< end of Step-7 >>>

    //Step-8: Recluster the weighted points in C into k clusters
    _centroidsRawData.resize(n_clusters * n_features, stream);
    kmeans::detail::kmeansPlusPlus(_handle, _verbose, n_clusters, options.seed,
                                   potentialCentroids, weights, _metric,
                                   _workspace, _centroidsRawData, stream);
  } else {
    ASSERT(potentialCentroids.getSize(0) < n_clusters,
           "failed to converge, restart with different seed");

    _centroidsRawData.resize(n_clusters * n_features, stream);
    MLCommon::copy(_centroidsRawData.data(), potentialCentroids.data(),
                   potentialCentroids.numElements(), stream);
  }
}

template <typename DataT, typename IndexT>
__host__ void KMeans<DataT, IndexT>::fit(Tensor<DataT, 2, IndexT> &X) {
  cudaStream_t stream = _handle.getStream();
  auto n_samples = X.getSize(0);
  auto n_features = X.getSize(1);
  ...
  // temporary buffer to store distance matrix, destructor releases the resource
  Tensor<DataT, 2, IndexT> pairwiseDistance(
    {dataBatchSize, n_clusters}, _handle.getDeviceAllocator(), stream);
  ...
  // temporary buffer to store the sample count per cluster, destructor releases the resource
  Tensor<int, 1, IndexT> sampleCountInCluster(
    {n_clusters}, _handle.getDeviceAllocator(), stream);
  ...
  for (n_iter = 0; n_iter < max_iter; ++n_iter) {
    ...
    kmeans::detail::minClusterAndDistance(
      _handle, X, centroids, pairwiseDistance, minClusterAndDistance,
      _workspace, _metric, stream);
    ...
    LinAlg::reduce_rows_by_key(X.data(), X.getSize(1), itr, _workspace.data(),
                               X.getSize(0), X.getSize(1), n_clusters,
                               newCentroids.data(), stream);

    // count # of samples in each cluster
    kmeans::detail::countLabels(_handle, itr, sampleCountInCluster.data(),
                                n_samples, n_clusters, _workspace, stream);
    ...
    _workspace.resize(sampleCountInCluster.numElements() * sizeof(DataT),
                      stream);
    auto sampleCountInClusterInverse = std::move(
      Tensor<DataT, 1, IndexT>((DataT *)_workspace.data(), {n_clusters}));
    ...
    MLCommon::copy(_centroidsRawData.data(), newCentroids.data(),
    ...
    if (options.inertia_check) {
      ...
      const cub::KeyValuePair<IndexT, DataT> clusteringCost =
        kmeans::detail::computeClusterCost(
          _handle, minClusterAndDistance, _workspace,
...
template <typename DataT, typename IndexT>
__host__ void KMeans<DataT, IndexT>::fit(const DataT *X, int n_samples,
                                         int n_features) {
  ...
  if (_init == kmeans::InitMethod::Random) {
    ...
    ASSERT(_centroidsRawData.data(),
           "call setCentroids() method to set the initial centroids or use the "
           "other initialized methods");
  } else {
...
template <typename DataT, typename IndexT>
__host__ void KMeans<DataT, IndexT>::predict(Tensor<DataT, 2, IndexT> &X) {
  cudaStream_t stream = _handle.getStream();
  auto n_samples = X.getSize(0);
  auto n_features = X.getSize(1);
  auto dataBatchSize = kmeans::detail::getDataBatchSize(n_samples, n_features);
  ...
  //   'key' is index to an sample in 'centroids' (index of the nearest centroid) and
  //   'value' is the distance between the sample 'X[i]' and the 'centroid[key]'
  kmeans::detail::minClusterAndDistance(_handle, X, centroids, pairwiseDistance,
                                        minClusterAndDistance, _workspace,
                                        _metric, stream);
  ...
  auto labels =
    std::move(Tensor<IndexT, 1>(_labelsRawData.data(), {n_samples}));
  ML::thrustAllocatorAdapter alloc(_handle.getDeviceAllocator(), stream);
  auto execution_policy = thrust::cuda::par(alloc).on(stream);
  thrust::transform(
...
template <typename DataT, typename IndexT>
__host__ void KMeans<DataT, IndexT>::predict(const DataT *X, int n_samples,
                                             int n_features,
                                             IndexT *labelsRawPtr) {
  cudaStream_t stream = _handle.getStream();

  ASSERT(_n_features == n_features,
         "model is trained for %d-dimensional data (provided data is "
         "%d-dimensional)",
         _n_features, n_features);

  ASSERT(n_clusters > 0, "no clusters exist");
  ...
  MLCommon::copy(labelsRawPtr, labels.data(), labels.numElements(), stream);
}

template <typename DataT, typename IndexT>
__host__ void KMeans<DataT, IndexT>::transform(const DataT *X, int n_samples,
                                               int n_features, DataT *X_new) {
  cudaStream_t stream = _handle.getStream();
  ...
  for (int dIdx = 0; dIdx < n_samples; dIdx += dataBatchSize) {
    ...
    // calculate pairwise distance between cluster centroids and current batch of input dataset
    kmeans::detail::pairwiseDistance(_handle, datasetView, centroids,
                                     pairwiseDistanceView, _workspace, _metric,
                                     stream);
  }
}
};  // end namespace ML
