...
void
Exact_TSNE(float *VAL, const int *COL, const int *ROW, const int NNZ,
...
{	
	...
	//---------------------------------------------------
	const float degrees_of_freedom = (float)MAX(dim-1, 1);
	...
	for (int iter = 0; iter < max_iter; iter++) {
		...
		TSNE::attractive_forces(VAL, COL, ROW, Y, norm, attract, NNZ, n, dim, df_power, recp_df, stream);
		// Compute repulsive forces
		const float Z = TSNE::repulsive_forces(Y, repel, norm, Z_sum, n, dim, df_power, recp_df, stream);

		// Apply / integrate forces
		const float gradient_norm = TSNE::apply_forces(Y, velocity, attract, repel, means, gains, Z, 
			learning_rate, C, momentum, dim, n, min_gain, gradient, check_convergence, stream);
		
		if (check_convergence) {
			if (verbose)
				printf("Z at iter = %d = %f and gradient norm = %f\n", iter, Z, gradient_norm);

			if (gradient_norm < min_grad_norm) {
				printf("Gradient norm = %f <= min_grad_norm = %f. Early stopped at iter = %d\n",
