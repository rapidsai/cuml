...
TSNE_fit(const cumlHandle &handle, const float *X, float *Y, const int n,
        ...
        const bool intialize_embeddings, bool barnes_hut)
{
    ASSERT(n > 0 && p > 0 && dim > 0 && n_neighbors > 0 && X != NULL && Y != NULL, "Wrong input args");
    if (dim > 2 and barnes_hut) {
        barnes_hut = false;
        printf("[Warn]  Barnes Hut only works for dim == 2. Switching to exact solution.\n");
    }
    ...
    if (verbose) {
        ...
        if (perplexity < 5 or perplexity > 50)
            printf("[Warn]  Perplexity should be within ranges (5, 50). Your results might be a bit strange...\n");
        if (n_neighbors < perplexity*3.0f)
            printf("[Warn]  # of Nearest Neighbors should be at least 3 * perplexity. Your results might be a bit strange...\n");
    }
    ...
    TSNE::symmetrize_perplexity(P, indices, n, n_neighbors, P_sum, early_exaggeration, &COO_Matrix, stream, handle);
    ...
    if (barnes_hut) {
        ...
        TSNE::Exact_TSNE(
            ...
            random_state, verbose, intialize_embeddings);
    }
