...
void _fit(const raft::handle_t &handle, tsne_input &input,
          knn_graph<value_idx, value_t> &k_graph, TSNEParams &params) {
  TSNE_runner<tsne_input, value_idx, value_t> runner(handle, input, k_graph,
                                                     params);
...
void TSNE_fit(const raft::handle_t &handle, float *X, float *Y, int n, int p,
  ...
  ASSERT(n > 0 && p > 0 && params.dim > 0 && params.n_neighbors > 0 &&
           X != NULL && Y != NULL,
         "Wrong input args");

  manifold_dense_inputs_t<float> input(X, Y, n, p);
  knn_graph<int64_t, float> k_graph(n, params.n_neighbors, knn_indices,
                                    knn_dists);
...
void TSNE_fit_sparse(const raft::handle_t &handle, int *indptr, int *indices,
  ...
  ASSERT(n > 0 && p > 0 && params.dim > 0 && params.n_neighbors > 0 &&
           indptr != NULL && indices != NULL && data != NULL && Y != NULL,
         "Wrong input args");
