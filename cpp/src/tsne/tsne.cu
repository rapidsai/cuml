              ...
              bool barnes_hut)
{
  ...
  if (pca_intialization == true)
  {
    ...
    if (verbose) printf("[Info] PCA largest value in intialization = %.3f\n", total_maximum);
    if (total_maximum == 0)
    {
  ...
  if (sizeof(long) == 2*sizeof(int))
    ...
    indices = (long *)d_alloc->allocate(sizeof(long) * 2 * n * n_neighbors, stream);
  ...
  // P holds both P and future P + P.T
  float *P = (float *)d_alloc->allocate(sizeof(float) * 2 * n * n_neighbors, stream);
  ...
  TSNE::symmetrize_perplexity(P, indices, n, n_neighbors, P_sum,
                              early_exaggeration, VAL, COL, ROW, stream, handle);
  //---------------------------------------------------
