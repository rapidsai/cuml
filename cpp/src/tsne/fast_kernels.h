...
float determine_sigmas(const float *__restrict__ distances,
                       ...
                       const int k, cudaStream_t stream, const int gridSize,
                       const int blockSize, const cumlHandle &handle) {
  const float desired_entropy = logf(perplexity);
  float *P_sum_ =
    (float *)handle.getDeviceAllocator()->allocate(sizeof(float), stream);
  float P_sum;
...
__global__ void __attractive_fast(const float *__restrict__ VAL,
  ...
  if (index < NNZ) {
    ...
    for (int k = 0; k < dim; k++) euclidean_d += (Y[k * n + i] * Y[k * n + j]);

    const float PQ =
      VAL[index] / (1.0f - 2.0f * euclidean_d + norm[i] + norm[j]);

    for (int k = 0; k < dim; k++)
      atomicAdd(&attract[k * n + i], PQ * (Y[k * n + i] - Y[k * n + j]));
  }
}
__global__ void __attractive_fast_2dim(
...
void attractive_fast(const float *__restrict__ VAL, const int *__restrict__ COL,
                     const int *__restrict__ ROW, const float *__restrict__ Y,
                     const float *__restrict__ norm,
                     float *__restrict__ attract, const int NNZ, const int n,
                     const int dim, cudaStream_t stream, const int gridSize,
                     const int blockSize) {
  cudaMemset(attract, 0, sizeof(float) * n * dim);

  if (dim == 2)
    __attractive_fast_2dim<<<gridSize, blockSize, 0, stream>>>(
      VAL, COL, ROW, Y, Y + n, norm, attract, attract + n, NNZ, n, dim);
  else
    __attractive_fast<<<gridSize, blockSize, 0, stream>>>(
      VAL, COL, ROW, Y, norm, attract, NNZ, n, dim);
  CUDA_CHECK(cudaPeekAtLastError());
}

__global__ void __repulsive_fast(const float *__restrict__ Y,
                                 float *__restrict__ repel,
                                 const float *__restrict__ norm,
                                 float *__restrict__ sum_Z, const int n,
                                 const int dim) {
  const int j =
...
__global__ void __repulsive_fast_2dim(const float *__restrict__ Y1,
  ...
  const int i = (blockIdx.y * blockDim.y) + threadIdx.y;  // for every row
  if (j > i && i < n && j < n) {
    const float Q = 1.0f / (1.0f - 2.0f * (Y1[i] * Y1[j] + Y2[i] * Y2[j]) +
                            norm[i] + norm[j]);
...
float repulsive_fast(const float *__restrict__ Y, float *__restrict__ repel,
                     const float *__restrict__ norm, float *__restrict__ sum_Z,
  ...
  if (dim == 2)
    ...
    __repulsive_fast<<<numBlocks, threadsPerBlock, 0, stream>>>(Y, repel, norm,
                                                                sum_Z, n, dim);
  CUDA_CHECK(cudaPeekAtLastError());

  thrust::device_ptr<float> begin = thrust::device_pointer_cast(sum_Z);
  double Z =
    (double)thrust::reduce(thrust::cuda::par.on(stream), begin, begin + n);
  return 1.0f / (2.0f * Z) + FLT_EPSILON;
}

__global__ void __subtract_mean_fast(float *__restrict__ Y,
...
__global__ void __apply_forces(const float *__restrict__ attract,
                               ...
                               const int dim, const float Z,
                               const float min_gain, const float momentum,
                               const float eta, float *__restrict__ means,
                               const float SIZE, const float gains_add = 0.3f,
                               const float gains_mult = 0.7f) {
  // Everything is F-Contiguous
  const int i =
    (blockIdx.x * blockDim.x) + threadIdx.x;  // for every item in column
  if (i < SIZE) {
    const float dy = attract[i] + Z * repel[i];

    if (signbit(dy) != signbit(iY[i]))
      gains[i] += gains_add;  // Notice original TSNE is 0.2
    else
      gains[i] *= gains_mult;  // Notice original TSNE is 0.8
...
template <int TPB_X = 32, int TPB_Y = 32>
void apply_forces(const float *__restrict__ attract, float *__restrict__ means,
                  ...
                  cudaStream_t stream, const int gridSize, const int blockSize,
                  const float gains_add = 0.3f, const float gains_mult = 0.7f) {
  cudaMemset(means, 0, sizeof(float) * dim);

  __apply_forces<<<gridSize, blockSize, 0, stream>>>(
    attract, repel, Y, iY, gains, n, dim, Z, min_gain, momentum, eta, means,
    n * dim, gains_add, gains_mult);
  CUDA_CHECK(cudaPeekAtLastError());

  // Divide by 1/n
  MLCommon::LinAlg::scalarMultiply(means, (const float *)means, 1.0f / n, dim,
                                   stream);

  // MLCommon::LinAlg::matrixVectorOp(
  //     Y, Y, means, n, dim, false, false,
  ...
  const dim3 numBlocks(ceildiv(n, (int)threadsPerBlock.x),
                       ceildiv(dim, (int)threadsPerBlock.y));
  __subtract_mean_fast<<<numBlocks, threadsPerBlock, 0, stream>>>(Y, means, n,
                                                                  dim);
  CUDA_CHECK(cudaPeekAtLastError());
}
