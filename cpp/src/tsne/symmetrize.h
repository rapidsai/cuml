...
__global__ static void symmetric_find_size(const float *__restrict__ data,
...
__global__ static void symmetric_sum(
  int *__restrict__ edges, const float *__restrict__ data,
  const long *__restrict__ indices, float *__restrict__ VAL,
  int *__restrict__ COL, int *__restrict__ ROW, const int n, const int k) {
  const int j =
    (blockIdx.x * blockDim.x) + threadIdx.x;  // for every item in row
  const int row = (blockIdx.y * blockDim.y) + threadIdx.y;  // for every row
  if (row >= n || j >= k) return;
...
struct COO_Matrix_t symmetrize_matrix(const float *__restrict__ data,
                                      const long *__restrict__ indices,
                                      const int n, const int k,
  ...
  CUDA_CHECK(cudaMemset(row_sizes, 0, sizeof(int) * n));
  CUDA_CHECK(cudaMemset(row_sizes2, 0, sizeof(int) * n));

  symmetric_find_size<<<numBlocks, threadsPerBlock, 0, stream>>>(
    data, indices, n, k, row_sizes, row_sizes2);
  CUDA_CHECK(cudaPeekAtLastError());

  reduce_find_size<<<ceil(n, 1024), 1024, 0, stream>>>(n, k, row_sizes,
                                                       row_sizes2);
  CUDA_CHECK(cudaPeekAtLastError());
  ...
  symmetric_sum<<<numBlocks, threadsPerBlock, 0, stream>>>(edges, data, indices,
                                                           VAL, COL, ROW, n, k);
  CUDA_CHECK(cudaPeekAtLastError());
  ...
  // (6) Return summed up VAL, COL, ROW
  struct COO_Matrix_t COO_Matrix = {
    .VAL = VAL, .COL = COL, .ROW = ROW, .NNZ = NNZ};
  return COO_Matrix;
}
