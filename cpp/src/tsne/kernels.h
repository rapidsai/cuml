...
float
determine_sigmas(const float *__restrict__ distances,
 ...
 */
__global__ void 
__get_norm(const float *__restrict__ Y,
                    ...
                    cudaStream_t stream)
{
    ...
    __attractive_fast<<<ceil(NNZ, 1024), 1024, 0, stream>>>(
        VAL, COL, ROW,
...
__global__ void
__postprocess_Q(float *__restrict__ Q,
                float *__restrict__ sum_Q, const int n) {
    const int j = (blockIdx.x * blockDim.x) + threadIdx.x;  // for every item in row
    const int i = (blockIdx.y * blockDim.y) + threadIdx.y;  // for every row
    if (i < n && j < n) {
        const float q = Q[i*n + j];
        atomicAdd(&sum_Q[i], q*q);
    }
}
