...
void
get_distances(const float *X, const int n, const int p,
	long *indices, float *distances, const int n_neighbors,
...
{	
  ...
  cumlHandle handle;
    ...
    LinAlg::scalarMultiply(distances, distances, div_maxNorm, n*n_neighbors, stream);
...
symmetrize_perplexity(float *P, long *indices, COO_t<float> *P_PT,
	const int n, const int k, const float P_sum, const float exaggeration, cuda_stream_t stream)
{
	// Convert to COO
	COO_t<float> P_COO;
	...
	Sparse::coo_symmetrize<32, float>(&P_COO, P_PT,
