...
float
normalize_distances(const int n, float *distances, const size_t SIZE, cudaStream_t stream)
{
    // Now D / max(abs(D)) to allow exp(D) to not explode
    assert(distances != NULL);
    thrust::device_ptr<float> begin = thrust::device_pointer_cast(distances);

    float maxNorm = MAX(*(thrust::max_element(thrust::cuda::par.on(stream), begin, begin + SIZE)),
    ...
    MLCommon::LinAlg::scalarMultiply(distances, (const float*) distances, div, SIZE, stream);
    return div;
}
...
void symmetrize_perplexity(float *P, long *indices, COO_t<float> *P_PT,
    ...
    MLCommon::LinAlg::scalarMultiply(P_COO.vals, (const float*) P_COO.vals, div, P_COO.nnz, stream);
    ...
    MLCommon::Sparse::coo_remove_zeros<TPB_X, float>(&P_PT_with_zeros, P_PT, stream);
