                    ...
                    cudaStream_t stream)
{
    ...
    float maxNorm = *thrust::max_element(thrust::cuda::par.on(stream), begin, begin + n*n_neighbors);
    if (maxNorm == 0.0f) maxNorm = 1.0f;

    // Divide distances inplace by max
    const float div = 1.0f / maxNorm;  // Mult faster than div
    MLCommon::LinAlg::scalarMultiply(distances, distances, div, n*n_neighbors, stream);
}
 ...
 */
template <int TPB_X = 32> void
symmetrize_perplexity(float *P, long *indices, const int n, const int k, 
                    ...
                    cudaStream_t stream, const cumlHandle &handle)
{
    ...
    MLCommon::Sparse::from_knn_symmetrize_matrix(indices, P, n, k, COO_Matrix, stream);
