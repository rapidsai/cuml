...
cuda_free_all(void) 
{
    ...
    for (size_t i = 0; i < garbage_no; i++) {
        if (GlobalGarbage[i] != NULL)
            cfree(GlobalGarbage[i]);
...
static void 
__cuda_memset(math_t *&ptr, const math_t val, const size_t n, cuda_stream_t stream)
{
    if (val == 0)
        CHECK(cudaMemset(ptr, 0, sizeof(math_t) * n));
    // else {
...
// ###################### Malloc some space ######################
static void * 
__cuda_malloc(const size_t n, bool garbage_collect = true) 
{
    void *data = NULL;
...
static math_t * 
__cuda_malloc_memset(const size_t n, cuda_stream_t stream, 
...
#define cuda_malloc(...) GET_MACRO(__VA_ARGS__, \
    __cuda_malloc3, __cuda_malloc2, __cuda_malloc1)(__VA_ARGS__)
