...
void
Barnes_Hut(float *VAL, const int *COL, const int *ROW, const int NNZ,
		const cumlHandle &handle, float *Y, const int n,
		const float theta = 0.5f, const float epssq = 0.0025,
		const float early_exaggeration = 12.0f,
		const int exaggeration_iter = 250, const float min_gain = 0.01f,
		const float pre_learning_rate = 200.0f, const float post_learning_rate = 500.0f,
...
{	
    ...
    TSNE::InitializationKernel<<<1, 1>>>(errl);
    CUDA_CHECK(cudaPeekAtLastError());

    const int FOUR_NNODES_ = 4 * nnodes;
    CUDA_CHECK(cudaMemcpyToSymbolAsync(FOUR_NNODES, &FOUR_NNODES_, sizeof(int), 0, cudaMemcpyHostToDevice, stream));
    const int FOUR_N_ = 4 * n;
    CUDA_CHECK(cudaMemcpyToSymbolAsync(FOUR_N, &FOUR_N_, sizeof(int), 0, cudaMemcpyHostToDevice, stream));
    const float theta_squared_ = theta * theta;
    CUDA_CHECK(cudaMemcpyToSymbolAsync(theta_squared, &theta_squared_, sizeof(float), 0, cudaMemcpyHostToDevice, stream));

    CUDA_CHECK(cudaMemcpyToSymbolAsync(NNODES, &nnodes, sizeof(int), 0, cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyToSymbolAsync(N, &n, sizeof(int), 0, cudaMemcpyHostToDevice, stream));


    // Actual mallocs
    int *startl = (int *)d_alloc->allocate(sizeof(int) * (nnodes + 1), stream);
    int *childl = (int *)d_alloc->allocate(sizeof(int) * (nnodes + 1) * 4, stream);
    float *massl = (float *)d_alloc->allocate(sizeof(float) * (nnodes + 1), stream);
	...
	float *maxxl = (float *)d_alloc->allocate(sizeof(float) * blocks * FACTOR1, stream);
	float *maxyl = (float *)d_alloc->allocate(sizeof(float) * blocks * FACTOR1, stream);
	float *minxl = (float *)d_alloc->allocate(sizeof(float) * blocks * FACTOR1, stream);
	float *minyl = (float *)d_alloc->allocate(sizeof(float) * blocks * FACTOR1, stream);
    ...
    // RepulsionKernel
    float *rep_forces = (float *)d_alloc->allocate(sizeof(float) * (nnodes + 1) * 2, stream);
    float *attr_forces = (float *)d_alloc->allocate(sizeof(float) * n * 4, stream); // n*2 double for reduction sum
    ...
    thrust::device_ptr<float> begin_gains_bh = thrust::device_pointer_cast(gains_bh);
	...
	float *YY = (float *)d_alloc->allocate(sizeof(float) * (nnodes + 1) * 2, stream);
	...
	// Set up CUB fast block summation
    //---------------------------------------------------
    void *temp_storage = NULL;
	size_t temp_size = 0;
	float *ZZ = (float *)d_alloc->allocate(sizeof(float), stream);

	CubDebugExit(cub::DeviceReduce::Sum(temp_storage, temp_size, norml, ZZ, nnodes + 1, stream));
	// Allocate temporary storage
	temp_storage = (void *)d_alloc->allocate(temp_size, stream);
    ...
    for (int iter = 0; iter < max_iter; iter++) {
    	...
    	CUDA_CHECK(cudaMemsetAsync(rep_forces, 0, sizeof(float)*(nnodes + 1)*2));
		CUDA_CHECK(cudaMemsetAsync(attr_forces, 0, sizeof(float)*n*4));
		...
		START_TIMER;
		TSNE::BoundingBoxKernel<<<blocks * FACTOR1, THREADS1>>>(startl, childl, massl, 
			                                                    YY, YY + nnodes + 1, 
			                                                    maxxl, maxyl, 
			                                                    minxl, minyl);
		CUDA_CHECK(cudaPeekAtLastError());
		...
		START_TIMER;
		TSNE::ClearKernel1<<<blocks, 1024>>>(childl);
		CUDA_CHECK(cudaPeekAtLastError());
		END_TIMER(ClearKernel1_time);


		START_TIMER;
		TSNE::TreeBuildingKernel<<<blocks * FACTOR2, THREADS2>>>(errl, childl, 
	                                                        	YY, YY + nnodes + 1);
		CUDA_CHECK(cudaPeekAtLastError());
		...
		START_TIMER;
		TSNE::ClearKernel2<<<blocks * 1, 1024>>>(startl, massl);
		CUDA_CHECK(cudaPeekAtLastError());
		END_TIMER(ClearKernel2_time);


		START_TIMER;
		TSNE::SummarizationKernel<<<blocks * FACTOR3, THREADS3>>>(countl, childl, massl,
	                                                        		YY, YY + nnodes + 1);
		CUDA_CHECK(cudaPeekAtLastError());
		...
		START_TIMER;
		TSNE::SortKernel<<<blocks * FACTOR4, THREADS4>>>(sortl, countl, startl, childl);
		CUDA_CHECK(cudaPeekAtLastError());
		END_TIMER(SortKernel_time);


		START_TIMER;
		TSNE::RepulsionKernel<<<blocks * FACTOR5, THREADS5>>>(errl, theta, epssq,
				                                                sortl, childl, massl, 
				                                               	YY, YY + nnodes + 1,
		...
		START_TIMER;
        CubDebugExit(cub::DeviceReduce::Sum(temp_storage, temp_size, norml, ZZ, nnodes + 1, stream));
        TSNE::Process_Z_norm<<<1, 1>>>(ZZ);
        END_TIMER(Reduction_time);


		START_TIMER;
		// Compute attractive forces
		TSNE::AttractiveKernel(VAL, COL, ROW, YY, nnodes, norm, norm_add1, attr_forces, NNZ, n, stream);
		END_TIMER(attractive_time);


		START_TIMER;
		TSNE::IntegrationKernel<<<blocks * FACTOR6, THREADS6>>>(learning_rate, momentum, early_exaggeration,
		                                                        YY, YY + nnodes + 1,
		                                                        attr_forces, attr_forces + n,
		                                                        rep_forces, rep_forces + nnodes + 1,
		                                                        gains_bh, gains_bh + n,
	...
	d_alloc->deallocate(attr_forces, sizeof(float) * n * 4, stream);
	...
	d_alloc->deallocate(ZZ, sizeof(float), stream);
	d_alloc->deallocate(temp_storage, temp_size, stream);
}
