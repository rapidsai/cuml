...
{   
    ...
    float *attr_forces = (float *)d_alloc->allocate(sizeof(float) * n * 2, stream); // n*2 double for reduction sum
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        START_TIMER;
        TSNE::SortKernel<<<blocks * FACTOR4, THREADS4, 0, stream>>>(sortl, countl, startl, childl);
        CUDA_CHECK(cudaPeekAtLastError());
        ...
        START_TIMER;
        TSNE::get_norm<<<ceil(n, 1024), 1024, 0, stream>>>(YY, YY + nnodes + 1, norm, norm_add1);
        CUDA_CHECK(cudaPeekAtLastError());
        ...
        TSNE::IntegrationKernel<<<blocks * FACTOR6, THREADS6, 0, stream>>>(
                                                                learning_rate, momentum, early_exaggeration,
                                                                YY, YY + nnodes + 1,
