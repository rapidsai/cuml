...
__constant__ int N;
__constant__ float theta_squared; // theta ** 2
...
__global__ void InitializationKernel(int * __restrict__ errd)
{
	*errd = 0;
	stepd = -1;
	maxdepthd = 1;
...
void BoundingBoxKernel(int * __restrict__ startd, 
						int * __restrict__ childd, 
						float * __restrict__ massd, 
						float * __restrict__ posxd, 
						float * __restrict__ posyd, 
						float * __restrict__ maxxd, 
						float * __restrict__ maxyd, 
						float * __restrict__ minxd, 
						float * __restrict__ minyd) 
{
	...
	if (i == 0) {
		...
		if (inc != atomicInc(&blkcntd, inc)) return;
...
void ClearKernel1(int * __restrict__ childd)
{
	const int inc = blockDim.x * gridDim.x;
	int k = (FOUR_N & -32) + threadIdx.x + blockIdx.x * blockDim.x;  // align to warp size
	if (k < FOUR_N) k += inc;
...
void TreeBuildingKernel(volatile int * __restrict__ errd, 
						int * __restrict__ childd, 
						const float * __restrict__ posxd, 
						const float * __restrict__ posyd) 
{
			...
			if (ch == -1) {
				if (-1 == atomicCAS(&childd[locked], -1, i)) {  // if null, just insert the new body
					...
					if (depth > localmaxdepth)
						localmaxdepth = depth; // localmaxdepth = max(depth, localmaxdepth);
				...
				if (ch == atomicCAS(&childd[locked], ch, -2)) {  // try to lock
					...
					do {
						...
						const int cell = atomicSub((int*) &bottomd, 1) - 1;
						if (cell <= N) {
							*errd = 1;
							bottomd = NNODES;
						}
					...
					} while (ch >= 0 and r > 1e-10); // add radius check because bodies that are very close together can cause this to fail... there is some error condition here that I'm not entirely sure of (not just when two bodies are equal)
					childd[n*4 + j] = i;
...
__global__
__launch_bounds__(1024, 1)
void ClearKernel2(int * __restrict__ startd, float * __restrict__ massd)
{
	const int bottom = bottomd;
	const int inc = blockDim.x * gridDim.x;
	int k = (bottom & -32) + threadIdx.x + blockIdx.x * blockDim.x;  // align to warp size
	if (k < bottom) k += inc;
...
void SummarizationKernel(int * __restrict__ countd, 
		...
		while (k <= NNODES) {
			if ( massd[k] < 0.0f ) {
				...
				for (int i = 0; i < 4; i++) {
					...
					if ((ch >= N) and ((mass[i*THREADS3+threadIdx.x] = massd[ch]) < 0.0f))
						goto CONTINUE_LOOP;
				}
				...
				for (int i = 0; i < 4; i++) {
					...
					if (ch >= 0) {
						const float m = (ch >= N) ? \
							(cnt += countd[ch], mass[i*THREADS3+threadIdx.x]) \
								: \
							(cnt++, massd[ch]);
		...
		if (massd[k] >= 0.0f) {
			...
			if (j == 0) {
				...
				for (int i = 0; i < 4; i++) {
					const int ch = childd[k*4+i];
					child[i*THREADS3+threadIdx.x] = ch;  // cache children
					if ((ch < N) or ((mass[i*THREADS3+threadIdx.x] = massd[ch]) >= 0.0f))
						j--;
				}
			}
			else {
...
void SortKernel(int * __restrict__ sortd, const int * __restrict__ countd, 
				volatile int * __restrict__ startd, int * __restrict__ childd)
{
	register int j, k, start;

	const int bottom = bottomd;
	const int dec = blockDim.x * gridDim.x;
	k = NNODES + 1 - dec + threadIdx.x + blockIdx.x * blockDim.x;
...
void RepulsionKernel(int * __restrict__ errd, 
					const float theta, 
					const float epssqd, // correction for zero distance
					const int * __restrict__ sortd, 
					const int * __restrict__ childd, 
					const float * __restrict__ massd, 
					const float * __restrict__ posxd, 
					const float * __restrict__ posyd, 
					float * __restrict__ velxd, 
				...
				while (pd < 4) {
					...
					if (n >= 0) {
						...
						tmp = dx*dx + dy*dy + epssqd; // distance squared plus small constant to prevent zeros
							...
							if ((n < N) or __all_sync(__activemask(), tmp >= dq[depth])) {  // check if all threads agree that cell is far enough away (or is a body)
						#else
							if ((n < N) or __all(tmp >= dq[depth])) {  // check if all threads agree that cell is far enough away (or is a body)
						#endif
...
__global__ void
get_norm(const float *__restrict__ Y1,
		const float *__restrict__ Y2,
		float *__restrict__ norm,
		float *__restrict__ norm_add1)
{
...
__global__ void
attractive_kernel_bh(const float *__restrict__ VAL,
					...
					float *__restrict__ attract1A,
					float *__restrict__ attract2A,
					float *__restrict__ attract1B,
					float *__restrict__ attract2B,
					const int NNZ)
{
	...
	const float PQ =  __fdividef(VAL[index] ,    norm_add1[i] + norm[j] - 2.0f*(Y1[i]*Y1[j] + Y2[i]*Y2[j]) ); // P*Q
...
__global__ void
reduce_attract(float *__restrict__ attract1A,
				float *__restrict__ attract2A,
...
/****************************************/
void
AttractiveKernel(const float *__restrict__ VAL,
				const int *__restrict__ COL,
				const int *__restrict__ ROW,
				...
				cudaStream_t stream)
{
	get_norm<<<ceil(n, 1024), 1024, 0, stream>>>(Y, Y + nnodes + 1, norm, norm_add1);
	CUDA_CHECK(cudaPeekAtLastError());

	// TODO: Calculate Kullback-Leibler divergence
	// For general embedding dimensions
	attractive_kernel_bh<<<ceil(NNZ, 1024), 1024, 0, stream>>>(
					VAL, COL, ROW, Y, Y + nnodes + 1, norm, norm_add1,
					attract, attract + n, attract + 2*n, attract + 3*n, NNZ);
	CUDA_CHECK(cudaPeekAtLastError());

	// Reduce attract1 = (attract1A + attract1B) and for attract2.
	reduce_attract<<<ceil(n, 1024), 1024, 0, stream>>>(attract, attract + n, attract + 2*n, attract + 3*n);
	CUDA_CHECK(cudaPeekAtLastError());
}
...
void IntegrationKernel(const float eta,
						const float momentum,
						const float exaggeration,
						float * __restrict__ Y1,
						float * __restrict__ Y2,
						const float *__restrict__ attract1,
						const float *__restrict__ attract2,
						const float *__restrict__ repel1,
						const float *__restrict__ repel2,
						float *__restrict__ gains1,
						float *__restrict__ gains2,
						float *__restrict__ old_forces1,
						float *__restrict__ old_forces2)
{
