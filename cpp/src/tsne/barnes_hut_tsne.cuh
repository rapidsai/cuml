                   ...
                   const TSNEParams& params)
{
  ...
  CUDA_CHECK(cudaFuncSetCacheConfig(BH::SortKernel<value_idx>, cudaFuncCachePreferL1));
  CUDA_CHECK(
  ...
  for (int iter = 0; iter < params.max_iter; iter++) {
    ...
    value_t grad_norm =
      compute_grad_norm(handle, old_forces.data(), attr_forces.data(), attr_forces.size());
