...
void preProcessData_impl(cumlHandle &handle,
		...
		std::vector<Matrix::Data<T>*> &labels, T *mu_input, T *mu_labels,
	...
	if (fit_intercept) {
		...
		Stats::opg::mean(mu_labels_data, labels, labels_desc, comm, allocator,
				streams, n_streams, cublas_handle);
		Stats::opg::mean_center(labels, labels_desc, mu_labels_data, comm,
				streams, n_streams);

		if (normalize) {
			Matrix::Data<T> norm2_input_data { norm2_input, size_t(input_desc.N) };
			LinAlg::opg::colNorm2(norm2_input_data, input_data, input_desc,
					comm, allocator, streams, n_streams, cublas_handle);

			Matrix::opg::matrixVectorBinaryDivSkipZero(input_data, input_desc,
					norm2_input_data, false, true, true, comm, streams, n_streams);
		}
...
void postProcessData_impl(cumlHandle &handle,
		...
		T *mu_input, T *mu_labels, T *norm2_input, bool fit_intercept,
		bool normalize, cudaStream_t *streams, int n_streams, bool verbose) {
	...
	if (normalize) {
		...
		Matrix::opg::matrixVectorBinaryMult(input_data, input_desc,
				norm2_input_data, false, true, comm, streams, n_streams);
		Matrix::matrixVectorBinaryDivSkipZero(coef, norm2_input, size_t(1),
				input_desc.N, false, true, streams[0], true);
	}
...
void preProcessData(cumlHandle &handle,
		...
		Matrix::PartDescriptor &input_desc,
		std::vector<Matrix::Data<double>*> &labels, double *mu_input,
...
void postProcessData(cumlHandle &handle,
		...
		float *intercept, float *mu_input, float *mu_labels, float *norm2_input,
		bool fit_intercept, bool normalize, cudaStream_t *streams,
		int n_streams, bool verbose) {

	postProcessData_impl(handle, input_data, input_desc, labels, coef,
