...
template <typename T>
inline void linearFwd(const handle_impl &handle, SimpleMat<T> &Z,
...
template <typename T>
inline void linearBwd(const handle_impl &handle, SimpleMat<T> &G,
...
struct GLMBase : GLMDims {
  ...
  GLMBase(const handle_impl &handle, int D, int C, bool fit_intercept)
