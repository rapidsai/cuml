...
__global__ void
logSoftmaxKernel(T *__restrict out,
                 T *__restrict dZ,
                 const T *__restrict in,
                 const T *__restrict labels,
                 const int C,
                 const int N,
                 const bool getDerivative = true)
{
  ...
  const T tmpMax = WarpRed(shm.warpStore[threadIdx.y]).Reduce(etaMax, cub::Max());
  if (threadIdx.x == 0) {
  ...
  if (C <= BX)
  {  // this means one block covers a column and myEta is valid
