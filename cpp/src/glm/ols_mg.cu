...
void fit_impl(cumlHandle &handle, std::vector<Matrix::Data<T>*> &input_data,
		...
		bool fit_intercept, bool normalize, int algo, cudaStream_t *streams,
	...
	if (algo == 0 || input_desc.N == 1) {
		...
		LinAlg::opg::lstsqEig(input_data, input_desc, labels, coef, comm,
				allocator, streams, n_streams, cublas_handle, cusolver_handle);
	} else {
		ASSERT(false, "olsFit: no algorithm with this id has been implemented");
	}
