...
void preprocess_quantile(const T *data, const unsigned int *rowids,
                         ...
                         std::shared_ptr<TemporaryMemory<T>> tempmem) {
  int threads = 128;
  int num_items =
    n_sampled_rows *
    ncols;  // number of items to sort across all segments (i.e., cols)
  ...
  CUDA_CHECK(cub::DeviceSegmentedRadixSort::SortKeys(
    ...
    num_items, num_segments, d_offsets->data(), d_offsets->data() + 1, 0,
  ...
  CUDA_CHECK(cub::DeviceSegmentedRadixSort::SortKeys(
    (void *)d_temp_storage->data(), temp_storage_bytes, d_keys_in,
    d_keys_out->data(), num_items, num_segments, d_offsets->data(),
    d_offsets->data() + 1, 0, 8 * sizeof(T), tempmem->stream));

  blocks = MLCommon::ceildiv(ncols * nbins, threads);
  get_all_quantiles<<<blocks, threads, 0, tempmem->stream>>>(
    d_keys_out->data(), tempmem->d_quantile->data(), n_sampled_rows, ncols,
    nbins);
  CUDA_CHECK(cudaGetLastError());
