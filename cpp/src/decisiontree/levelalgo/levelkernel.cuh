...
__global__ void setup_counts_kernel(int* sample_cnt,
  ...
  for (tid = threadid; tid < n_sampled_rows; tid += blockDim.x * gridDim.x) {
    unsigned int stid = rowids[tid];
    atomicAdd(&sample_cnt[stid], 1);
  }
}
__global__ void setup_flags_kernel(unsigned int* flags,
                                   const int* __restict__ sample_cnt,
                                   const int nrows) {
  int threadid = threadIdx.x + blockIdx.x * blockDim.x;
  if (tid = threadid; tid < nrows; tid += blockDim.x * gridDim.x) {
