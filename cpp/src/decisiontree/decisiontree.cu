...
void DecisionTreeParams::validity_check() const {
  ...
  ASSERT((min_rows_per_node > 0), "Invalid min # rows per node %d",
         min_rows_per_node);
}
...
void DecisionTreeClassifier<T>::fit(const ML::cumlHandle& handle, T* data,
  ...
  return plant(handle.getImpl(), data, ncols, nrows, labels, rowids,
               n_sampled_rows, unique_labels, tree_params.max_depth,
               tree_params.max_leaves, tree_params.max_features,
               tree_params.n_bins, tree_params.split_algo,
 ...
 */
template <typename T>
void DecisionTreeClassifier<T>::predict(const ML::cumlHandle& handle,
...
template <typename T>
void DecisionTreeClassifier<T>::plant(
  ...
  int unique_labels, int maxdepth, int max_leaf_nodes, const float colper,
  int n_bins, int split_algo_flag, int cfg_min_rows_per_node,
  bool cfg_bootstrap_features) {
  ...
  for (int i = 0; i < MAXSTREAMS; i++) {
    tempmem[i] = std::make_shared<TemporaryMemory<T>>(
...
template <typename T>
TreeNode<T>* DecisionTreeClassifier<T>::grow_tree(T* data, const float colper,
  ...
  if (treedepth != -1) condition = (condition || (depth == treedepth));
  ...
  if (condition) {
    ...
    leaf_counter++;
    if (depth > depth_counter) depth_counter = depth;
  } else {
...
void DecisionTreeClassifier<T>::find_best_fruit_all(
  T* data, int* labels, const float colper, GiniQuestion<T>& ques, float& gain,
  unsigned int* rowids, const int n_sampled_rows, GiniInfo split_info[3],
  int depth) {
  ...
  if (depth == 0) {
    CUDA_CHECK(cudaHostRegister(colselector.data(),
                                sizeof(int) * colselector.size(),
                                cudaHostRegisterDefault));
    // Copy sampled column IDs to device memory
    CUDA_CHECK(cudaMemcpyAsync(tempmem[0]->d_colids->data(), colselector.data(),
                               sizeof(int) * colselector.size(),
                               cudaMemcpyHostToDevice, tempmem[0]->stream));
  ...
  best_split_all_cols(data, rowids, labels, current_nbins, n_sampled_rows,
                      n_unique_labels, dinfo.NLocalrows, colselector,
                      tempmem[0], &split_info[0], ques, gain, split_algo);
}

template <typename T>
void DecisionTreeClassifier<T>::split_branch(T* data, GiniQuestion<T>& ques,
                                             const int n_sampled_rows,
...
template <typename T>
void DecisionTreeClassifier<T>::classify_all(const T* rows, const int n_rows,
                                             const int n_cols, int* preds,
                                             bool verbose) const {
...
template <typename T>
int DecisionTreeClassifier<T>::classify(const T* row,
                                        const TreeNode<T>* const node,
                                        bool verbose) const {
...
template <typename T>
void DecisionTreeClassifier<T>::print_node(const std::string& prefix,
                                           const TreeNode<T>* const node,
                                           bool isLeft) const {
  if (node != nullptr) {
    std::cout << prefix;
