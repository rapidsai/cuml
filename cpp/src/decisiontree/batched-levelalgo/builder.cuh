...
struct Builder {
 ...
 private:
  ...
  void computeSplit(IdxT col, size_t n_blocks_dimx, size_t n_large_nodes)
  {
    ...
    // For RandomObjectiveFunction, pass seed and treeid for proper randomization
    ObjectiveT objective = [&]() {
      if constexpr (std::is_same_v<ObjectiveT,
                                   RandomObjectiveFunction<DataT, LabelT, IdxT>>) {
        return ObjectiveT(dataset.num_outputs, params.min_samples_leaf, seed, treeid);
      } else {
        return ObjectiveT(dataset.num_outputs, params.min_samples_leaf);
      }
    }();
