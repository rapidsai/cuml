...
__global__ void computeSplitClassificationKernel(
  ...
  // if (node.info.unique_id == 0 && threadIdx.x == 0 && col == 1) {
  //   printf("[bsl] pdf_shist: ");
  //   for(int i = 0; i < pdf_shist_len; i++) {
  //     printf("%d, ", pdf_shist[i]);
  //   }
  //   printf("\n");
  // }
...
template <typename DataT, typename LabelT, typename IdxT, int TPB>
__global__ void computeSplitClassificationKernel_2col(
  int* hist, IdxT n_bins, IdxT min_samples_leaf, DataT min_impurity_decrease,
  Input<DataT, LabelT, IdxT> input, const Node<DataT, LabelT, IdxT>* nodes,
  IdxT colStart, int* done_count, int* mutex,
  volatile Split<DataT, IdxT>* splits, CRITERION splitType, IdxT treeid,
  ...
  auto cdf_shist_len = n_bins * 2 * nclasses;
  auto* pdf_shist = alignPointer<int>(smem);
  auto* cdf_shist = alignPointer<int>(pdf_shist + 2*pdf_shist_len);
  auto* sbins1 = alignPointer<DataT>(cdf_shist + 2*cdf_shist_len);
  auto* sbins2 = alignPointer<DataT>(sbins1 + n_bins);
  auto* sDone = alignPointer<int>(sbins2 + n_bins);
  IdxT stride = blockDim.x * num_blocks;
  IdxT tid = threadIdx.x + offset_blockid * blockDim.x;
  ...
  IdxT col1, col2;
  if (input.nSampledCols == input.N) {
    col1 = colStart + 2*blockIdx.y;
    col2 = colStart + 2*blockIdx.y + 1;
  } else {
    int colIndex = colStart + blockIdx.y;
    col1 = select(colIndex, treeid, node.info.unique_id, seed, input.N);
    col2 = select(colIndex + 1, treeid, node.info.unique_id, seed, input.N);
  }

  // populating shared memory with initial values
  for (IdxT i = threadIdx.x; i < 2*pdf_shist_len; i += blockDim.x)
    pdf_shist[i] = 0;
  for (IdxT j = threadIdx.x; j < 2*cdf_shist_len; j += blockDim.x)
    cdf_shist[j] = 0;
  for (IdxT b = threadIdx.x; b < n_bins; b += blockDim.x) {
    sbins1[b] = input.quantiles[col1 * n_bins + b];
  }

  for (IdxT b = threadIdx.x; b < n_bins; b += blockDim.x) {
    sbins2[b] = input.quantiles[col2 * n_bins + b];
  }
  ...
  // compute pdf shared histogram for all bins for all classes in shared mem
  auto coloffset1 = col1 * input.M;
  auto coloffset2 = col2 * input.M;
  for (auto i = range_start + tid; i < end; i += stride) {
    // each thread works over a data point and strides to the next
    auto row = input.rowids[i];
    auto d1 = input.data[row + coloffset1];
    auto d2 = input.data[row + coloffset2];
    auto label = input.labels[row];
    for (IdxT b = 0; b < n_bins; ++b) {
      if (d1 <= sbins1[b]) {
        auto offset = label * (1 + n_bins) + b;
    ...
    for (IdxT b = 0; b < n_bins; ++b) {
      if (d2 <= sbins2[b]) {
        auto offset = label * (1 + n_bins) + b;
        atomicAdd(pdf_shist + offset + pdf_shist_len, 1);
        break;
      }
    }
  }
  ...
  if (num_blocks > 1) {
    // update the corresponding global location
    auto histOffset = ((nid * gridDim.y) + blockIdx.y) * pdf_shist_len * 2;
    for (IdxT i = threadIdx.x; i < 2*pdf_shist_len; i += blockDim.x) {
      atomicAdd(hist + histOffset + i, pdf_shist[i]);
    }
  ...
  // if (node.info.unique_id == 0 && threadIdx.x == 0 && col1 == 0) {
  //   printf("[dev] pdf_shist: ");
  //   for(int i = 0; i < 2*pdf_shist_len; i++) {
  //     printf("%d, ", pdf_shist[i]);
  //   }
  //   printf("\n");
  // }
  ...
  for (IdxT c = 0; c < nclasses; ++c) {
    ...
    auto offset_cdf = (2 * n_bins) * c;
    // converting pdf to cdf
    int total_sum = pdf_to_cdf<int, IdxT, TPB>(pdf_shist + offset_pdf + pdf_shist_len,
                                               cdf_shist + offset_cdf + cdf_shist_len, n_bins);
     ...
     **/
    for (IdxT i = threadIdx.x; i < n_bins; i += blockDim.x) {
      *(cdf_shist + offset_cdf + i + cdf_shist_len) =
        total_sum - *(cdf_shist + 2 * n_bins * c + i + cdf_shist_len);
    }
  }
  ...
  // calculate the best candidate bins (one for each block-thread) in current feature and corresponding information gain for splitting
  if (splitType == CRITERION::GINI) {
    giniGain<DataT, IdxT>(cdf_shist, sbins1, sp, col1, range_len, n_bins,
                          nclasses, min_samples_leaf, min_impurity_decrease);
    __syncthreads();
    giniGain<DataT, IdxT>(cdf_shist+cdf_shist_len, sbins2, sp, col2, range_len, n_bins,
                          nclasses, min_samples_leaf, min_impurity_decrease);
  } else {
    entropyGain<DataT, IdxT>(cdf_shist, sbins1, sp, col1, range_len, n_bins,
                             nclasses, min_samples_leaf, min_impurity_decrease);
    entropyGain<DataT, IdxT>(cdf_shist+cdf_shist_len, sbins2, sp, col2, range_len, n_bins,
                             nclasses, min_samples_leaf, min_impurity_decrease);
  }
  __syncthreads();
...
template <typename DataT, typename LabelT, typename IdxT, int TPB>
__global__ void computeSplitClassificationKernel_static_smem(
  int* hist, IdxT _n_bins, IdxT min_samples_leaf, DataT min_impurity_decrease,
  Input<DataT, LabelT, IdxT> input, const Node<DataT, LabelT, IdxT>* nodes,
  IdxT colStart, int* done_count, int* mutex,
  volatile Split<DataT, IdxT>* splits, CRITERION splitType, IdxT treeid,
  WorkloadInfo<IdxT>* workload_info, uint64_t seed) {
  ...
  auto end = range_start + range_len;
  constexpr IdxT nclasses = 4;
  constexpr IdxT n_bins = 128;
  constexpr IdxT pdf_shist_len = (n_bins + 1) * nclasses;
  constexpr IdxT cdf_shist_len = n_bins * 2 * nclasses;
  __shared__ int pdf_shist[pdf_shist_len];
  __shared__ int cdf_shist[cdf_shist_len];
  __shared__ DataT sbins[n_bins];
