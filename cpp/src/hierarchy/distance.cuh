...
void pairwise_distances(const raft::handle_t &handle,
                        ...
                        value_idx *indptr, value_idx *indices,
                        value_t *data) {
  ...
  MLCommon::Distance::pairwise_distance<value_t, size_t>(X, X, data,
                                                         m, m, n, workspace,
                                                         metric, stream);
}
...
void knn_graph(const raft::handle_t &handle,
               const value_t *X,
               size_t m, size_t n,
               raft::distance::DistanceType metric,
  ...
  conv_indices(int64_indices.data(), indices.data(), int64_indices.size(), stream);
  ...
  MLCommon::Sparse::coo_remove_zeros<1024, value_t>(&out, &comp_coo,
                                                    d_alloc, stream);
}
