...
struct forest {
  ...
  void predict(const raft::handle_t& h, float* preds, const float* data,
    ...
    */
    ...
    if (leaf_algo_ == leaf_algo_t::FLOAT_SCALAR) {
      if (predict_proba) {
  ...
  leaf_algo_t leaf_algo_ = leaf_algo_t::FLOAT_SCALAR;
  int num_classes_ = 1;
};
...
void check_params(const forest_params_t* params, bool dense) {
  ...
  switch (params->leaf_algo) {
    ...
    case leaf_algo_t::CATEGORICAL_LEAF:
      ...
      break;
    default:
      ASSERT(false, "leaf_algo should be FLOAT_SCALAR or CATEGORICAL_LEAF");
  }
...
void tl2fil_common(forest_params_t* params, const tl::Model& model,
  ...
  if (leaf_vec_size > 0) {
    ...
    params->leaf_algo = leaf_algo_t::FLOAT_SCALAR;
  }
