...
void segmented_reduce(const value_t *in, value_t *out, const value_idx *offsets,
                      cudaStream_t stream) {
  void *d_temp_storage = NULL;
  size_t temp_storage_bytes = 0;
  cub_reduce_func(d_temp_storage, temp_storage_bytes, in, out, n_clusters,
...
void compute_stabilities(
  ...
  rmm::device_uvector<value_idx> sorted_child(condensed_child, n_points,
                                              stream);
  ...
  segmented_reduce<value_idx, value_t, cub::DeviceSegmentedReduce::Min>(
    lambdas, births.data(), sorted_child_offsets.data(), stream);
  ...
  segmented_reduce<value_idx, value_t, cub::DeviceSegmentedReduce::Sum>(
    lambdas, stabilities.data(), sorted_child_offsets.data(), stream);

  // now transform, and calculate summation lambda(point) - lambda(birth)
  auto transform_op =
    transform_functor<value_t>(stabilities.data(), birth.data());
  thrust::transform(policy, thrust::make_counting_iterator(0),
