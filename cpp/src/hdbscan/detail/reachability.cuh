...
void runL2SelectMin(
  faiss::gpu::Tensor<value_t, 2, true>& productDistances,
...
void mutual_reachability_knn_l2(const raft::handle_t &handle,
  ...
  auto core_dists_tensor = faiss::gpu::toDeviceTemporary<value_t, 1>(
    gpu_res, device, const_cast<value_t*>(reinterpret_cast<const value_t*>(core_dists)),
    stream, {(int)m});

  auto x_tensor = faiss::gpu::toDeviceTemporary<value_t, 2>(
    gpu_res, device, const_cast<value_t*>(reinterpret_cast<const value_t*>(X)),
    stream, {(int)m, (int)n});


  auto out_dists_tensor = faiss::gpu::toDeviceTemporary<value_t, 2>(
    gpu_res, device, const_cast<value_t*>(reinterpret_cast<const value_t*>(out_dists)),
    stream, {(int)m, k});

  auto out_inds_tensor = faiss::gpu::toDeviceTemporary<value_idx, 2>(
    gpu_res, device, const_cast<value_idx*>(reinterpret_cast<const value_idx*>(out_inds)),
    stream, {(int)m, k});

  auto norms_tensor = faiss::gpu::toDeviceTemporary<value_t, 1>(
    gpu_res, device, const_cast<value_t*>(reinterpret_cast<const value_t*>(norms.data())),
    stream, {(int)m});
  ...
  int tileRows = 0;
  int tileCols = 0;
  faiss::gpu::chooseTileSize(m, m, n, sizeof(value_t),
                             tmp_mem_cur_device,
                             tileRows, tileCols);
  ...
  faiss::gpu::DeviceTensor<value_t, 2, true>* distanceBufs[2] = {
  ...
  for (int i = 0; i < m; i += tileRows) {
    ...
    auto queryView =
      x_tensor.narrow(0, i, curQuerySize);
    ...
    for (int j = 0; j < m; j += tileCols) {
      ...
      auto centroidsView = sliceCentroids(
        x_tensor, true, j, curCentroidSize);
