             ...
             const math_t* sample_weight)
{
  ...
  cudaStream_t stream = handle_impl.get_stream();
  {
    ...
    model.unique_labels                      = (math_t*)rmm_alloc.allocate(
      stream, model.n_classes * sizeof(math_t), rmm::CUDA_ALLOCATION_ALIGNMENT);
...
void svmFreeBuffers(const raft::handle_t& handle, SvmModel<math_t>& m)
{
  ...
  if (m.dual_coefs)
    rmm_alloc.deallocate(
      stream, m.dual_coefs, m.n_support * sizeof(math_t), rmm::CUDA_ALLOCATION_ALIGNMENT);
  if (m.support_idx)
    rmm_alloc.deallocate(
      stream, m.support_idx, m.n_support * sizeof(int), rmm::CUDA_ALLOCATION_ALIGNMENT);
  if (m.support_matrix.indptr) {
    rmm_alloc.deallocate(stream,
                         m.support_matrix.indptr,
  ...
  if (m.unique_labels)
    rmm_alloc.deallocate(
      stream, m.unique_labels, m.n_classes * sizeof(math_t), rmm::CUDA_ALLOCATION_ALIGNMENT);
