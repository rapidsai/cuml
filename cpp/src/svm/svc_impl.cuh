...
void svcPredict(const cumlHandle &handle, math_t *input, int n_rows, int n_cols,
  ...
  int n_ws = model.n_support;
  KernelCache<math_t> cache(handle_impl, input, n_rows, n_cols, n_ws, kernel,
                            buffer_size, -1, C_SVC, false);
  // it does not matter that we have SVC here, since no special need to handle
  // duplicate training vectors during prediction.
  math_t beta = 0;
  cache.KernelMV(model.dual_coefs, model.n_support, model.x_support,
                 model.n_support, model.n_support, beta, y.data());
