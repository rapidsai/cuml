 ...
 *
 * Based on Platt's SMO [1], using improvements from Keerthy and Shevade [2]. A
 * concise summary of the math can be found in Appendix A1 of [3]. We solve the
 * QP subproblem for the vectors in the working set (WS).
 *
 ...
 * @f] subject to
 * @f[ \mathbf{\alpha}^T \mathbf{y} = 0 @\ \mathbf{0} \le \mathbf{\alpha}\le C
 * \mathbf{1},\f] where
 * @f$ Q_{i,j} = y_i y_j K(\mathbf{x}_i, \mathbf{x}_j)\f$
 *
 * This corresponds to Lagrangian for the dual is:
 * @f[ L = \frac{1}{2}  \sum_{i,j}\alpha_i Q \alpha_j - \sum_i \alpha_i -\sum_i
 * \delta_i \alpha_i + \sum_i\mu_i(\alpha_i -C)
 ...
 * y_j \alpha_j K(\mathbf{x}_i, \mathbf{x}_j) = -y_i + y_i Q_{i,j} \alpha_j.
 * @f]
 *
 * The Karush-Kuhn-Tucker conditions are necessary and sufficient for
 * optimality. According to [2], the conditions simplify to
 * @f[ \beta \le f_i, \forall i \in I_\mathrm{upper}, \quad \beta \ge f_i
 * \forall i \in I_\mathrm{lower}.
 * @f]
 *
 * If
 * @f$ \max\{f_i | i \in I_\mathrm{lower}\} \le \min\{f_i| i\in
 * I_\mathrm{upper}\}\f$, then we are converged because any beta value in this
 * interval would lead to an optimal solution. Otherwise we modify the alpha
 * parameters until the corresponding changes in f lead to an on optimal
 * solution.
 *
 * Before the first iteration, one should set
 * @f$ \alpha_i = 0\f$, and
 * @f$ f_i = -y_i
 * @f$, for each
 * @f$ i \in [0..n_{rows}]\f$.
 ...
 * vectors u and l from the WS and update the dual coefficients of these
 * vectors. We iterate several times, and accumulate the change in the dual
 * coeffs in
 * @f$\Delta\alpha\f$.
 *
 * In every iteration we select the two vectors using the following formulas
 * @f[ u = \mathrm{argmin}_{i=1}^{n_{ws}}\left[ f_i | x_i \in X_\mathrm{upper}
 * \right]
 * @f]
 *
 * @f[ l = \mathrm{argmax}_{i=1}^{n_{ws}} \left[ \frac{(f_u-f_i)^2}{\eta_i}| f_u
 * < f_i \land x_i \in X_{\mathrm{lower}}\right],
 * @f] where
 * @f[\eta_i = K(x_u, x_u) + K(x_i, x_i) - 2K(x_u, x_i).
 * @f]
 *
 ...
 * @f[ \Delta \alpha_l = - y_l \frac{f_l - f_u}{\eta_l} = -y_l q,
 * @f]
 * @f[ \alpha_l += \Delta \alpha_l,
 * @f]
 * @f[ \Delta \alpha_u = -y_u y_l \Delta \alpha_l = y_u q,
 * @f]
 * @f[ \alpha_u += \Delta \alpha_u.
 * @f]
 *
 * We also update the optimality indicator vector for the WS:
 * @f[ f_i += \Delta\alpha_u y_u K(x_u,x_i) + \Delta\alpha_l y_l K(x_l, x_i)
 * @f]
 *
 * During the inner iterations, the f values are updated only for the WS (since
 * we are solving the optimization subproblem for the WS subproblem). For
 * consistency, f is kept as an input parameter, the changed values are not
 * saved back to global memory. After this solver finishes,  all the f values
 * (WS and outside WS) f should be updated using the delta_alpha output
 * parameter.
 *
 ...
 * @param[in]    kernel      kernel function calculated between the working set
 *                           and all other training vectors, size [n_rows *
 *                           n_ws]
 * @param[in]    ws_idx      indices of traning vectors in the working set, size
 *                           [n_ws]
 * @param[in]    C_vec       penalty parameter vector including class and sample
 *                           weights size [n_train]
 * @param[in]    eps         tolerance, iterations will stop if the duality gap
 *                           is smaller than this value (or if the gap is
 *                           smaller than 0.1 times the initial gap)
