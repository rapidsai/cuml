...
class WorkingSet {
 public:
   ...
   *
   *        Here we follow the working set selection strategy by Joachims [1],
   *        we select n training instances as:
   *   - select n/2 element of upper set, where f is largest
   *   - select n/2 from lower set, where f is smallest
   *
   *        The difference compared to Joachims' strategy is that we can already
   *        have some elements selected by a different strategy, therefore we
   *        select only n = n_ws - n_already_selected.
   *
   *        References: [1] Joachims, T. (1998). Making large-scale support
   *        vector machine learning practical. In B. Scholkopf, C. Burges, & A.
   *        Smola (Eds.), Advances in kernel methods: Support vector machines.
   ...
   *
   *        To avoid training vectors oscillating in and out of the working set,
   *        we keep half of the previous working set, and fill new elements only
   *        to the other half.
   *
   *        We can have a FIFO retention policy, or we can consider the time
   *        (=ws_priority) a vector already spent in the ws. References: [1] Z.
   *        Wen et al. ThunderSVM: A Fast SVM Library on GPUs and CPUs, Journal
   ...
   *        priority.
   *
   *        We sort the old working set based on their priority in ascending
   *        order, and then select nc elements from free, and then lower/upper
   *        bound vectors. For details see [2].
   *
   *        See Issue #946.
   *
   *        References: [2] T Serafini, L Zanni: On the Working Set selection in
   *        grad. projection based decomposition techniques for Support Vector
   *        Machines DOI: 10.1080/10556780500140714
   *
 ...
 private:
   ...
   *        f_idx_sorted. We ignore the elements that are already selected, and
   *        those where this->available is false.
   *
   ...
   * @param copy_front         if true, then copy the elements from the front of
   *                           the selected list, otherwise copy from the end of
   *                           the list
   *
  ...
  /**
   * @brief Select the first n_needed elements from ws_idx_sorted where op is
   *        true.
   *
