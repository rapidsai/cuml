 ...
 *
 * The working set is a subset of the training vectors, by default it has 1024
 * elements. At every outer iteration in SmoSolver::Solve, we select a different
 * working set, and optimize the dual coefficients for the working set.
 *
 * The vectors are selected based on the f values, which is the difference
 * between the target label and the decision function value.
 *
...
class WorkingSet {
 public:
  ...
  /**
   * @brief      Set the size of the working set and allocate buffers
   *             accordingly.
   *
   ...
   *
   *             Here we follow the working set selection strategy by Joachims
   *             [1], we select n training instances as:
   *   - select n/2 element of upper set, where f is largest
   *   - select n/2 from lower set, where f is smallest
   *
   *             The difference compared to Joachims' strategy is that we can
   *             already have some elements selected by a different strategy,
   *             therefore we select only n = n_ws - n_already_selected.
   *
   *             References: [1] Joachims, T. (1998). Making large-scale support
   *             vector machine learning practical. In B. Scholkopf, C. Burges,
   ...
   * @param      n_already_selected  The n already selected
   */
   ...
   *
   *             To avoid training vectors oscillating in and out of the working
   *             set, we keep half of the previous working set, and fill new
   *             elements only to the other half.
   *
   *             We can have a FIFO retention policy, or we can consider the
   *             time (=ws_priority) a vector already spent in the ws.
   *             References: [1] Z. Wen et al. ThunderSVM: A Fast SVM Library on
   *             GPUs and CPUs, Journal of Machine Learning Research, 19, 1-5
   *             (2018)
   *
   * @param      f      optimality indicator vector, size [n_train]
   * @param      alpha  dual coefficients, size [n_train]
  ...
  /**
   * @brief      Select elements from the previous working set based on their
   *             priority.
   *
   *             We sort the old working set based on their priority in
   *             ascending order, and then select nc elements from free, and
   *             then lower/upper bound vectors. For details see [2].
   *
   *             See Issue #946.
   *
   *             References: [2] T Serafini, L Zanni: On the Working Set
   *             selection in grad. projection based decomposition techniques
   *             for Support Vector Machines DOI: 10.1080/10556780500140714
   *
 ...
 private:
   ...
   *
   *             We select the first (last) n_needed element from the front
   *             (end) of f_idx_sorted. We ignore the elements that are already
   *             selected, and those where this->available is false.
   *
   * @param      n_already_selected  number of element already selected (their
   *                                 indices are stored in
   *                                 idx[0:n_already_selected])
   * @param      n_needed            number of elements to be selected
   * @param      copy_front          if true, then copy the elements from the
   *                                 front of the selected list, otherwise copy
   *                                 from the end of the list
   *
   * @return     the number of elements copied (which might be less than
   *             n_needed)
   */
  ...
  /**
   * @brief      Select the first n_needed elements from ws_idx_sorted where op
   *             is true.
   *
