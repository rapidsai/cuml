...
template <typename Dtype>
__device__ Dtype fix_step(Dtype a, Dtype b, Dtype c, Dtype step, double e) {
...
template <typename Dtype>
__device__ Dtype calculate_step(Dtype a, Dtype b, Dtype c, double loss_a,
                                double loss_b, double loss_c, Dtype pstep,
                                double e) {
  // parabola step
...
__device__ void parabolic_interpolation_golden_optim(
  ...
  bool optim_alpha, double *alpha_, bool optim_beta, double *beta_,
  bool optim_gamma, double *gamma_, double eps, bool ADDITIVE_KERNEL) {
  Dtype a = (Dtype).0;
  Dtype b = (Dtype)GOLD;
  Dtype c = (Dtype)1.;

  double loss_a = holtwinters_eval_device<Dtype>(
  ...
  while (abs_device(c - a) > abs_device(b * eps) + PG_EPS) {
    ...
    double loss_val = holtwinters_eval_device<Dtype>(
      tid, ts, n, batch_size, frequency, shift, plevel, ptrend, pseason,
...
__device__ void holtwinters_finite_gradient_device(
  ...
  double *g_gamma, double eps, bool ADDITIVE_KERNEL) {
...
__device__ ML::OptimCriterion holtwinters_bfgs_optim_device(
  ...
  Dtype plevel, Dtype ptrend, Dtype *pseason, int pseason_width,
  const Dtype *start_season, const double *beta, const double *gamma,
  bool optim_alpha, double *x1, bool optim_beta, double *x2, bool optim_gamma,
  double *x3, const ML::OptimParams<double> optim_params,
  bool ADDITIVE_KERNEL) {
  double H11 = 1., H12 = .0, H13 = .0, H22 = 1., H23 = .0,
         H33 = 1.;  // Hessian approximiation (Hessian is symmetric)
  double g1 = .0, g2 = .0, g3 = .0;  // gradients
  ...
  for (int iter = 0; iter < optim_params.bfgs_iter_limit; ++iter) {
    // Step direction
    double p1 = -H11 * g1 - H12 * g2 - H13 * g3;
    double p2 = -H12 * g1 - H22 * g2 - H23 * g3;
    double p3 = -H13 * g1 - H23 * g2 - H33 * g3;

    const double phi = p1 * g1 + p2 * g2 + p3 * g3;
    if (phi > 0) {
      H11 = 1.;
      H12 = 0.;
      H13 = 0.;
    ...
    if (optim_params.linesearch_step_size <= 0)
      step_size = (double)0.866 / sqrt(p1 * p1 + p2 * p2 + p3 * p3);
    else
      step_size = optim_params.linesearch_step_size;
    double nx1 = *x1 + step_size * p1;
    double nx2 = *x2 + step_size * p2;
    double nx3 = *x3 + step_size * p3;

    // line search params
    const double cauchy =
      optim_params.linesearch_c * (g1 * p1 + g2 * p2 + g3 * p3);
    const double loss_ref = holtwinters_eval_device<Dtype>(
      tid, ts, n, batch_size, frequency, shift, plevel, ptrend, pseason,
      pseason_width, start_season, beta, gamma, *x1, *x2, *x3, nullptr, nullptr,
      nullptr, nullptr, ADDITIVE_KERNEL);
    double loss = holtwinters_eval_device<Dtype>(
      tid, ts, n, batch_size, frequency, shift, plevel, ptrend, pseason,
    ...
    const double dx1 = abs_device(*x1 - nx1);
    const double dx2 = abs_device(*x2 - nx2);
    const double dx3 = abs_device(*x3 - nx3);
    double max = max3(dx1, dx2, dx3);
    // update {params}
    *x1 = nx1;
    *x2 = nx2;
    *x3 = nx3;
    ...
    double ng1 = .0, ng2 = .0, ng3 = .0;  // next gradient
    holtwinters_finite_gradient_device<Dtype>(
      tid, ts, n, batch_size, frequency, shift, plevel, ptrend, pseason,
    ...
    // s = step_size*p;
    const double s1 = step_size * p1;
    const double s2 = step_size * p2;
    const double s3 = step_size * p3;

    // y = next_grad-grad
    const double y1 = ng1 - g1;
    const double y2 = ng2 - g2;
    const double y3 = ng3 - g3;

    // rho_ = y(*)s; rho = 1/rho_
    const double rho_ = y1 * s1 + y2 * s2 + y3 * s3;
    const double rho = 1.0 / rho_;

    const double Hy1 = H11 * y1 + H12 * y2 + H13 * y3;
    const double Hy2 = H12 * y1 + H22 * y2 + H23 * y3;
    const double Hy3 = H13 * y1 + H23 * y2 + H33 * y3;
    const double k = rho * rho * (y1 * Hy1 + y2 * Hy2 + y3 * Hy3 + rho_);
...
__global__ void holtwinters_optim_gpu_shared_kernel(
  ...
  double *alpha, bool optim_alpha, double *beta, bool optim_beta, double *gamma,
  bool optim_gamma, Dtype *level, Dtype *trend, Dtype *season, Dtype *xhat,
  double *error, ML::OptimCriterion *optim_result,
  const ML::OptimParams<double> optim_params, bool ADDITIVE_KERNEL,
  bool single_param) {
  int tid = GET_TID;
  ...
  if (tid < batch_size) {
    ...
    double alpha_ = alpha[tid];
    double beta_ = beta ? beta[tid] : .0;
    double gamma_ = gamma ? gamma[tid] : .0;
    ...
    if (error || level || trend || season || xhat) {
      // Final fit
      double error_ = holtwinters_eval_device<Dtype>(
        tid, ts, n, batch_size, frequency, shift, plevel, ptrend,
...
__global__ void holtwinters_optim_gpu_global_kernel(
  const Dtype *ts, int n, int batch_size, int frequency,
  const Dtype *start_level, const Dtype *start_trend, const Dtype *start_season,
  Dtype *pseason, double *alpha, bool optim_alpha, double *beta,
  bool optim_beta, double *gamma, bool optim_gamma, Dtype *level, Dtype *trend,
  Dtype *season, Dtype *xhat, double *error, ML::OptimCriterion *optim_result,
  const ML::OptimParams<double> optim_params, bool ADDITIVE_KERNEL,
  bool single_param) {
  int tid = GET_TID;
  if (tid < batch_size) {
...
void holtwinters_optim_gpu(
  ...
  const Dtype *start_season, double *alpha, bool optim_alpha, double *beta,
  bool optim_beta, double *gamma, bool optim_gamma, Dtype *level, Dtype *trend,
  Dtype *season, Dtype *xhat, double *error, ML::OptimCriterion *optim_result,
  ML::SeasonalType seasonal, const ML::OptimParams<double> optim_params) {
