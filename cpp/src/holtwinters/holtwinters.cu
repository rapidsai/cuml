...
cudaError_t conv1d(const Dtype* input, int batch_size,
  const Dtype *filter, int filter_size, Dtype *output, int output_size);
template <typename Dtype>
cudaError_t season_mean(const Dtype* season, int len, int batch_size,
  Dtype *start_season, int frequency, int half_filter_size, aion::SeasonalType seasonal);
template <typename Dtype>
cudaError_t batched_ls(const Dtype *data, int trend_len,
...
aion::AionStatus stl_decomposition_gpu(const Dtype *ts,
    ...
    Dtype *start_level, Dtype *start_trend, Dtype *start_season, aion::SeasonalType seasonal) {
  ...
  Dtype* filter_d;
  if (cudaMalloc(reinterpret_cast<void**>(&filter_d), sizeof(Dtype)*filter_size))
    return aion::AION_ALLOC_FAILED;
  if (cudaMemcpy(filter_d, filter_h.data(), sizeof(Dtype)*filter_size, cudaMemcpyDefault)) {
    cudaFree(filter_d);
    return aion::AION_INTERNAL_ERROR;
  }

  // Set Trend
  Dtype* trend_d;
  if (cudaMalloc(reinterpret_cast<void**>(&trend_d), sizeof(Dtype)*batch_size*trend_len)) {
    cudaFree(filter_d);
    return aion::AION_ALLOC_FAILED;
  }
  ...
  Dtype* season_d;
  if (cudaMalloc(reinterpret_cast<void**>(&season_d), sizeof(Dtype)*batch_size*trend_len)) {
    cudaFree(filter_d);
    cudaFree(trend_d);
    return aion::AION_ALLOC_FAILED;
  }
  ...
  if (seasonal == aion::SeasonalType::ADDITIVE) {
    ...
    aion::cublas::geam<Dtype>(CUBLAS_OP_N, CUBLAS_OP_N,
      trend_len, batch_size,
      &one, ts+ts_offset, trend_len,
      &minus_one, trend_d, trend_len,
      season_d, trend_len);
  } else {
    aion::math::div_gpu<Dtype>(trend_len*batch_size, ts+ts_offset, trend_d, season_d);
  }

  season_mean(season_d, trend_len, batch_size,
    start_season, frequency, filter_size/2, seasonal);  // TODO(ahmad): return

  batched_ls(trend_d, trend_len, batch_size, start_level, start_trend);  // TODO(ahmad): return
...
cudaError_t conv1d(const Dtype* input, int batch_size,
  ...
  conv1d_kernel <Dtype> <<<GET_NUM_BLOCKS(total_threads), GET_THREADS_PER_BLOCK(total_threads)>>> (input, batch_size,
    filter, filter_size,
    output, output_size);
...
cudaError_t season_mean(const Dtype* season, int len, int batch_size,
  ...
  if (seasonal == aion::SeasonalType::ADDITIVE)
    season_mean_kernel <Dtype, true> <<<GET_NUM_BLOCKS(batch_size), GET_THREADS_PER_BLOCK(batch_size)>>> (season,
      len, batch_size, start_season, frequency, half_filter_size);
  else
    season_mean_kernel <Dtype, false> <<<GET_NUM_BLOCKS(batch_size), GET_THREADS_PER_BLOCK(batch_size)>>> (season,
      len, batch_size, start_season, frequency, half_filter_size);
...
__global__ void batched_ls_solver_kernel(const Dtype *B, const Dtype *rq,
    int batch_size, int len, Dtype *level, Dtype *trend) {
  int tid = GET_TID;
  if (tid < batch_size) {
...
cudaError_t batched_ls(const Dtype *data, int trend_len,
  ...
  Dtype *A_d = nullptr, *tau_d = nullptr, *Rinv_d = nullptr, *R1Qt_d = nullptr, *lwork_d = nullptr;
  int *dev_info_d = nullptr;
  status = cudaMalloc(reinterpret_cast<void**>(&A_d), sizeof(Dtype)*2*trend_len);
  if (status)
    goto clean;
  ...
  if (status)
    goto clean;
  status = cudaMalloc(reinterpret_cast<void**>(&R1Qt_d), sizeof(Dtype)*2*trend_len);
  if (status)
    goto clean;
  ...
  status = cudaMemcpy(A_d, A_h.data(), sizeof(Dtype)*2*trend_len, cudaMemcpyDefault);
  if (status)
    goto clean;

  aion::cusolver::geqrf_bufferSize<Dtype>(trend_len, 2, A_d, 2, &geqrf_buffer);
  aion::cusolver::orgqr_bufferSize<Dtype>(trend_len, 2, 2, A_d, 2, tau_d, &orgqr_buffer);
  lwork_size = geqrf_buffer > orgqr_buffer ? geqrf_buffer : orgqr_buffer;
  status = cudaMalloc(reinterpret_cast<void**>(&lwork_d), sizeof(Dtype)*lwork_size);
  if (status)
    goto clean;

  // QR decomposition of A
  // TODO(ahmad): return value
  aion::cusolver::geqrf<Dtype>(trend_len, 2, A_d, trend_len, tau_d, lwork_d, lwork_size, dev_info_d);
  ...
  // TODO(ahmad): return value
  aion::cusolver::orgqr<Dtype>(trend_len, 2, 2, A_d, trend_len, tau_d, lwork_d, lwork_size, dev_info_d);
  aion::cublas::gemm<Dtype>(CUBLAS_OP_N, CUBLAS_OP_T,
    2, trend_len, 2,
    &one, Rinv_d, 2,
    A_d, trend_len,
    &zero, R1Qt_d, 2);  // TODO(ahmad): return value

  batched_ls_solver_kernel<Dtype> <<<GET_NUM_BLOCKS(batch_size), GET_THREADS_PER_BLOCK(batch_size)>>> (data,
    R1Qt_d, batch_size, trend_len, level, trend);
...
template aion::AionStatus stl_decomposition_gpu<float>(const float *ts,
  int n, int batch_size, int frequency, int start_periods,
  float *level, float *trend, float *season, aion::SeasonalType seasonal);
template aion::AionStatus stl_decomposition_gpu<double>(const double *ts,
  int n, int batch_size, int frequency, int start_periods,
  double *level, double *trend, double *season, aion::SeasonalType seasonal);

template<typename Dtype, bool additive_seasonal>
__global__ void holtwinters_eval_gpu_global_kernel(const Dtype *ts, int n, int batch_size, int frequency,
const Dtype *start_level, const Dtype *start_trend, const Dtype *start_season, Dtype *pseason,
const Dtype *alpha, const Dtype *beta, const Dtype *gamma,
Dtype *level, Dtype *trend, Dtype *season, Dtype *xhat, Dtype *error);
template<typename Dtype, bool additive_seasonal>
__global__ void holtwinters_eval_gpu_shared_kernel(const Dtype *ts, int n, int batch_size, int frequency,
const Dtype *start_level, const Dtype *start_trend, const Dtype *start_season,
const Dtype *alpha, const Dtype *beta, const Dtype *gamma,
Dtype *level, Dtype *trend, Dtype *season, Dtype *xhat, Dtype *error);


template<> __device__  float bound_device<float>(float val, float min, float max) {
return fminf(fmaxf(val, min), max);
}
template<> __device__  double bound_device<double>(double val, double min, double max) {
return fmin(fmax(val, min), max);
}

template<> __device__  float abs_device<float>(float val) {
return fabsf(val);
}

template<> __device__  double abs_device<double>(double val) {
return fabs(val);
}
...
if (sm_needed > prop.sharedMemPerBlock) {  // TODO(ahmad): test shared/general kernels
    Dtype *pseason;
    cudaMalloc(reinterpret_cast<void**>(&pseason), sizeof(Dtype)*batch_size*frequency);  // TODO(ahmad): return value;
    if (seasonal == aion::SeasonalType::ADDITIVE)
    holtwinters_eval_gpu_global_kernel <Dtype, true> <<<total_blocks,
        threads_per_block>>> (ts, n, batch_size, frequency,
        start_level, start_trend, start_season, pseason,
        alpha, beta, gamma,
        level, trend, season, xhat, error);
    else
    holtwinters_eval_gpu_global_kernel <Dtype, false> <<<total_blocks,
        threads_per_block>>> (ts, n, batch_size, frequency,
        start_level, start_trend, start_season, pseason,
        alpha, beta, gamma,
        level, trend, season, xhat, error);
    ...
    if (seasonal == aion::SeasonalType::ADDITIVE)
    holtwinters_eval_gpu_shared_kernel <Dtype, true> <<<total_blocks,
        threads_per_block, sm_needed>>> (ts, n, batch_size, frequency,
        start_level, start_trend, start_season,
        alpha, beta, gamma,
        level, trend, season, xhat, error);
    else
    holtwinters_eval_gpu_shared_kernel <Dtype, false> <<<total_blocks,
        threads_per_block, sm_needed>>> (ts, n, batch_size, frequency,
        start_level, start_trend, start_season,
        alpha, beta, gamma,
        level, trend, season, xhat, error);
...
for (int i = 0; i < n-shift; i++) {
    ...
    if (level)
    level[IDX(tid, i, batch_size)] = clevel;
    if (trend)
    trend[IDX(tid, i, batch_size)] = ctrend;
    if (season)
    season[IDX(tid, i, batch_size)] = cseason;
    if (xhat)
    xhat[IDX(tid, i, batch_size)] = xhat_;
}
return error_;
}
...
if (tid < batch_size) {
    ...
    Dtype error_ = holtwinters_eval_device<Dtype, additive_seasonal>(tid,
    ts, n, batch_size, frequency,  shift,
    plevel, ptrend, pseason+threadIdx.x, blockDim.x, start_season,
    beta, gamma,
    alpha_, beta_, gamma_,
    level, trend, season, xhat);
    if (error)
    error[tid] = error_;
}
}
...
if (tid < batch_size) {
    ...
    Dtype error_ = holtwinters_eval_device<Dtype, additive_seasonal>(tid,
    ts, n, batch_size, frequency,  shift,
    plevel, ptrend, pseason+tid, batch_size, start_season,
    beta, gamma,
    alpha_, beta_, gamma_,
    level, trend, season, xhat);
    if (error)
...
template void holtwinters_eval_gpu<float>(const float *ts, int n, int batch_size, int frequency,
...
aion::SeasonalType seasonal);
template void holtwinters_eval_gpu<double>(const double *ts, int n, int batch_size, int frequency,
...
template __device__ float holtwinters_eval_device<float, false>(int tid,
const float *ts, int n, int batch_size, int frequency, int shift,
float plevel, float ptrend, float *pseason, int pseason_width, const float* start_season,
const float *beta, const float *gamma,
float alpha_, float beta_, float gamma_,
float *level, float *trend, float *season, float *xhat);
template __device__ float holtwinters_eval_device<float, true>(int tid,
const float *ts, int n, int batch_size, int frequency, int shift,
float plevel, float ptrend, float *pseason, int pseason_width, const float* start_season,
const float *beta, const float *gamma,
float alpha_, float beta_, float gamma_,
float *level, float *trend, float *season, float *xhat);
template __device__ double holtwinters_eval_device<double, false>(int tid,
const double *ts, int n, int batch_size, int frequency, int shift,
double plevel, double ptrend, double *pseason, int pseason_width, const double* start_season,
const double *beta, const double *gamma,
double alpha_, double beta_, double gamma_,
double *level, double *trend, double *season, double *xhat);
template __device__ double holtwinters_eval_device<double, true>(int tid,
const double *ts, int n, int batch_size, int frequency, int shift,
double plevel, double ptrend, double *pseason, int pseason_width, const double* start_season,
const double *beta, const double *gamma,
double alpha_, double beta_, double gamma_,
...
__global__ void holtwinters_seasonal_forecast_kernel(Dtype *forecast, int h, int batch_size, int frequency,
...
template <typename Dtype>
__global__ void holtwinters_nonseasonal_forecast_kernel(Dtype *forecast, int h, int batch_size,
...
template <typename Dtype>
__global__ void holtwinters_level_forecast_kernel(Dtype *forecast, int h, int batch_size,
...
void holtwinters_forecast_gpu(Dtype *forecast, int h, int batch_size, int frequency,
  ...
  if (trend_coef == nullptr && season_coef == nullptr) {
    holtwinters_level_forecast_kernel<Dtype> <<<total_blocks, threads_per_block>>>(forecast,
      h, batch_size, level_coef);
  } else if (season_coef == nullptr) {
    holtwinters_nonseasonal_forecast_kernel<Dtype> <<<total_blocks, threads_per_block>>>(forecast,
      h, batch_size, level_coef, trend_coef);
  } else {
    if (seasonal == aion::SeasonalType::ADDITIVE)
      holtwinters_seasonal_forecast_kernel <Dtype, true> <<<total_blocks, threads_per_block>>>(forecast,
        h, batch_size, frequency, level_coef, trend_coef, season_coef);
    else
      holtwinters_seasonal_forecast_kernel <Dtype, false> <<<total_blocks, threads_per_block>>>(forecast,
        h, batch_size, frequency, level_coef, trend_coef, season_coef);
  }
}

template void holtwinters_forecast_gpu<float>(float *forecast,
  int h, int batch_size, int frequency,
  const float *level_coef, const float *trend_coef, const float *season_coef,
  aion::SeasonalType seasonal);
template void holtwinters_forecast_gpu<double>(double *forecast,
  int h, int batch_size, int frequency,
...
__global__ void holtwinters_optim_gpu_shared_kernel(const Dtype *ts, int n, int batch_size, int frequency,
const Dtype *start_level, const Dtype *start_trend, const Dtype *start_season,
Dtype *alpha, bool optim_alpha, Dtype *beta, bool optim_beta, Dtype *gamma, bool optim_gamma,
Dtype *level, Dtype *trend, Dtype *season,
Dtype *xhat, Dtype *error, aion::OptimCriterion *optim_result,
const aion::OptimParams<Dtype> optim_params);

template<typename Dtype, bool ADDITIVE_KERNEL, bool single_param>
__global__ void holtwinters_optim_gpu_global_kernel(const Dtype *ts, int n, int batch_size, int frequency,
const Dtype *start_level, const Dtype *start_trend, const Dtype *start_season, Dtype* pseason,
Dtype *alpha, bool optim_alpha, Dtype *beta, bool optim_beta, Dtype *gamma, bool optim_gamma,
Dtype *level, Dtype *trend, Dtype *season,
Dtype *xhat, Dtype *error, aion::OptimCriterion *optim_result,
...
Dtype plevel, Dtype ptrend, Dtype *pseason, int pseason_width, const Dtype *start_season,
const Dtype *beta, const Dtype *gamma,
bool optim_alpha, Dtype *alpha_, bool optim_beta, Dtype *beta_, bool optim_gamma, Dtype *gamma_,
Dtype eps);

template<typename Dtype, bool ADDITIVE_KERNEL>
__device__ aion::OptimCriterion holtwinters_bfgs_optim_device(int tid,
const Dtype *ts, int n, int batch_size, int frequency, int shift,
Dtype plevel, Dtype ptrend, Dtype *pseason, int pseason_width, const Dtype *start_season,
const Dtype *beta, const Dtype *gamma,
bool optim_alpha, Dtype* x1, bool optim_beta, Dtype* x2, bool optim_gamma, Dtype* x3,
const aion::OptimParams<Dtype> optim_params);
...
template<typename Dtype>
void holtwinters_optim_gpu(const Dtype *ts, int n, int batch_size, int frequency,
    const Dtype *start_level, const Dtype *start_trend, const Dtype *start_season,
    Dtype *alpha, bool optim_alpha, Dtype *beta, bool optim_beta, Dtype *gamma, bool optim_gamma,
    Dtype *level, Dtype *trend, Dtype *season,
...
if (sm_needed > prop.sharedMemPerBlock) {  // Global memory // TODO(ahmad): test shared/general kernels
    Dtype *pseason;
    cudaMalloc(reinterpret_cast<void**>(&pseason), sizeof(Dtype)*batch_size*frequency);  // TODO(ahmad): return value;
    if (seasonal == aion::SeasonalType::ADDITIVE) {
    if (optim_alpha+optim_beta+optim_gamma > 1)
        holtwinters_optim_gpu_global_kernel <Dtype, true, false> <<<total_blocks,
        threads_per_block>>> (ts, n, batch_size, frequency,
        start_level, start_trend, start_season, pseason,
        alpha, optim_alpha, beta, optim_beta, gamma, optim_gamma,
        level, trend, season,
        xhat, error, optim_result,
        optim_params);
    else
        holtwinters_optim_gpu_global_kernel <Dtype, true, true> <<<total_blocks,
        threads_per_block>>> (ts, n, batch_size, frequency,
        start_level, start_trend, start_season, pseason,
        alpha, optim_alpha, beta, optim_beta, gamma, optim_gamma,
        level, trend, season,
    ...
    if (optim_alpha+optim_beta+optim_gamma > 1)
        holtwinters_optim_gpu_global_kernel <Dtype, false, false> <<<total_blocks,
        threads_per_block>>> (ts, n, batch_size, frequency,
        start_level, start_trend, start_season, pseason,
        alpha, optim_alpha, beta, optim_beta, gamma, optim_gamma,
        level, trend, season,
        ...
        holtwinters_optim_gpu_global_kernel <Dtype, false, true> <<<total_blocks,
        threads_per_block>>> (ts, n, batch_size, frequency,
        start_level, start_trend, start_season, pseason,
        alpha, optim_alpha, beta, optim_beta, gamma, optim_gamma,
        level, trend, season,
    ...
    if (seasonal == aion::SeasonalType::ADDITIVE) {
    if (optim_alpha+optim_beta+optim_gamma > 1)
        holtwinters_optim_gpu_shared_kernel <Dtype, true, false> <<<total_blocks,
        threads_per_block, sm_needed>>> (ts, n, batch_size, frequency,
        start_level, start_trend, start_season,
        alpha, optim_alpha, beta, optim_beta, gamma, optim_gamma,
        level, trend, season,
        ...
        holtwinters_optim_gpu_shared_kernel <Dtype, true, true> <<<total_blocks,
        threads_per_block, sm_needed>>> (ts, n, batch_size, frequency,
        start_level, start_trend, start_season,
        alpha, optim_alpha, beta, optim_beta, gamma, optim_gamma,
        level, trend, season,
    ...
    if (optim_alpha+optim_beta+optim_gamma > 1)
        holtwinters_optim_gpu_shared_kernel <Dtype, false, false> <<<total_blocks,
        threads_per_block, sm_needed>>> (ts, n, batch_size, frequency,
        start_level, start_trend, start_season,
        alpha, optim_alpha, beta, optim_beta, gamma, optim_gamma,
        level, trend, season,
        ...
        holtwinters_optim_gpu_shared_kernel <Dtype, false, true> <<<total_blocks,
        threads_per_block, sm_needed>>> (ts, n, batch_size, frequency,
        start_level, start_trend, start_season,
        alpha, optim_alpha, beta, optim_beta, gamma, optim_gamma,
        level, trend, season,
...
if (tid < batch_size) {
    ...
    // Optimization
    if (single_param)
    parabolic_interpolation_golden_optim<Dtype, ADDITIVE_KERNEL>(tid,
        ts, n, batch_size, frequency, shift,
        plevel, ptrend, pseason+threadIdx.x, blockDim.x, start_season,
        beta, gamma,
    ...
    optim = holtwinters_bfgs_optim_device<Dtype, ADDITIVE_KERNEL>(tid,
        ts, n, batch_size, frequency, shift,
        plevel, ptrend, pseason+threadIdx.x, blockDim.x, start_season,
        beta, gamma,
        optim_alpha, &alpha_, optim_beta, &beta_, optim_gamma, &gamma_,
    ...
    if (optim_alpha)
    alpha[tid] = bound_device(alpha_);
    if (optim_beta)
    beta[tid] = bound_device(beta_);
    if (optim_gamma)
    gamma[tid] = bound_device(gamma_);
    if (!single_param && optim_result)
    optim_result[tid] = optim;

    if (error || level || trend || season || xhat) {
    // Final fit
    Dtype error_ = holtwinters_eval_device<Dtype, ADDITIVE_KERNEL>(tid, ts, n, batch_size, frequency,  shift,
        plevel, ptrend, pseason+threadIdx.x, blockDim.x, start_season,
        beta, gamma,
        alpha_, beta_, gamma_,
        level, trend, season, xhat);
...
template<typename Dtype>
__device__ Dtype calculate_step(Dtype a, Dtype b, Dtype c,
    Dtype loss_a, Dtype loss_b, Dtype loss_c,
    Dtype pstep, Dtype e) {
// parabola step
...
Dtype loss_a = holtwinters_eval_device<Dtype, ADDITIVE_KERNEL>(tid,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, pseason, pseason_width, start_season,
    beta, gamma,
    optim_alpha ? a : *alpha_,
    optim_beta ? a : *beta_,
    optim_gamma ? a : *gamma_,
    nullptr, nullptr, nullptr, nullptr);
Dtype loss_b = holtwinters_eval_device<Dtype, ADDITIVE_KERNEL>(tid,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, pseason, pseason_width, start_season,
    beta, gamma,
    ...
    nullptr, nullptr, nullptr, nullptr);
Dtype loss_c = holtwinters_eval_device<Dtype, ADDITIVE_KERNEL>(tid,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, pseason, pseason_width, start_season,
    beta, gamma,
...
while (abs_device(c-a) > abs_device(b*eps)+PG_EPS) {
    ...
    Dtype loss_val = holtwinters_eval_device<Dtype, ADDITIVE_KERNEL>(tid,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, pseason, pseason_width, start_season,
    beta, gamma,
...
if (g_alpha) {  // alpha gradient
    left_error = holtwinters_eval_device<Dtype, ADDITIVE_KERNEL>(tid,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, pseason, pseason_width, start_season,
    beta, gamma,
    alpha_-eps, beta_, gamma_,
    nullptr, nullptr, nullptr, nullptr);
    right_error = holtwinters_eval_device<Dtype, ADDITIVE_KERNEL>(tid,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, pseason, pseason_width, start_season,
    beta, gamma,
    alpha_+eps, beta_, gamma_,
...
Dtype H11 = 1., H12 = .0, H13 = .0, H22 = 1., H23 = .0, H33 = 1.;  // Hessian approximiation (Hessian is symmetric)
Dtype g1 = .0, g2 = .0, g3 = .0;  // gradients

// initial gradient
holtwinters_finite_gradient_device<Dtype, ADDITIVE_KERNEL>(tid,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, pseason, pseason_width, start_season,
    beta, gamma,
    *x1, *x2, *x3,
...
for (int iter = 0; iter < optim_params.bfgs_iter_limit; ++iter) {
    ...
    const Dtype loss_ref = holtwinters_eval_device<Dtype, ADDITIVE_KERNEL>(tid,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, pseason, pseason_width, start_season,
    beta, gamma,
    *x1, *x2, *x3,
    nullptr, nullptr, nullptr, nullptr);
    Dtype loss = holtwinters_eval_device<Dtype, ADDITIVE_KERNEL>(tid,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, pseason, pseason_width, start_season,
    beta, gamma,
    nx1, nx2, nx3,
    ...
    for (int i = 0; i < optim_params.linesearch_iter_limit && (loss > loss_ref+step_size*cauchy); ++i) {
    step_size *= optim_params.linesearch_tau;
    nx1 = *x1+step_size*p1;
    nx2 = *x2+step_size*p2;
    nx3 = *x3+step_size*p3;
    loss = holtwinters_eval_device<Dtype, ADDITIVE_KERNEL>(tid,
        ts, n, batch_size, frequency, shift,
        plevel, ptrend, pseason, pseason_width, start_season,
        beta, gamma,
        nx1, nx2, nx3,
...
return aion::OptimCriterion::OPTIM_BFGS_ITER_LIMIT;
}


template void holtwinters_optim_gpu<float>(const float *ts, int n, int batch_size, int frequency,
const float *start_level, const float *start_trend, const float *start_season,
float *alpha, bool optim_alpha, float *beta, bool optim_beta, float *gamma, bool optim_gamma,
float *level, float *trend, float *season,
float *xhat, float *error,
aion::OptimCriterion *optim_result,
aion::SeasonalType seasonal,
const aion::OptimParams<float> optim_params);
template void holtwinters_optim_gpu<double>(const double *ts, int n, int batch_size, int frequency,
const double *start_level, const double *start_trend, const double *start_season,
...
double *xhat, double *error,
aion::OptimCriterion *optim_result,
aion::SeasonalType seasonal,
const aion::OptimParams<double> optim_params);


template __device__ void holtwinters_finite_gradient_device<float, true>(int tid,
const float *ts, int n, int batch_size, int frequency, int shift,
float plevel, float ptrend, float *pseason, int pseason_width, const float *start_season,
const float *beta, const float *gamma,
float alpha_, float beta_, float gamma_,
float *g_alpha, float *g_beta, float *g_gamma,
float eps);
template __device__ void holtwinters_finite_gradient_device<float, false>(int tid,
const float *ts, int n, int batch_size, int frequency, int shift,
float plevel, float ptrend, float *pseason, int pseason_width, const float *start_season,
const float *beta, const float *gamma,
float alpha_, float beta_, float gamma_,
float *g_alpha, float *g_beta, float *g_gamma,
float eps);
template __device__ void holtwinters_finite_gradient_device<double, true>(int tid,
const double *ts, int n, int batch_size, int frequency, int shift,
double plevel, double ptrend, double *pseason, int pseason_width, const double *start_season,
const double *beta, const double *gamma,
double alpha_, double beta_, double gamma_,
double *g_alpha, double *g_beta, double *g_gamma,
double eps);
template __device__ void holtwinters_finite_gradient_device<double, false>(int tid,
const double *ts, int n, int batch_size, int frequency, int shift,
double plevel, double ptrend, double *pseason, int pseason_width, const double *start_season,
const double *beta, const double *gamma,
double alpha_, double beta_, double gamma_,
