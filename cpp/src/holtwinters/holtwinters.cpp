...
void batched_ls(const std::vector<Dtype>& data, int trend_len,
  int batch_size, Dtype *level, Dtype* trend);
template <typename Dtype>
void batched_ls_solver(const std::vector<Dtype>& B, const std::vector<Dtype>& rq, int batch_size, int len,
...
aion::AionStatus stl_decomposition_cpu(const Dtype *ts,
  ...
  for (int j = 0; j < batch_size; ++j) {
    ...
    for (int i = 0; i < frequency; ++i) {
      if (seasonal == aion::SeasonalType::ADDITIVE)
        start_season[i*batch_size+j] = start_season[i*batch_size+j]-season_mean;
  ...
  batched_ls<Dtype>(trend, trend_len,
    batch_size, start_level, start_trend);
...
void batched_ls(const std::vector<Dtype>& data, int trend_len,
  ...
  aion::lapack::geqrf<Dtype>(trend_len, 2, A.data(), 2, tau.data());  // TODO(ahmad): return value
  ...
  aion::cblas::gemm<Dtype>(CblasNoTrans, CblasTrans,
    2, trend_len, 2, 1., R_inv, 2, A.data(), 2, 0., R1Qt.data(), trend_len);

  batched_ls_solver<Dtype>(data, R1Qt, batch_size, trend_len, level, trend);
}

template <typename Dtype>
void batched_ls_solver(const std::vector<Dtype>& B, const std::vector<Dtype>& rq,
...
template aion::AionStatus stl_decomposition_cpu<float>(const float *ts,
  int batch_size, int frequency, int start_periods,
  float *level, float *trend, float *season, aion::SeasonalType seasonal);
template aion::AionStatus stl_decomposition_cpu<double>(const double *ts,
  int batch_size, int frequency, int start_periods,
...
void holtwinters_eval_cpu(const Dtype *ts, int n, int batch_size, int frequency,
  ...
  for (int j = 0; j < batch_size; ++j) {
    ...
    Dtype error_ = holtwinters_eval_host<Dtype>(j, ts, n, batch_size, frequency, shift,
      plevel, ptrend, start_season,
      beta, gamma,
      alpha_, beta_, gamma_,
...
Dtype holtwinters_eval_host(int id, const Dtype *ts, int n, int batch_size, int frequency, int shift,
  ...
  for (int i = 0; i < n-shift; i++) {
    ...
    if (level)
      level[IDX(id, i, batch_size)] = clevel;
    if (trend)
      trend[IDX(id, i, batch_size)] = ctrend;
    if (season)
      season[IDX(id, i, batch_size)] = cseason;
    if (xhat)
      xhat[IDX(id, i, batch_size)] = xhat_;
  }
  return error_;
}


template void holtwinters_eval_cpu<float>(const float *ts, int n, int batch_size, int frequency,
  ...
  aion::SeasonalType seasonal);
template void holtwinters_eval_cpu<double>(const double *ts, int n, int batch_size, int frequency,
...
template float holtwinters_eval_host<float>(int id, const float *ts, int n, int batch_size, int frequency, int shift,
  float plevel, float ptrend, const float *start_season,
  const float *beta, const float *gamma,
  float alpha_, float beta_, float gamma_,
  float *level, float *trend, float *season, float *xhat,
  aion::SeasonalType seasonal);
template double holtwinters_eval_host<double>(int id, const double *ts, int n, int batch_size, int frequency, int shift,
  double plevel, double ptrend, const double *start_season,
...
void holtwinters_forecast_cpu(Dtype *forecast, int h, int batch_size, int frequency,
  ...
  for (int j = 0; j < batch_size; ++j) {
    ...
    for (int i = 0; i < h; ++i) {
      const Dtype season = (season_coef) ? season_coef[j+(i%frequency)*batch_size] :
...
template void holtwinters_forecast_cpu<float>(float *forecast,
  int h, int batch_size, int frequency,
  const float *level_coef, const float *trend_coef, const float *season_coef,
  aion::SeasonalType seasonal);
template void holtwinters_forecast_cpu<double>(double *forecast,
  int h, int batch_size, int frequency,
  const double *level_coef, const double *trend_coef, const double *season_coef,
  aion::SeasonalType seasonal);

template<typename Dtype>
aion::OptimCriterion holtwinters_bfgs_optim_host(int id,
  const Dtype *ts, int n, int batch_size, int frequency, int shift,
  Dtype plevel, Dtype ptrend, const Dtype *start_season,
  const Dtype *beta, const Dtype *gamma,
  bool optim_alpha, bool optim_beta, bool optim_gamma, std::vector<Dtype>& x,
  aion::SeasonalType seasonal,
  const aion::OptimParams<Dtype> optim_params);

template<typename Dtype>
void parabolic_interpolation_golden_optim(int id,
  const Dtype *ts, int n, int batch_size, int frequency, int shift,
  Dtype plevel, Dtype ptrend, const Dtype *start_season,
  const Dtype *beta, const Dtype *gamma,
  bool optim_alpha, bool optim_beta, bool optim_gamma, std::vector<Dtype>& x,
  aion::SeasonalType seasonal, Dtype e);
...
void holtwinters_optim_cpu(const Dtype *ts,
  ...
  for (int j = 0; j < batch_size; ++j) {
    ...
    if (dim == 1)
      parabolic_interpolation_golden_optim<Dtype>(j, ts, n, batch_size, frequency, shift,
        plevel, ptrend, start_season,
        beta, gamma,
        optim_alpha, optim_beta, optim_gamma, x,
        seasonal, optim_params.eps);
    else
      optim = holtwinters_bfgs_optim_host<Dtype>(j, ts, n, batch_size, frequency, shift,
        plevel, ptrend, start_season,
        beta, gamma,
        optim_alpha, optim_beta, optim_gamma, x,
        seasonal,
    ...
    if (optim_alpha)
      alpha[j] = bound_host(x[0]);
    if (optim_beta)
      beta[j] = bound_host(x[1]);
    if (optim_gamma)
      gamma[j] = bound_host(x[2]);
    if (dim > 1 && optim_result)
      optim_result[j] = optim;
...
template<typename Dtype>
Dtype calculate_step(Dtype a, Dtype b, Dtype c,
  ...
  if (fabs(step) > fabs(pstep/2) || step == 0.0)
...
void parabolic_interpolation_golden_optim(int id,
    ...
    const Dtype *beta, const Dtype *gamma,
    bool optim_alpha, bool optim_beta, bool optim_gamma, std::vector<Dtype>& x,
    aion::SeasonalType seasonal, Dtype eps) {

  Dtype a = (Dtype).0;
  Dtype b = (Dtype)GOLD;
  Dtype c = (Dtype)1.;

  Dtype loss_a = holtwinters_eval_host<Dtype>(id,
    ...
    seasonal);
  Dtype loss_b = holtwinters_eval_host<Dtype>(id,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, start_season,
    beta, gamma,
    optim_alpha ? b : x[0],
    ...
    nullptr, nullptr, nullptr, nullptr,
    seasonal);
  Dtype loss_c = holtwinters_eval_host<Dtype>(id,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, start_season,
    beta, gamma,
    optim_alpha ? c : x[0],
  ...
  while (fabs(c-a) > fabs(b*eps)+PG_EPS) {
    ...
    Dtype loss_val = holtwinters_eval_host<Dtype>(id,
      ts, n, batch_size, frequency, shift,
      plevel, ptrend, start_season,
      beta, gamma,
      optim_alpha ? optim_val : x[0],
      optim_beta ? optim_val : x[1],
      optim_gamma ? optim_val : x[2],
      nullptr, nullptr, nullptr, nullptr,
      seasonal);
    if (loss_val < loss_b) {
      if (optim_val > b) {
...
void holtwinters_finite_gradient_host(int id,
  ...
  if (g_alpha) {  // alpha gradient
    left_error = holtwinters_eval_host<Dtype>(id,
      ts, n, batch_size, frequency, shift,
      plevel, ptrend, start_season,
      beta, gamma,
      ...
      nullptr, nullptr, nullptr, nullptr,
      seasonal);
    right_error = holtwinters_eval_host<Dtype>(id,
      ts, n, batch_size, frequency, shift,
      plevel, ptrend, start_season,
      beta, gamma,
      alpha_+eps, beta_, gamma_,
...
template<typename Dtype>
Dtype linesearch(int dim, int id,
    const Dtype *ts, int n, int batch_size, int frequency, int shift,
    Dtype plevel, Dtype ptrend, const Dtype *start_season,
    const Dtype *beta, const Dtype *gamma,
    const std::vector<Dtype>& p, const std::vector<Dtype>& g,
    const std::vector<Dtype>& x, std::vector<Dtype>& nx,
    Dtype* loss_diff, const aion::OptimParams<Dtype> optim_params,
    aion::SeasonalType seasonal) {
  // {next_params} = {params}+step_size*p;
  Dtype step_size = optim_params.linesearch_step_size;
  if (step_size <= 0)
    step_size = (Dtype)0.866/sqrt(aion::cblas::dot(dim, p.data(), 1, p.data(), 1));
  nx = x;
  aion::cblas::axpy<Dtype>(dim, step_size, p.data(), nx.data());

  const Dtype cauchy = optim_params.linesearch_c*aion::cblas::dot<Dtype>(dim, g.data(), 1, p.data(), 1);
  const Dtype loss_ref = holtwinters_eval_host<Dtype>(id,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, start_season,
    beta, gamma,
    x[0], x[1], x[2],
    nullptr, nullptr, nullptr, nullptr,
    seasonal);
  Dtype loss = holtwinters_eval_host<Dtype>(id,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, start_season,
    beta, gamma,
    nx[0], nx[1], nx[2],
  ...
  for (int i = 0; (i < optim_params.linesearch_iter_limit) && (loss > loss_ref+step_size*cauchy); ++i) {
    ...
    nx = x;
    aion::cblas::axpy<Dtype>(dim, step_size, p.data(), nx.data());
    loss = holtwinters_eval_host<Dtype>(id,
      ts, n, batch_size, frequency, shift,
      plevel, ptrend, start_season,
      beta, gamma,
      nx[0], nx[1], nx[2],
...
aion::OptimCriterion holtwinters_bfgs_optim_host(int id,
  ...
  holtwinters_finite_gradient_host<Dtype>(id,
    ts, n, batch_size, frequency, shift,
    plevel, ptrend, start_season,
    beta, gamma,
    x[0], x[1], x[2],
    optim_alpha ? &g[0] : nullptr,
    optim_beta ? &g[1] : nullptr,
    optim_gamma ? &g[2] : nullptr,
    seasonal, optim_params.eps);

  for (int iter = 0; iter < optim_params.bfgs_iter_limit; ++iter) {
    // p = -H*gradient;
    aion::cblas::gemv(CblasNoTrans, dim, dim,
      (Dtype)-1., H.data(), dim, g.data(), 1, (Dtype)0., p.data(), 1);
    ...
    // {next_params} = {params}+step_size*p;
    Dtype loss_diff;
    Dtype step_size = linesearch(dim, id,
      ts, n, batch_size, frequency, shift,
      plevel, ptrend, start_season,
      beta, gamma,
      p, g,
      x, nx, &loss_diff, optim_params,
      seasonal);
    ...
    holtwinters_finite_gradient_host<Dtype>(id,
      ...
      beta, gamma,
      nx[0], nx[1], nx[2],
      optim_alpha ? &ng[0] : nullptr,
      optim_beta ? &ng[1] : nullptr,
      optim_gamma ? &ng[2] : nullptr,
      seasonal, optim_params.eps);
    ...
    aion::cblas::gemv<Dtype>(CblasNoTrans, dim, dim, (Dtype)1.,
      H.data(), dim, y.data(), 1, (Dtype)0., tmp_buffer.data(), 1);  // Hy = H*y
    const Dtype yHy = aion::cblas::dot<Dtype>(dim, y.data(), 1, tmp_buffer.data(), 1);

    // H = H + [(rho+zeta)/rho^2]*[s*st]
    aion::cblas::ger<Dtype>(dim, dim, (rho_+yHy)*(rho*rho), s.data(), 1, s.data(), 1, H.data(), dim);

    // TODO(ahmad): use SSYR2 instead (H is symmetric)
    // H = H + [(rho+zeta)/rho^2]*[Hy*st+s*yt*H]
    aion::cblas::ger<Dtype>(dim, dim, -rho, tmp_buffer.data(), 1, s.data(), 1, H.data(), dim);
    aion::cblas::ger<Dtype>(dim, dim, -rho, s.data(), 1, tmp_buffer.data(), 1, H.data(), dim);
...
template void holtwinters_optim_cpu<float>(const float *ts,
  int n, int batch_size, int frequency,
  const float *start_level, const float *start_trend, const float *start_season,
  float *alpha, bool optim_alpha, float *beta, bool optim_beta, float *gamma, bool optim_gamma,
  float *level, float *trend, float *season,
  float *xhat, float *error, aion::OptimCriterion *optim_result, aion::SeasonalType seasonal,
  const aion::OptimParams<float> optim_params);
template void holtwinters_optim_cpu<double>(const double *ts,
  int n, int batch_size, int frequency,
  const double *start_level, const double *start_trend, const double *start_season,
  double *alpha, bool optim_alpha, double *beta, bool optim_beta, double *gamma, bool optim_gamma,
  double *level, double *trend, double *season,
  double *xhat, double *error, aion::OptimCriterion *optim_result, aion::SeasonalType seasonal,
  const aion::OptimParams<double> optim_params);

template void holtwinters_finite_gradient_host<float>(int id,
  const float *ts, int n, int batch_size, int frequency, int shift,
  float plevel, float ptrend, const float *start_season,
  const float *beta, const float *gamma,
  float alpha_, float beta_, float gamma_,
  float *g_alpha, float *g_beta, float *g_gamma,
  aion::SeasonalType seasonal, float eps);
template void holtwinters_finite_gradient_host<double>(int id,
  const double *ts, int n, int batch_size, int frequency, int shift,
  double plevel, double ptrend, const double *start_season,
  const double *beta, const double *gamma,
  double alpha_, double beta_, double gamma_,
  double *g_alpha, double *g_beta, double *g_gamma,
  aion::SeasonalType seasonal, double eps);
