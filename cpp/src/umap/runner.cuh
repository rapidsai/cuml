...
void _get_strengths(const raft::handle_t& handle,
                    ...
                    UMAPParams* params,
                    raft::device_coo_matrix<float, int, int, uint64_t>& strengths)
{
  ...
  strengths.initialize_sparsity(n_x_n_neighbors);
                ...
                raft::device_coo_matrix<float, int, int, uint64_t>& graph)
{
  ...
  auto fss_graph =
    raft::make_device_coo_matrix<float, int, int, uint64_t>(handle, inputs.n, inputs.n);
  {
    auto strengths =
      raft::make_device_coo_matrix<float, int, int, uint64_t>(handle, inputs.n, inputs.n);
                ...
                value_t* transformed)
{
  ...
  auto graph_coo =
    raft::make_device_coo_matrix<float, int, int, uint64_t>(handle, inputs.n, inputs.n);
  graph_coo.initialize_sparsity(nnz);

  nnz_t to_process =
    static_cast<nnz_t>(graph_coo.structure_view().get_n_rows()) * params->n_neighbors;
  FuzzySimplSetImpl::compute_membership_strength_kernel<value_t, value_idx, nnz_t, TPB_X>
    <<<grid_nnz, blk, 0, stream>>>(knn_graph.knn_indices,
                                   ...
                                   graph_coo.structure_view().get_rows().data(),
  ...
  rmm::device_uvector<value_t> vals_normed(graph_coo.structure_view().get_nnz(), stream);
  RAFT_CUDA_TRY(cudaMemsetAsync(
    vals_normed.data(), 0, graph_coo.structure_view().get_nnz() * sizeof(value_t), stream));
  ...
  raft::sparse::linalg::csr_row_normalize_l1<value_t>(
    row_ind.data(),
    graph_coo.get_elements().data(),
    static_cast<nnz_t>(graph_coo.structure_view().get_nnz()),
    graph_coo.structure_view().get_n_rows(),
    vals_normed.data(),
    stream);

  init_transform<TPB_X, value_t>
    <<<grid_n, blk, 0, stream>>>(graph_coo.structure_view().get_cols().data(),
                                 vals_normed.data(),
                                 graph_coo.structure_view().get_n_rows(),
                                 embedding,
                                 embedding_n,
  ...
  /**
   * Go through raft::device_coo_matrix_view values and set everything that's less than
   * vals.max() / params->n_epochs to 0.0
   */
  thrust::device_ptr<value_t> d_ptr = thrust::device_pointer_cast(graph_coo.get_elements().data());
   ...
   * Remove zeros
   */
  auto comp_coo = raft::make_device_coo_matrix<float, int, int, uint64_t>(handle, 0, 0);
  ...
  SimplSetEmbedImpl::make_epochs_per_sample(comp_coo.get_elements().data(),
                                            comp_coo.structure_view().get_nnz(),
                                            n_epochs,
  ...
  SimplSetEmbedImpl::optimize_layout<value_t, nnz_t, TPB_X>(
    transformed,
    inputs.n,
    embedding,
    orig_x_inputs.n,
    comp_coo.structure_view().get_rows().data(),
    comp_coo.structure_view().get_cols().data(),
    comp_coo.structure_view().get_nnz(),
    epochs_per_sample.data(),
