...
UMAP_API::UMAP_API(UMAPParams *params) : params(params) { knn = nullptr; };

UMAP_API::~UMAP_API() { delete knn; }
...
/**
     ...
     */
void UMAP_API::fit(cumlHandle &handle, float *X, int n, int d,
                   float *embeddings) {
  this->knn = new kNN(handle, d);
  UMAPAlgo::_fit<float, TPB_X>(handle, X, n, d, knn, get_params(), embeddings,
                               handle.getStream());
}

void UMAP_API::fit(cumlHandle &handle, float *X, float *y, int n, int d,
                   float *embeddings) {
  this->knn = new kNN(handle, d);
  UMAPAlgo::_fit<float, TPB_X>(handle, X, y, n, d, knn, get_params(),
                               embeddings, handle.getStream());
}
...
void UMAP_API::transform(cumlHandle &handle, float *X, int n, int d,
  ...
  UMAPAlgo::_transform<float, TPB_X>(handle, X, n, d, embedding, embedding_n,
                                     knn, get_params(), out,
                                     handle.getStream());
}
