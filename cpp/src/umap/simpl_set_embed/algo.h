...
/**
	         ...
	         */
template <typename T>
__device__ __host__ T repulsive_grad(T dist_squared, float gamma,
                                     UMAPParams params) {
...
__device__ __host__ T attractive_grad(T dist_squared, UMAPParams params) {
  T grad_coeff = -2.0 * params.a * params.b * pow(dist_squared, params.b - 1.0);
...
__global__ void optimize_batch_kernel(
  ...
  T *epoch_of_next_negative_sample, T *epoch_of_next_sample, float alpha,
  int epoch, float gamma, uint64_t seed, UMAPParams params) {
