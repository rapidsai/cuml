                     ...
                     cudaStream_t stream)
{
  if (params->build_algo == ML::UMAPParams::graph_build_algo::BRUTE_FORCE_KNN) {
    ...
    // number of columns (n_neightbors) should be smaller than the graph degree computed by nn
    // descent
                     ...
                     cudaStream_t stream)
{
  if (params->build_algo == ML::UMAPParams::graph_build_algo::BRUTE_FORCE_KNN) {  // brute_force_knn
    ...
    // n_neightbors should be smaller than the graph degree computed by nn descent
    assert(n_neighbors <= params->nn_descent_params.graph_degree);
    auto dataset =
