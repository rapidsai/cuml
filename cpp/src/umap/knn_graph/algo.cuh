                     ...
                     cudaStream_t stream)
{
  ...
  if (params->build_algo == ML::UMAPParams::graph_build_algo::BRUTE_FORCE_KNN) {
    ...
    // number of columns (n_neightbors) should be smaller than the graph degree computed by nn descent
    assert(n_neighbors <= params->nn_descent_params.graph_degree);
    auto dataset = raft::make_host_matrix_view<const float, int64_t>(inputsA.X, inputsA.n, inputsA.d);
    auto graph = NNDescent::detail::build<float, int64_t>(handle, params->nn_descent_params, dataset);
                     ...
                     cudaStream_t stream)
{
  if (params->build_algo == ML::UMAPParams::graph_build_algo::BRUTE_FORCE_KNN) {  // brute_force_knn
    ...
    // number of columns (n_neightbors) should be smaller than the graph degree computed by nn descent
    assert(n_neighbors <= params->nn_descent_params.graph_degree);
    auto dataset = raft::make_host_matrix_view<const float, int64_t>(inputsA.data, inputsA.n, inputsA.d);
