...
raftHandle_impl::raftHandle_impl(int n_streams) {
    ...
    _deviceAllocator = std::shared_ptr<rmmAllocatorAdapter>(new rmmAllocatorAdapter(_raftHandle->get_device_allocator()));
}
...
int raftHandle_impl::getDevice() const { 
    return _raftHandle->get_device();
}

void raftHandle_impl::setStream(cudaStream_t stream) { _raftHandle->set_stream(stream); }

cudaStream_t raftHandle_impl::getStream() const { return _raftHandle->get_stream(); }
...
int raftHandle_impl::getNumInternalStreams() const { return _raftHandle->get_num_internal_streams(); }
...
raft::handle_t& raftHandle_impl::getRaftHandle() {
    return *_raftHandle;
}
