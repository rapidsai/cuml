 ...
 * @param coef             pointer to an array for coefficients (size of
 *                         n_cols). This will be filled with coefficients once
 *                         the function is executed.
 * @param intercept        pointer to a scalar for intercept. This will be
 *                         filled once the function is executed
 * @param fit_intercept    boolean parameter to control if the intercept will be
 *                         fitted or not
 * @param batch_size       number of rows in the minibatch
 * @param epochs           number of iterations that the solver will run
 * @param lr_type          type of the learning rate function (i.e. OPTIMAL,
 *                         CONSTANT, INVSCALING, ADAPTIVE)
 * @param eta0             learning rate for contant lr_type. It's used to
 *                         calculate learning rate function for other types of
 *                         lr_type
 * @param power_t          power value in the INVSCALING lr_type
 * @param loss             enum to use different loss functions.
 * @param penalty          None, L1, L2, or Elastic-net penalty
 * @param alpha            alpha value in L1
 * @param l1_ratio         ratio of alpha will be used for L1. (1 - l1_ratio) *
 *                         alpha will be used for L2.
 * @param shuffle          boolean parameter to control whether coordinates will
 *                         be picked randomly or not.
 * @param tol              tolerance to stop the solver
 * @param n_iter_no_change solver stops if there is no update greater than tol
 *                         after n_iter_no_change iterations
