...
void fit_impl(cumlHandle &handle, std::vector<Matrix::Data<T> *> &input_data,
  ...
  if (fit_intercept) {
    ...
    GLM::opg::preProcessData(handle, input_data, input_desc, labels,
                             ...
                             norm2_input.dathttps://github.com/rapidsai/cuml/issues/2470
  ...
  https://github.com/rapidsai/cuml/issues/2470
 ...
 */
  template <typename T>
  void fit_impl(cumlHandle & handle,
                std::vector<Matrix::Data<T> *> & input_data,
                Matrix::PartDescriptor & input_desc,
                std::vector<Matrix::Data<T> *> & labels, T * coef,
                T * intercept, bool fit_intercept, bool normalize, int epochs,
                T alpha, T l1_ratio, bool shuffle, T tol, bool verbose) {
  ...
  void predict_impl(cumlHandle & handle, Matrix::RankSizePair * *rank_sizes,
                    ...
                    size_t n_cols, T * coef, T intercept,
                    Matrix::Data<T> * *preds, bool verbose) {
    int rank = handle.getImpl().getCommunicator().getRank();
  ...
  void fit(
    cumlHandle & handle, std::vector<Matrix::Data<double> *> & input_data,
    Matrix::PartDescriptor & input_desc,
