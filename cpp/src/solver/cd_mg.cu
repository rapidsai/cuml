...
void fit_impl(cumlHandle &handle, std::vector<Matrix::Data<T>*> &input_data,
		...
		bool fit_intercept, bool normalize, int epochs, T alpha, T l1_ratio,
		bool shuffle, T tol, cudaStream_t *streams, int n_streams,
		bool verbose) {
	...
	std::vector<Matrix::RankSizePair*> partsToRanks = input_desc.blocksOwnedBy(comm.getRank());
	...
	if (fit_intercept) {
		...
		GLM::opg::preProcessData(handle, input_data, input_desc, labels,
				...
				fit_intercept, normalize, streams, n_streams, verbose);
	}
	...
	if (normalize) {
		...
		LinAlg::addScalar(squared.data(), squared.data(), l2_alpha,
	...
	for (int i = 0; i < epochs; i++) {
		...
		for (int j = 0; j < input_desc.N; j++) {
			...
			for (int k = 0; k < input_data.size(); k++) {
				...
				input_col_loc = input_data[k]->ptr
						+ (ci * partsToRanks[k]->size);

				input_data_temp[k]->ptr = input_col_loc;
				input_data_temp[k]->totalSize =
						partsToRanks[k]->size;
				...
				LinAlg::add(residual_loc, residual_loc, pred_loc,
						partsToRanks[k]->size,
						streams[k % n_streams]);
	...
	if (fit_intercept) {
		GLM::opg::postProcessData(handle, input_data, input_desc, labels, coef,
				...
				norm2_input.data(), fit_intercept, normalize, streams,
				n_streams, verbose);
	} else {
		*intercept = T(0);
	}
...
void fit_impl(cumlHandle &handle, std::vector<Matrix::Data<T>*> &input_data,
		Matrix::PartDescriptor &input_desc, 
		std::vector<Matrix::Data<T>*> &labels, T *coef, T *intercept, bool fit_intercept,
		bool normalize, int epochs, T alpha, T l1_ratio, bool shuffle, T tol,
		bool verbose) {
...
template<typename T>
void predict_impl(cumlHandle &handle, std::vector<Matrix::Data<T>*> &input_data,
	...
	for (int i = 0; i < input_data.size(); i++) {
		int si = i % n_streams;
		LinAlg::gemm(input_data[i]->ptr, local_blocks[i]->size, input_desc.N,
				coef, preds[i]->ptr, local_blocks[i]->size, size_t(1),
				CUBLAS_OP_N, CUBLAS_OP_N, alpha, beta,
				handle.getImpl().getCublasHandle(), streams[si]);
...
void predict_impl(cumlHandle &handle, Matrix::RankSizePair **rank_sizes,
		size_t n_parts, Matrix::Data<T> **input, size_t n_rows, size_t n_cols,
		T *coef, T intercept, Matrix::Data<T> **preds, bool verbose) {
...
void fit(cumlHandle &handle, std::vector<Matrix::Data<float>*> &input_data,
		Matrix::PartDescriptor &input_desc,
		std::vector<Matrix::Data<float>*> &labels, float *coef, float *intercept,
		bool fit_intercept, bool normalize, int epochs, float alpha,
		float l1_ratio, bool shuffle, float tol, bool verbose) {

	fit_impl(handle, input_data, input_desc, labels, coef,
			intercept, fit_intercept, normalize, epochs, alpha, l1_ratio,
			shuffle, tol, verbose);
}

void fit(cumlHandle &handle, std::vector<Matrix::Data<double>*> &input_data,
		Matrix::PartDescriptor &input_desc, 
		std::vector<Matrix::Data<double>*> &labels, double *coef, double *intercept,
		bool fit_intercept, bool normalize, int epochs, double alpha,
		double l1_ratio, bool shuffle, double tol, bool verbose) {
