                                   ...
                                   double metric_arg)
{
  ...
  switch (metric) {
    case raft::distance::DistanceType::RusselRaoExpanded:
      raft::distance::
        pairwise_distance_impl<double, int, raft::distance::DistanceType::RusselRaoExpanded>(
                                   ...
                                   float metric_arg)
{
  ...
  switch (metric) {
    case raft::distance::DistanceType::RusselRaoExpanded:
      raft::distance::
        pairwise_distance_impl<float, int, raft::distance::DistanceType::RusselRaoExpanded>(
          x, y, dist, m, n, k, workspace, handle.get_stream(), isRowMajor);
      break;
    default: THROW("Unknown or unsupported distance metric '%d'!", (int)metric);
  }
