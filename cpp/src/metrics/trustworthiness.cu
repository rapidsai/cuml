...
long* get_knn_indexes(const cumlHandle& h, math_t* input, int n, int d,
                      int n_neighbors) {
...
/**
    ...
    */
template <typename math_t>
__global__ void compute_rank(math_t* ind_X, long* ind_X_embedded, int n,
...
double trustworthiness_score(const cumlHandle& h, math_t* X, math_t* X_embedded,
  ...
  size_t workspaceSize =
    0;  // EucUnexpandedL2Sqrt does not reauire workspace (may need change for other distances)
  ...
  long* ind_X_embedded = get_knn_indexes(h, X_embedded, n, d, n_neighbors + 1);
  ...
  while (toDo > 0) {
    ...
    sortColumnsPerRow(d_pdist_tmp, d_ind_X_tmp, batchSize, n, bAllocWorkspace,
                      NULL, workspaceSize, stream);
    CUDA_CHECK(cudaPeekAtLastError());
...
template double trustworthiness_score<float, EucUnexpandedL2Sqrt>(
