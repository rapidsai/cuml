...
namespace opg {
  ...
  void reduce(
    ...
    MLCommon::Selection::knn_merge_parts(
        ...
        D->ptr+batch_offset, I->ptr+batch_offset,
      ...
      for(int cur_batch = 0; cur_batch < total_batches; cur_batch++) {
        ...
        if(my_rank_is_idx) {
          ...
          perform_local_knn(res_I.data(), res_D.data(),
              idx_data, idx_desc, local_idx_parts,
              start_indices, stream,
              &*int_streams, handle.getNumInternalStreams(),
        ...
        if(part_rank == my_rank) {
          ...
          reduce(out_I[local_parts_completed], out_D[local_parts_completed],
              res_I, res_D, idx_desc, comm, allocator, stream,
              cur_batch_size, k, local_parts_completed,
              cur_batch, total_n_processed, idxRanks);
