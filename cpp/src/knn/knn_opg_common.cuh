...
struct opg_knn_work {
  opg_knn_work(opg_knn_param<in_t, ind_t, dist_t, out_t> &params,
    ...
    this->local_idx_parts =
      params.idx_desc->blocksOwnedBy(handle.get_comms().get_rank());
...
void opg_knn(opg_knn_param<in_t, ind_t, dist_t, out_t> &params,
  ...
  for (int i = 0; i < params.query_desc->totalBlocks();
    ...
    for (int cur_batch = 0; cur_batch < total_batches;
      ...
      device_buffer<in_t> part_data(handle.get_device_allocator(),
                                    handle.get_stream(), 0);
      ...
      device_buffer<in_t> tmp_batch_buf(handle.get_device_allocator(),
                                        handle.get_stream(), 0);
        ...
        if (!params.rowMajorQuery && total_batches > 1) {
          ...
          for (int col_data = 0; col_data < params.query_desc->N; col_data++) {
            raft::copy(
              ...
              cur_batch_size, handle.get_stream());
          }
      ...
      if (work.my_rank == part_rank || my_rank_is_idx)
        broadcast_query(work, handle, part_rank, cur_query_ptr,
                        batch_input_elms);
      ...
      if (my_rank_is_idx) {
        ...
        if (params.knn_op != knn_operation::knn) {
          ...
          work.res.resize(batch_knn_elms * params.n_outputs,
                          handle.get_stream());
        }
        ...
        if (params.knn_op != knn_operation::knn) {
          ...
          copy_label_outputs_from_index_parts(params, work, handle,
                                              cur_batch_size);
        }
      }
      ...
      if (part_rank == work.my_rank) {
        ...
        reduce(params, work, handle, local_parts_completed, total_n_processed,
               cur_batch_size);
...
void broadcast_query(opg_knn_work<in_t, ind_t, dist_t, out_t> &work,
                     raft::handle_t &handle, int part_rank, in_t *broadcast,
  ...
  if (part_rank == work.my_rank) {  // Either broadcast to other workers
    ...
    for (int rank : work.idxRanks) {
      if (rank != work.my_rank) {
        handle.get_comms().isend(broadcast, broadcast_size, rank, 0,
    ...
    requests.resize(1);
    handle.get_comms().irecv(broadcast, broadcast_size, part_rank, 0,
                             requests.data() + request_idx);
    ++request_idx;
  }
...
void perform_local_knn(opg_knn_param<in_t, ind_t, dist_t, out_t> &params,
                       ...
                       raft::handle_t &handle, in_t *query, size_t query_size) {
...
void copy_label_outputs_from_index_parts(
  ...
  opg_knn_work<in_t, ind_t, dist_t, out_t> &work, raft::handle_t &handle,
  size_t batch_size) {
  const int TPB_X = 256;
  ...
  device_buffer<uint64_t> offsets_d(handle.get_device_allocator(),
                                    handle.get_stream(), n_parts);
  raft::update_device(offsets_d.data(), offsets_h.data(), n_parts,
                      handle.get_stream());

  std::vector<out_t *> parts_h(n_parts);
  device_buffer<out_t *> parts_d(handle.get_device_allocator(),
                                 handle.get_stream(), n_parts);
  for (int o = 0; o < params.n_outputs; o++) {
    for (int p = 0; p < n_parts; p++) {
      parts_h[p] = params.y->at(p)[o];
    }
    raft::update_device(parts_d.data(), parts_h.data(), n_parts,
                        handle.get_stream());

    copy_label_outputs_from_index_parts_kernel<TPB_X, ind_t, out_t>
      <<<grid, blk, 0, handle.get_stream()>>>(
...
void exchange_results(opg_knn_param<in_t, ind_t, dist_t, out_t> &params,
                      opg_knn_work<in_t, ind_t, dist_t, out_t> &work,
                      raft::handle_t &handle, int part_rank,
                      size_t batch_size) {
  ...
  if (part_rank != work.my_rank) {  // Either send local KNN results
    requests.resize(2);
    handle.get_comms().isend(work.res_I.data(), batch_elms, part_rank, 0,
                             requests.data() + request_idx);
    ++request_idx;

    handle.get_comms().isend(work.res_D.data(), batch_elms, part_rank, 0,
                             requests.data() + request_idx);
    ++request_idx;
    ...
    if (params.knn_op != knn_operation::knn) {
      ...
      for (size_t o = 0; o < params.n_outputs; o++) {
        handle.get_comms().isend(work.res.data() + (o * batch_elms), batch_elms,
      ...
      for (int rank : work.idxRanks) {
        if (rank == work.my_rank) {
          ...
          raft::copy_async(work.res_I.data() + batch_offset, work.res_I.data(),
                           batch_elms, handle.get_stream());
          raft::copy_async(work.res_D.data() + batch_offset, work.res_D.data(),
                           batch_elms, handle.get_stream());

          if (params.knn_op != knn_operation::knn) {
            device_buffer<out_t> tmp_res(handle.get_device_allocator(),
                                         handle.get_stream(),
            ...
            for (int o = 0; o < params.n_outputs; ++o) {
              ...
              raft::copy_async(work.res.data() +
                                 (o * work.idxRanks.size() * batch_elms) +
                                 batch_offset,
                               tmp_res.data() + (o * batch_elms), batch_elms,
                               handle.get_stream());
            }
          }
          CUDA_CHECK(cudaStreamSynchronize(handle.get_stream()));
    ...
    for (int rank : work.idxRanks) {
      if (rank != work.my_rank) {
        ...
        // Indices and distances are stored in rank order
        handle.get_comms().irecv(work.res_I.data() + batch_offset, batch_elms,
                                 rank, 0, requests.data() + request_idx);
        ++request_idx;
        handle.get_comms().irecv(work.res_D.data() + batch_offset, batch_elms,
                                 rank, 0, requests.data() + request_idx);
        ++request_idx;
        ...
        if (params.knn_op != knn_operation::knn) {
          for (size_t o = 0; o < params.n_outputs; o++) {
            ...
            handle.get_comms().irecv(r, batch_elms, rank, 0,
                                     requests.data() + request_idx);
            ++request_idx;
          }
        }
...
void reduce(opg_knn_param<in_t, ind_t, dist_t, out_t> &params,
            opg_knn_work<in_t, ind_t, dist_t, out_t> &work,
            raft::handle_t &handle, int part_idx, size_t processed_in_part,
            size_t batch_size) {
  device_buffer<trans_t> trans(handle.get_device_allocator(),
                               handle.get_stream(), work.idxRanks.size());
  CUDA_CHECK(cudaMemsetAsync(trans.data(), 0,
                             work.idxRanks.size() * sizeof(trans_t),
                             handle.get_stream()));
  ...
  device_buffer<ind_t> indices_b(handle.get_device_allocator(),
                                 handle.get_stream());
  device_buffer<dist_t> distances_b(handle.get_device_allocator(),
                                    handle.get_stream());
  ...
  if (params.knn_op != knn_operation::knn) {
    device_buffer<out_t> merged_outputs_b(
      ...
      params.n_outputs * batch_size * params.k);
    // Get the right labels for indices obtained after local KNN searches
    merge_labels(params, work, handle, merged_outputs_b.data(), indices,
    ...
    perform_local_operation(params, work, handle, outputs, probas_with_offsets,
...
void merge_labels(opg_knn_param_t &params, opg_knn_work_t &work,
                  raft::handle_t &handle, out_t *output, ind_t *knn_indices,
  ...
  device_buffer<uint64_t> offsets_d(handle.get_device_allocator(),
                                    handle.get_stream(), offsets_h.size());
  ...
  device_buffer<int> parts_to_ranks_d(handle.get_device_allocator(),
                                      handle.get_stream(),
                                      parts_to_ranks_h.size());
  raft::update_device(parts_to_ranks_d.data(), parts_to_ranks_h.data(),
                      parts_to_ranks_h.size(), handle.get_stream());

  merge_labels_kernel<TPB_X><<<grid, blk, 0, handle.get_stream()>>>(
...
void perform_local_operation(opg_knn_param<in_t, ind_t, dist_t, out_t> &params,
                             opg_knn_work<in_t, ind_t, dist_t, out_t> &work,
                             raft::handle_t &handle, out_t *outputs,
                             std::vector<float *> &probas_with_offsets,
  ...
  MLCommon::Selection::knn_regress<float, 32, true>(
    outputs, nullptr, y, n_labels, batch_size, params.k, handle.get_stream(),
  ...
  switch (params.knn_op) {
    case knn_operation::classification:
      MLCommon::Selection::knn_classify<32, true>(
        ...
        *(params.uniq_labels), *(params.n_unique),
        handle.get_device_allocator(), handle.get_stream(),
