...
namespace knn_common {
  ...
  __global__ void copy_outputs_kernel(T* out,
    int64_t* knn_indices, T** parts, int64_t* offsets,
  ...
  void copy_outputs(T* out, int64_t* knn_indices,
    ...
    for (int o = 0; o < n_outputs; o++) {
      ...
      copy_outputs_kernel<T, TPB_X><<<grid, blk, 0, stream>>>(
        out + (o * n_labels), knn_indices, parts_d.data(),
  ...
  void launch_local_operation(
    T* out, int64_t* knn_indices, std::vector<T*> y,
  ...
  void launch_local_operation<int>(
    int* out, int64_t* knn_indices, std::vector<int*> y,
    size_t total_labels, size_t cur_batch_size, int k,
    const std::shared_ptr<deviceAllocator> alloc,
    cudaStream_t stream, cudaStream_t *int_streams, int n_int_streams,
    bool probas_only, std::vector<float*> *probas,
    std::vector<int*> *uniq_labels, std::vector<int> *n_unique) {
      if (probas_only) {
        MLCommon::Selection::class_probs<32, true>(*probas, nullptr, y,
          total_labels, cur_batch_size, k, *uniq_labels,
          *n_unique, alloc, stream, &int_streams[0], n_int_streams);
      } else {
        MLCommon::Selection::knn_classify<32, true>(out, nullptr, y,
          total_labels, cur_batch_size, k, *uniq_labels,
          *n_unique, alloc, stream, &int_streams[0], n_int_streams);
      }
  }
  ...
  void launch_local_operation<float>(
    float* out, int64_t* knn_indices, std::vector<float*> y,
    size_t total_labels, size_t cur_batch_size, int k,
    const std::shared_ptr<deviceAllocator> alloc,
    cudaStream_t stream, cudaStream_t *int_streams, int n_int_streams,
    bool probas_only, std::vector<float*> *probas,
    std::vector<int*> *uniq_labels, std::vector<int> *n_unique) {
      MLCommon::Selection::knn_regress<float, 32, true>(out, nullptr, y,
          total_labels, cur_batch_size, k,
          stream, &int_streams[0], n_int_streams);
  }

  template <typename T>
  void perform_local_operation(
    T* out, int64_t* knn_indices, T* labels,
    ...
    launch_local_operation<T>(
      out, knn_indices, y,
      total_labels, cur_batch_size, k,
      alloc, stream, int_streams, n_int_streams,
      probas_only, probas,
  ...
  void reduce(
    ...
    CUDA_CHECK(cudaMemsetAsync(trans.data(), 0, idxRanks.size()*sizeof(int64_t), stream));
    ...
    MLCommon::Selection::knn_merge_parts(
      res_D.data(), res_I.data(),
      distances, indices,
      cur_batch_size, idxRanks.size(),
      k, stream, trans.data());

    perform_local_operation(output, indices, res.data(),
                            cur_batch_size, k, n_outputs,
                            handle, probas_only, &probas_with_offsets,
                            uniq_labels, n_unique);
  ...
  // starting indices.
  MLCommon::Selection::brute_force_knn(ptrs, sizes,
      (int)idx_desc.N,
      cur_query_ptr, (int)cur_batch_size,
      res_I, res_D, k, allocator, stream,
      internal_streams, n_internal_streams,
  ...
  void exchange_results(
    ...
    if(part_rank != my_rank) {
       ...
       for (size_t o = 0; o < n_outputs; o++) {
        comm.isend(res.data() + (o * batch_elms), batch_elms,
      ...
      for(int rank : idxRanks) {
        if(rank != my_rank) {
          ...
          comm.irecv(res_I.data() + batch_offset,
                     batch_elms, rank, 0,
                     requests.data() + request_idx);
          ++request_idx;
          comm.irecv(res_D.data() + batch_offset,
                     batch_elms, rank, 0,
                     requests.data() + request_idx);
          ++request_idx;
  ...
  void opg_knn(
    ...
    bool probas_only = false) {
      ...
      for(Matrix::RankSizePair* rsp : idx_desc.partsToRanks) {
        ASSERT(rsp->size >= k, "k must be <= the number of rows in the smallest index partition.");
      }
      ...
      for(int i = 0; i < query_desc.totalBlocks(); i++) {
        ...
        for(int cur_batch = 0; cur_batch < total_batches; cur_batch++) {
           ...
           */
          if(verbose) {
            std::cout << "Rank " << my_rank << ": Performing Broadcast" << std::endl;
          }
          ...
          if(my_rank == part_rank || my_rank_is_idx)
            broadcast_query(cur_query_ptr, batch_input_elms, part_rank, idxRanks, comm, stream);
          ...
          if(my_rank_is_idx) {
             ...
             */
            if(verbose)
              std::cout << "Rank " << my_rank << ": Performing Local KNN" << std::endl;
            ...
            perform_local_knn(res_I.data(), res_D.data(),
                idx_data, idx_desc, local_idx_parts, start_indices,
                stream, int_streams, handle.getNumInternalStreams(),
                handle.getDeviceAllocator(),
                cur_batch_size, k, cur_query_ptr,
                rowMajorIndex, rowMajorQuery);
  
            // Synchronize before running labels copy
            CUDA_CHECK(cudaStreamSynchronize(stream));
  
            copy_outputs(res.data(), res_I.data(), y,
              (size_t)cur_batch_size, (int)k, (int)n_outputs,
              (int)idx_desc.N, my_rank, idx_desc.partsToRanks,
          ...
          if(part_rank == my_rank) {
            ...
            reduce(handle, out, out_I, out_D,
                res, res_I, res_D, idx_desc,
                cur_batch_size, k, n_outputs, local_parts_completed,
                cur_batch, total_n_processed, idxRanks, my_rank,
        ...
        if(my_rank == part_rank)
          local_parts_completed++;
    }
  };


  template void opg_knn<int>(
    ML::cumlHandle &handle,
    ...
    int k, int n_outputs, size_t batch_size, bool verbose,
    std::vector<std::vector<float*>> *probas,
    std::vector<int*> *uniq_labels,
    std::vector<int> *n_unique,
    bool probas_only);

    template void opg_knn<float>(
      ML::cumlHandle &handle,
  ...
  template void reduce<int>(
    ...
    std::vector<Matrix::Data<int64_t>*> *out_I,
    std::vector<Matrix::floatData_t*> *out_D,
    device_buffer<int> &res,
    device_buffer<int64_t> &res_I,
    device_buffer<float> &res_D,
    Matrix::PartDescriptor &index_desc,
    size_t cur_batch_size, int k, int n_outputs, int local_parts_completed,
    int cur_batch, size_t total_n_processed, std::set<int> idxRanks,
    int my_rank, bool probas_only, std::vector<std::vector<float*>> *probas,
    std::vector<int*> *uniq_labels, std::vector<int> *n_unique
  );
  ...
  template void copy_outputs<int>(int* out, int64_t* knn_indices,
    std::vector<std::vector<int*>> &y, size_t cur_batch_size,
    int k, int n_outputs, int n_features, int my_rank,
    std::vector<Matrix::RankSizePair*> &idxPartsToRanks,
    std::shared_ptr<deviceAllocator> alloc, cudaStream_t stream);

  template void copy_outputs<float>(float* out, int64_t* knn_indices,
    std::vector<std::vector<float*>> &y, size_t cur_batch_size,
    int k, int n_outputs, int n_features, int my_rank,
    std::vector<Matrix::RankSizePair*> &idxPartsToRanks,
    std::shared_ptr<deviceAllocator> alloc, cudaStream_t stream);
