...
namespace ML {
	 ...
	 */
	template<typename T = size_t, typename ARR_T>
	inline void chunk_to_device(const ARR_T *ptr, int n, int D,
	    int* devices, ArrayPtr<ARR_T> *output, int n_chunks,
	    cudaStream_t stream) {
	...
	void kneighbors(
	    ...
	    const ArrayPtr<float> *input, int n_params, int D,
	  ...
	  MLCommon::updateDevice(res_D, result_D, k*size_t(n), handle.getImpl().getStream());
	  MLCommon::updateDevice(res_I, result_I, k*size_t(n), handle.getImpl().getStream());
  ...
  class kNN {
		...
		std::vector<ArrayPtr<float>> knn_params;
     ...
     */
		void fit(ArrayPtr<float> *input, int N);
