...
//! @param y series to fit: shape = (nobs, num_bathces) and expects column major data layout
...
//! @param params parameters to evaluate group by series: [mu0, ar.., ma.., mu1, ..]
//! @param trans run `jones_transform` on params
//! @return vector of log likelihood, one for each series (size: num_batches)
